# coding: utf-8

"""
    edu-sharing Repository REST API

    The public restful API of the edu-sharing repository.  # noqa: E501

    The version of the OpenAPI document: 1.1
    Generated by: https://openapi-generator.tech
"""

import re  # noqa: F401
import sys  # noqa: F401
import typing  # noqa: F401
import functools  # noqa: F401

from frozendict import frozendict  # noqa: F401

import decimal  # noqa: F401
from datetime import date, datetime  # noqa: F401
from frozendict import frozendict  # noqa: F401

from edu_sharing_client_api.schemas import (  # noqa: F401
    AnyTypeSchema,
    ComposedSchema,
    DictSchema,
    ListSchema,
    StrSchema,
    IntSchema,
    Int32Schema,
    Int64Schema,
    Float32Schema,
    Float64Schema,
    NumberSchema,
    UUIDSchema,
    DateSchema,
    DateTimeSchema,
    DecimalSchema,
    BoolSchema,
    BinarySchema,
    NoneSchema,
    none_type,
    Configuration,
    Unset,
    unset,
    ComposedBase,
    ListBase,
    DictBase,
    NoneBase,
    StrBase,
    IntBase,
    Int32Base,
    Int64Base,
    Float32Base,
    Float64Base,
    NumberBase,
    UUIDBase,
    DateBase,
    DateTimeBase,
    BoolBase,
    BinaryBase,
    Schema,
    NoneClass,
    BoolClass,
    _SchemaValidator,
    _SchemaTypeChecker,
    _SchemaEnumMaker
)


class Connector(
    DictSchema
):
    """NOTE: This class is auto generated by OpenAPI Generator.
    Ref: https://openapi-generator.tech

    Do not edit the class manually.
    """
    _required_property_names = set((
        'showNew',
    ))
    id = StrSchema
    icon = StrSchema
    showNew = BoolSchema
    
    
    class parameters(
        ListSchema
    ):
        _items = StrSchema
    
    
    class filetypes(
        ListSchema
    ):
    
        @classmethod
        @property
        def _items(cls) -> typing.Type['ConnectorFileType']:
            return ConnectorFileType
    onlyDesktop = BoolSchema
    hasViewMode = BoolSchema


    def __new__(
        cls,
        *args: typing.Union[dict, frozendict, ],
        showNew: showNew,
        id: typing.Union[id, Unset] = unset,
        icon: typing.Union[icon, Unset] = unset,
        parameters: typing.Union[parameters, Unset] = unset,
        filetypes: typing.Union[filetypes, Unset] = unset,
        onlyDesktop: typing.Union[onlyDesktop, Unset] = unset,
        hasViewMode: typing.Union[hasViewMode, Unset] = unset,
        _configuration: typing.Optional[Configuration] = None,
        **kwargs: typing.Type[Schema],
    ) -> 'Connector':
        return super().__new__(
            cls,
            *args,
            showNew=showNew,
            id=id,
            icon=icon,
            parameters=parameters,
            filetypes=filetypes,
            onlyDesktop=onlyDesktop,
            hasViewMode=hasViewMode,
            _configuration=_configuration,
            **kwargs,
        )

from edu_sharing_client_api.model.connector_file_type import ConnectorFileType
