...
def cast_to_allowed_types(arg: typing.Union[str, date, datetime, int, float, None, dict, list, bytes], from_server=False) -> typing.Union[str, bytes, int, float, None, dict, list]:
    """
    ...
    """
    if isinstance(arg, (date, datetime)) and not from_server:
        ...
        return [cast_to_allowed_types(item) for item in arg]
    elif isinstance(arg, tuple):
        return tuple([cast_to_allowed_types(item) for item in arg])
    elif isinstance(arg, dict):
        return {key: cast_to_allowed_types(val) for key, val in arg.items()}
    return arg
...
class InstantiationMetadata:
    ...
    def __init__(
        self,
        path_to_item: typing.Tuple[typing.Union[str, int], ...] = tuple(),
        ...
        Args:
            path_to_item: the path to the current data being instantiated.
                For {'a': [1]} if the code is handling, 1, then the path is ('dict', 'a', 0)
...
class MetaForAllTypes(type):
    """
    This is a shared metaclass for:
    - Enums
    - ComposedSchema classes
    - ListSchema/DictSchema container classed
    - StringSchema/BoolSchema etc primitive classes
    If we did not make this class, then we could not have Enums and XSchemas in composed schemas
    because they would not share a metaclass
    """
    pass

    def __call__(cls, *args, _instantiation_metadata: typing.Optional[InstantiationMetadata] = None, **kwargs):
        """
        When instantiating new class instances,
        convert all str types to BytesWithEncoding before they are passed in to __new__ and __init__
        """
        _instantiation_metadata = InstantiationMetadata() if _instantiation_metadata is None else _instantiation_metadata
        fixed_args = cast_to_allowed_types(args)
        fixed_kwargs = cast_to_allowed_types(kwargs)
        return super().__call__(*fixed_args, _instantiation_metadata=_instantiation_metadata, **fixed_kwargs)


class EnumMeta(MetaForAllTypes):
    """Metaclass for Enum
    ...
    def _convert_(cls, name, module, filter, source=None):
        ...
        new_cls = cls(name, members, module=module)
        new_cls.__reduce_ex__ = _reduce_ex_by_name
        module_globals.update(new_cls.__members__)
        module_globals[name] = new_cls
        return new_cls
