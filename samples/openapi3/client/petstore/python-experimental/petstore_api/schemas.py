...
class EnumMakerBase:
    @classmethod
    def _enum_suffix_classes(cls, value: typing.Any) -> typing.Tuple[type]:
        """
        These are the classes that are added at the end (as the bottom-most)
        classes when manufacturing an enum class from other classes
        """
        assert value in cls._enum_value_to_name
        base_class = type(value)
        if base_class is none_type:
            return (NoneClass,)
        elif base_class is bool:
            return (BoolClass,)
        return (Singleton, base_class)
...
class Schema:
    ...
    def _class_by_base_class(cls, base_cls: type) -> type:
        ...
        if base_cls is bool:
            ...
            raise ValueError(f'Invalid input base_cls={base_cls}')
        log_cache_usage(get_new_class)
        return new_cls

    @classmethod
    def _suffix_classes(cls, value: typing.Any) -> typing.Tuple[type]:
        """
        These are the classes that are added at the end (as the bottom-most)
        classes when manufacturing an enum class from other classes
        """
        base_class = type(value)
        if value is None:
            return (NoneBase, NoneClass,)
        elif isinstance(value, bool):
            return (BoolBase, BoolClass,)
        raise ValueError(f'Invalid input value={value} base_cls={base_cls}')
