...
class ApiClient(object):
        ...
        for auth in auth_settings:
            ...
            if auth_setting:
                if auth_setting['type'] == 'http-signature':
                    # The HTTP signature scheme requires multiple HTTP headers that are calculated dynamically.
                    auth_headers = self.get_http_signature_headers(resource_path, method, headers, body, querys)
    ...
    def get_http_signature_headers(self, resource_path, method, headers, body, query_params):
        """
        Create a message signature for the HTTP request and add the signed headers.
        ...
        signed_headers = {
            'Date': cdate,
            'Host': target_host,
            'Digest': digest_prefix + b64_body_digest.decode('ascii')
        }
        for hdr_key in self.configuration.signed_headers:
            signed_headers[hdr_key] = headers[hdr_key]

        string_to_sign = self.get_str_to_sign(request_target, signed_headers)

        digest, digest_prefix = self.get_message_digest(string_to_sign.encode())
        b64_signed_msg = self.sign_digest(digest)

        auth_header = self.get_authorization_header(signed_headers, b64_signed_msg)

        return {
            'Date': '{0}'.format(cdate),
            'Host': '{0}'.format(target_host),
            'Digest': '{0}{1}'.format(digest_prefix, b64_body_digest.decode('ascii')),
    ...
    def get_message_digest(self, data):
        ...
        if self.configuration.signing_scheme in ["rsa-sha512", "hs2019"]:
            ...
            raise Exception("Unsupported signing algorithm: {0}".format(self.configuration.signing_scheme))
        digest.update(data)
    ...
    def sign_digest(self, digest):
        ...
        if isinstance(privkey, RSA.RsaKey):
            ...
            elif self.configuration.signing_algorithm == 'PKCS1-v1_5':
    ...
    def get_str_to_sign(self, req_tgt, hdrs):
        """
    ...
    def get_authorization_header(self, hdrs, signed_msg):
        ...
        auth_str = auth_str + " " + "keyId=\"" + self.configuration.key_id + "\"," + "algorithm=\"" + self.configuration.signing_scheme + "\"," + "headers=\"(request-target)"
