...
from petstore_api.model_utils import AnyTypeSchema, Schema, Enum
...
class TestCombineNonObjectSchemas(unittest.TestCase):
    ...
    def test_combine_schemas(self):
        ...
        cls_pair_by_expected_cls = {
            (str,): [DateWithValidations, AnyTypeSchema],
            (str,): [DateTimeWithValidations, AnyTypeSchema],
            (str,): [NullableString, StringWithValidation],
        }
        for expected_types, cls_pair in cls_pair_by_expected_cls.items():
            new_cls = type("NewClass", tuple(cls_pair), {})
            assert new_cls._types == expected_types
    ...
    def test_combine_validations(self):
        ...
        combined_validations = Schema._combine_validations(validations_a, validations_b)
        self.assertEqual(combined_validations, {"max_length": 10, "min_length": 5})

        # identical values stay the same
        validations_a = {"max_length": 10}
        combined_validations = Schema._combine_validations(validations_a, validations_a)
        ...
        combined_validations = Schema._combine_validations(validations_b, validations_a)
        self.assertEqual(
            combined_validations,
            {
                'max_length': 10,
        ...
        for (max_var, min_var) in var_pairs:
            with self.assertRaisesRegex(
                ...
                Schema._combine_validations({max_var: 9}, {min_var: 10})
        ...
        for (max_val, min_val) in max_min_pairs:
            with self.assertRaisesRegex(petstore_api.ApiValueError, err_regex.format(max_val, min_val)):
                Schema._combine_validations({'exclusive_maximum': max_val}, {'exclusive_minimum': min_val})
