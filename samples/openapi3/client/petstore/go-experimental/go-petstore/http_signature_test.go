...
func writeRandomTestRsaPemKey(t *testing.T, fileName string, bits int, format keyFormat, passphrase string, alg *x509.PEMCipher) {
	...
	case keyFormatPem:
		if passphrase != "" {
			f.Fatalf("Encrypting PKCS#1-encoded private key with passphrase is not supported")
		}
		privKeyBytes = x509.MarshalPKCS1PrivateKey(key)
	case keyFormatPkcs8Pem:
		privKeyBytes = x509.MarshalPKCS8PrivateKey(key)
	case keyFormatPkcs8Der:
		if passphrase != "" {
			f.Fatalf("Encrypting DER-encoded private key with passphrase is not supported")
		}
		_, err = outFile.Write(x509.MarshalPKCS8PrivateKey(key))
		if err != nil {
			t.Fatalf("Error writing RSA private key: %v", err)
		}
		return
	default:
	...
	if passphrase == "" {
		...
		pemBlock, err = x509.EncryptPEMBlock(rand.Reader, "ENCRYPTED PRIVATE KEY", privKeyBytes, passphrase, *alg)
		if err != nil {
	...
	for _, bits := range []int{1024, 2048, 3072, 4096} {
		...
		for _, format := range []keyFormat{keyFormatPem, keyFormatPkcs8Pem, keyFormatPkcs8Der} {
			...
			for _, alg := range pemCiphers {
				writeRandomTestRsaPemKey(t, privateKeyPath, bits, format, passphrase, alg)
				err := authConfig.LoadPrivateKey(privateKeyPath)
				if err != nil {
					t.Fatalf("Error loading private key: %v", err)
				}
