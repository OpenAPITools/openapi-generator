...
class ModelTests(unittest.TestCase):
    ...
    def test_cat(self):
        self.cat = petstore_client_aiohttp.Cat(className="cat")
    ...
    def test_oneOf(self):
        # test new Pig
        new_pig = petstore_client_aiohttp.Pig()
        self.assertEqual("null", new_pig.to_json())
        self.assertEqual(None, new_pig.actual_instance)

        # test from_json
        json_str = '{"className": "BasquePig", "color": "red"}'
        p = petstore_client_aiohttp.Pig.from_json(json_str)
        self.assertIsInstance(p.actual_instance, petstore_client_aiohttp.BasquePig)

        # test init
        basque_pig = p.actual_instance
        pig2 = petstore_client_aiohttp.Pig(actual_instance=basque_pig)
        self.assertIsInstance(pig2.actual_instance, petstore_client_aiohttp.BasquePig)

        # test failed init
        try:
            pig3 = petstore_client_aiohttp.Pig(actual_instance="123")
        ...
        # failure
        try:
            p2 = petstore_client_aiohttp.Pig.from_json("1")
            self.assertTrue(False)  # this line shouldn't execute
        except ValueError as e:
    ...
    def test_anyOf(self):
        ...
        # test failed init
        try:
            pig3 = petstore_client_aiohttp.AnyOfPig(actual_instance="123")
            self.assertTrue(False)  # this line shouldn't execute
        except ValueError as e:
        ...
        # failure
        try:
            p2 = petstore_client_aiohttp.AnyOfPig.from_json("1")
            self.assertTrue(False)  # this line shouldn't execute
        except ValueError as e:
    ...
    def test_inheritance(self):
        dog = petstore_client_aiohttp.Dog(breed="bulldog", className="dog", color="white")
        self.assertEqual(dog.to_json(), '{"className": "dog", "color": "white", "breed": "bulldog"}')
        self.assertEqual(dog.to_dict(), {'breed': 'bulldog', 'className':
            'dog', 'color': 'white'})
        dog2 = petstore_client_aiohttp.Dog.from_json(dog.to_json())
    ...
    def test_list(self):
        ...
        try:
            l3 = petstore_client_aiohttp.List(var_123_list=123)
            self.assertTrue(False)  # this line shouldn't execute
        except ValueError as e:
            #error_message = (
        ...
        l = petstore_client_aiohttp.List(var_123_list="bulldog")
        self.assertEqual(l.to_json(), '{"123-list":"bulldog"}')
        self.assertEqual(l.to_dict(), {'123-list': 'bulldog'})
        l2 = petstore_client_aiohttp.List.from_json(l.to_json())
        self.assertEqual(l2.var_123_list, 'bulldog')

        self.assertTrue(isinstance(l2, petstore_client_aiohttp.List))

    def test_enum_ref_property(self):
        # test enum ref property
        # test to_json
        d = petstore_client_aiohttp.OuterObjectWithEnumProperty(value=petstore_client_aiohttp.OuterEnumInteger.NUMBER_1)
        self.assertEqual(d.to_json(), '{"value": 1}')
        d2 = petstore_client_aiohttp.OuterObjectWithEnumProperty(value=petstore_client_aiohttp.OuterEnumInteger.NUMBER_1, str_value=petstore_client_aiohttp.OuterEnum.DELIVERED)
        self.assertEqual(d2.to_json(), '{"str_value": "delivered", "value": 1}')
        # test from_json (round trip)
        d3 = petstore_client_aiohttp.OuterObjectWithEnumProperty.from_json(d2.to_json())
        self.assertEqual(d3.str_value, petstore_client_aiohttp.OuterEnum.DELIVERED)
        self.assertEqual(d3.value, petstore_client_aiohttp.OuterEnumInteger.NUMBER_1)
    ...
    def test_float_strict_type(self):
        ...
        a = petstore_client_aiohttp.FormatTest(number=39.8, float=123, byte=bytes("string", 'utf-8'), date="2013-09-17", password="testing09876")
        ...
        f = petstore_client_aiohttp.FormatTest.from_json(json_str)
        self.assertEqual(f.float, 456.0)

    def test_valdiator(self):
        # test regular expression
        a = petstore_client_aiohttp.FormatTest(number=123.45, byte=bytes("string", 'utf-8'), date="2013-09-17", password="testing09876")
        try:
    ...
    def test_constraints(self):
        ...
        color = petstore_client_aiohttp.Color(oneof_schema_1_validator=rgb)
        self.assertEqual(rgb, color.oneof_schema_1_validator)

        color = petstore_client_aiohttp.Color(oneof_schema_2_validator=rgba)
        self.assertEqual(rgba, color.oneof_schema_2_validator)

        color = petstore_client_aiohttp.Color(oneof_schema_3_validator=hex_color)
        self.assertEqual(hex_color, color.oneof_schema_3_validator)

        try:
            petstore_client_aiohttp.Color(oneof_schema_1_validator=rgba)
            self.fail("invalid validation")
        except ValidationError as e:
            self.assertIn("List should have at most 3 items after validation, not 4", str(e))

        try:
            petstore_client_aiohttp.Color(oneof_schema_2_validator=rgb)
            self.fail("invalid validation")
        except ValidationError as e:
            self.assertIn("List should have at least 4 items after validation, not 3", str(e))

        try:
            petstore_client_aiohttp.Color(oneof_schema_3_validator="too long string")
            self.fail("invalid validation")
        except ValidationError as e:
