...
class PetModelTests(unittest.TestCase):
    ...
    def setUp(self):
        self.pet = petstore_client_aiohttp.Pet(name="test name", photoUrls=["string"])
        self.pet.id = 1
        self.pet.status = "available"
        cate = petstore_client_aiohttp.Category(name="dog")
        cate.id = 1
        # cate.name = "dog"
        self.pet.category = cate
        tag = petstore_client_aiohttp.Tag()
        tag.id = 1
        self.pet.tags = [tag]
    ...
    def test_equal(self):
        self.pet1 = petstore_client_aiohttp.Pet(name="test name", photoUrls=["string"])
        self.pet1.id = 1
        self.pet1.status = "available"
        cate1 = petstore_client_aiohttp.Category(name="dog")
        cate1.id = 1
        # cate1.name = "dog"
        self.pet.category = cate1
        tag1 = petstore_client_aiohttp.Tag()
        tag1.id = 1
        self.pet1.tags = [tag1]

        self.pet2 = petstore_client_aiohttp.Pet(name="test name", photoUrls=["string"])
        self.pet2.id = 1
        self.pet2.status = "available"
        cate2 = petstore_client_aiohttp.Category(name="dog")
        cate2.id = 1
        # cate2.name = "dog"
        self.pet.category = cate2
        tag2 = petstore_client_aiohttp.Tag()
        tag2.id = 1
        self.pet2.tags = [tag2]
    ...
    def test_from_to_methods(self):
        json_str = ("{\"category\": {\"id\": 1, \"name\": \"dog\"},\n"
                    ...
                    " \"status\": \"available\",\n"
                    " \"tags\": [{\"id\": 1, \"name\": \"None\"}]}")
        pet = petstore_client_aiohttp.Pet.from_json(json_str)
        ...
        # test from_dict
        pet2 = petstore_client_aiohttp.Pet.from_dict(pet.to_dict())
    ...
    def test_unpack_operator(self):
        ...
        pet = petstore_client_aiohttp.Pet(**d)
    ...
    def test_optional_fields(self):
        pet = petstore_client_aiohttp.Pet(name="required name",
