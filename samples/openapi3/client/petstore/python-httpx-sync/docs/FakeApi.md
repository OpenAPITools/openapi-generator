...
**Synchronous variant:** `fake_any_type_request_body_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_enum_ref_query_parameter_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_health_get_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_http_signature_test_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_outer_boolean_serialize_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_outer_composite_serialize_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_outer_number_serialize_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_outer_string_serialize_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_property_enum_integer_serialize_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_ref_enum_string_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_boolean_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_byte_like_json_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_enum_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_enum_like_json_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_float_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_int_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_list_of_objects_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_str_like_json_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_return_string_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `fake_uuid_example_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_additional_properties_reference_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_body_with_binary_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_body_with_file_schema_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_body_with_query_params_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_client_model_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_date_time_query_parameter_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_empty_and_non_empty_responses_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_endpoint_parameters_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_error_responses_with_model_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_group_parameters_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_inline_additional_properties_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_inline_freeform_additional_properties_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_json_form_data_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_object_for_multipart_requests_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_query_parameter_collection_format_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `test_string_map_reference_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
...
**Synchronous variant:** `upload_file_with_additional_properties_sync(...)` — same parameters and return type, but blocks until completion instead of requiring `await`.
