...
When supplying license metadata, use a valid SPDX expression (for example `MIT`
or `Apache-2.0`); the build backend validates it according to PEP 639.
Synchronous `_sync` variants are also available through `supportHttpxSync=true`.
## Installation & Usage
### pip install
