...
@unittest.skipUnless(hasattr(PetApi, "get_pet_by_id_sync"), "sync wrappers not enabled")
class TestHttpx2Sync(unittest.TestCase):
    def test_repeated_calls_reuse_loop_and_close_client(self):
        ...
        client = ApiClient()
        pool = httpx2.AsyncClient(transport=httpx2.MockTransport(handle))
        client.rest_client.pool_manager = pool
        api = PetApi(client)
        try:
            self.assertEqual(api.get_pet_by_id_sync(1).id, 1)
            self.assertEqual(api.get_pet_by_id_sync(1).id, 1)
