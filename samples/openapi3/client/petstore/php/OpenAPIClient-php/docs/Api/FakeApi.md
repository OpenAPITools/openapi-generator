...
> \OpenAPI\Client\Model\Resource resourceNamedResourceTest()
...
try {
    $result = $apiInstance->resourceNamedResourceTest();
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling FakeApi->resourceNamedResourceTest: ', $e->getMessage(), PHP_EOL;
}
?>
```
