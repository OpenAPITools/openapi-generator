...
class PetModelTests(unittest.TestCase):
    ...
    def setUp(self):
        self.pet = petstore_client.Pet(name="test name", photoUrls=["string"])
        self.pet.id = 1
        self.pet.status = "available"
        cate = petstore_client.Category(name="dog")
        cate.id = 1
        # cate.name = "dog"
        self.pet.category = cate
        tag = petstore_client.Tag()
        tag.id = 1
        self.pet.tags = [tag]
    ...
    def test_equal(self):
        self.pet1 = petstore_client.Pet(name="test name", photoUrls=["string"])
        self.pet1.id = 1
        self.pet1.status = "available"
        cate1 = petstore_client.Category(name="dog")
        cate1.id = 1
        # cate1.name = "dog"
        ...
        self.pet2 = petstore_client.Pet(name="test name", photoUrls=["string"])
        self.pet2.id = 1
        self.pet2.status = "available"
        cate2 = petstore_client.Category(name="dog")
        cate2.id = 1
        # cate2.name = "dog"
    ...
    def test_from_to_methods(self):
        json_str = ("{\"category\": {\"id\": 1, \"name\": \"dog\"},\n"
                    ...
                    " \"status\": \"available\",\n"
                    " \"tags\": [{\"id\": 1, \"name\": \"None\"}]}")
        pet = petstore_client.Pet.from_json(json_str)
        ...
        # test from_dict
        pet2 = petstore_client.Pet.from_dict(pet.to_dict())
    ...
    def test_optional_fields(self):
        _pet = petstore_client.Pet(name="required name",
