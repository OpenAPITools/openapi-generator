...
#
package Something::Deep::AnotherFakeApi;
...
#
# @param string $uuid_test to test uuid example value (required)
...
sub call_123_test_special_tags {
    ...
    # verify the required parameter 'uuid_test' is set
    unless (exists $args{'uuid_test'}) {
      croak("Missing the required parameter 'uuid_test' when calling call_123_test_special_tags");
    }
    ...
    # header params
    if ( exists $args{'uuid_test'}) {
        $header_params->{'uuid_test'} = $self->{api_client}->to_header_value($args{'uuid_test'});
    }
