...
type t = OneOf0 of Child.t | OneOf1 of int32 [@@deriving show, eq]
...
let of_yojson json =
  ...
  | [] ->
      Error
        ("Failed to parse JSON " ^ Yojson.Safe.show json
       ^ " into a value of type Example.t")
  | ts ->
      let parsed_ts =
        ts |> Stdlib.List.map show |> Stdlib.String.concat " | "
      in
      Error
        ("Failed to parse JSON " ^ Yojson.Safe.show json
       ^ " into a value of type Example.t: oneOf should only succeed on one \
          parser, but the JSON was parsed into [" ^ parsed_ts ^ "]")
