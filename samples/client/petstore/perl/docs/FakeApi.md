...
eval { 
    my $result = $api_instance->file_response_test();
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling FakeApi->file_response_test: $@\n";
}
```
...
eval { 
    my $result = $api_instance->resource_named_resource_test();
    print Dumper($result);
};
if ($@) {
    warn "Exception when calling FakeApi->resource_named_resource_test: $@\n";
}
```
