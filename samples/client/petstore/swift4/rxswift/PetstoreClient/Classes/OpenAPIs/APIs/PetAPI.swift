...
open class PetAPI {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func uploadFileWithRequiredFile(petId: Int64, file: URL, additionalMetadata: String? = nil, completion: @escaping ((_ data: ApiResponse?,_ error: Error?) -> Void)) {
        uploadFileWithRequiredFileWithRequestBuilder(petId: petId, file: file, additionalMetadata: additionalMetadata).execute { (response, error) -> Void in
            completion(response?.body, error)
        }
    }
    ...
    /**
     ...
     - returns: Observable<ApiResponse>
     */
    open class func uploadFileWithRequiredFile(petId: Int64, file: URL, additionalMetadata: String? = nil) -> Observable<ApiResponse> {
        return Observable.create { observer -> Disposable in
            uploadFileWithRequiredFile(petId: petId, file: file, additionalMetadata: additionalMetadata) { data, error in
                if let error = error {
                    observer.on(.error(error))
                } else {
                    observer.on(.next(data!))
    ...
    /**
     ...
     - returns: RequestBuilder<ApiResponse> 
     */
    open class func uploadFileWithRequiredFileWithRequestBuilder(petId: Int64, file: URL, additionalMetadata: String? = nil) -> RequestBuilder<ApiResponse> {
        var path = "/pet/{petId}/uploadImageWithRequiredFile"
        let petIdPreEscape = "\(petId)"
        let petIdPostEscape = petIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{petId}", with: petIdPostEscape, options: .literal, range: nil)
        let URLString = PetstoreClientAPI.basePath + path
