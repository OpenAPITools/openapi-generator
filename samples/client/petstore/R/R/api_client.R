  ...
  public = list(
    ...
    CallApi = function(url, method, queryParams, headerParams, body, ...){
      ...
      if (!is.null(self$timeout)) {
        httrtimeout <- httr::timeout(self$timeout)
      }

      if (method == "GET") {
        httr::GET(url, query = queryParams, headers, httrtimeout, ...)
      } else if (method == "POST") {
        httr::POST(url, query = queryParams, headers, body = body, httr::content_type("application/json"), httrtimeout, ...)
      } else if (method == "PUT") {
        httr::PUT(url, query = queryParams, headers, body = body, httr::content_type("application/json"), httrtimeout, ...)
      } else if (method == "PATCH") {
        httr::PATCH(url, query = queryParams, headers, body = body, httr::content_type("application/json"), httrtimeout, ...)
      } else if (method == "HEAD") {
        httr::HEAD(url, query = queryParams, headers, httrtimeout, ...)
      } else if (method == "DELETE") {
        httr::DELETE(url, query = queryParams, headers, httrtimeout, ...)
      } else {
