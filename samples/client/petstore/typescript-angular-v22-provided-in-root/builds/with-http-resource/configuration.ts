...
constructor({ accessToken, apiKeys, basePath, credentials, encodeParam, encoder, password, username, withCredentials }: ConfigurationParameters = {}) {
        ...
        // init default api_key_query credential
        if (!this.credentials['api_key_query']) {
            this.credentials['api_key_query'] = () => {
                if (this.apiKeys === null || this.apiKeys === undefined) {
                    return undefined;
                } else {
                    return this.apiKeys['api_key_query'] || this.apiKeys['api_key'];
                }
            };
        }
