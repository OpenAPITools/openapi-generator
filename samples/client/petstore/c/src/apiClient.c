...
char *assembleTargetUrl(char *basePath, char *operationName,
	...
	if (queryParameters != NULL) {
		...
		neededBufferSizeForQueryParameters +=
			(queryParameters->count *
			 2); // each keyValuePair is separated by a = and a & except the last,
		             // but this makes up for the ? at the beginning
	}
	...
	char *targetUrl =
		malloc(strlen(operationName) + neededBufferSizeForQueryParameters +
		       basePathLength + operationParameterLength + 1);
...
char *assembleHeaderField(char *key, char *value) {
	...
	strcpy(header, key), strcat(header, ": ");
	strcat(header, value);
...
void postData(CURL *handle, char *bodyParameters) {
	...
	curl_easy_setopt(handle, CURLOPT_POSTFIELDSIZE_LARGE, strlen(bodyParameters));
}

void apiClient_invoke(apiClient_t *apiClient, char *operationName,
                      char *operationParameter, list_t *queryParameters,
                      list_t *headerParameters, list_t *formParameters,
                      char *bodyParameters, char *requestType) {
		...
		if (formParameters != NULL) {
			...
			list_ForEach(listEntry, formParameters) {
				...
				if ((keyValuePair->key != NULL) && (keyValuePair->value != NULL)) {
					...
					curl_mime_data(part, keyValuePair->key, CURL_ZERO_TERMINATED);
					curl_mime_name(part, keyValuePair->value);
				}
			}
		...
		list_ForEach(listEntry, headerParameters) {
			...
			if ((keyValuePair->key != NULL) && (keyValuePair->value != NULL)) {
				char *headerValueToWrite =
					assembleHeaderField(keyValuePair->key, keyValuePair->value);
...
// this would only be generated for apiKey authentication
		...
		list_ForEach(listEntry, apiClient->apiKeys) {
			...
			if ((apiKey->key != NULL) && (apiKey->value != NULL)) {
				char *headerValueToWrite =
					assembleHeaderField(apiKey->key, apiKey->value);
				curl_slist_append(headers, headerValueToWrite);
				free(headerValueToWrite);
			}
		}
		...
		char *targetUrl = assembleTargetUrl(apiClient->basePath, operationName,
		                                    operationParameter, queryParameters);

		curl_easy_setopt(handle, CURLOPT_URL, targetUrl);
		curl_easy_setopt(handle, CURLOPT_WRITEFUNCTION, writeDataCallback);
		curl_easy_setopt(handle, CURLOPT_WRITEDATA, &apiClient->dataReceived);
...
// this would only be generated for OAuth2 authentication
		...
		if (apiClient->accessToken != NULL) {
			...
			curl_easy_setopt(handle, CURLOPT_XOAUTH2_BEARER, apiClient->accessToken);
		}
#endif
...
// this would only be generated for basic authentication:
		...
		if ((apiClient->username != NULL) && (apiClient->password != NULL)) {
			authenticationToken =
				malloc(strlen(apiClient->username) + strlen(apiClient->password) + 2);
			sprintf(authenticationToken, "%s:%s", apiClient->username,
			        apiClient->password);

			curl_easy_setopt(handle, CURLOPT_HTTPAUTH, CURLAUTH_BASIC);
			curl_easy_setopt(handle, CURLOPT_USERPWD, authenticationToken);
		}
