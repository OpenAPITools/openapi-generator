...
FakeApi <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return void
    fake_path_array = function(path_array, ..., .parse = TRUE) {
      local_var_response <- self$fake_path_array_with_http_info(path_array, ..., .parse = .parse)
      if (local_var_response$status_code >= 200 && local_var_response$status_code <= 299) {
        return(local_var_response$content)
      } else if (local_var_response$status_code >= 300 && local_var_response$status_code <= 399) {
        return(local_var_response)
    ...
    fake_path_array_with_http_info = function(path_array, ..., .parse = TRUE) {
      args <- list(...)
      query_params <- list()
      header_params <- c()
      form_params <- list()
