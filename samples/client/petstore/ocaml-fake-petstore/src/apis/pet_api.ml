...
let delete_pet ~pet_id ?api_key () =
    ...
    let headers = Request.default_headers in
    let headers = Request.maybe_add_header headers "api_key" (fun x -> x)
 api_key in
...
let find_pets_by_tags ~tags =
    ...
    let headers = Request.default_headers in
    let uri = Request.add_query_param_list uri "tags" (Stdlib.List.map (fun x -> x)
)
 tags in
    Cohttp_lwt_unix.Client.call `GET uri ~headers >>= fun (resp, body) ->
    Request.read_json_body_as_list_of (JsonSupport.unwrap Pet.of_yojson) resp body
...
let update_pet_with_form ~pet_id ?name ?status () =
 ...
 pet_id in
    ...
    let body = Request.maybe_add_form_encoded_body_param body "name" (fun x -> x)
 name in
    let body = Request.maybe_add_form_encoded_body_param body "status" (fun x -> x)
 status in
...
let upload_file ~pet_id ?additional_metadata ?file () =
 ...
 pet_id in
    let body = Request.init_form_encoded_body () in
    let body = Request.maybe_add_form_encoded_body_param body "additional_metadata" (fun x -> x)
 additional_metadata in
    let body = Request.maybe_add_form_encoded_body_param body "file" (fun x -> x)
 file in
    let body = Request.finalize_form_encoded_body body in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
...
let upload_file_with_required_file ~pet_id ~required_file ?additional_metadata () =
 ...
 pet_id in
    ...
    let body = Request.maybe_add_form_encoded_body_param body "additional_metadata" (fun x -> x)
 additional_metadata in
    let body = Request.add_form_encoded_body_param body "required_file" (fun x -> x)
 required_file in
    let body = Request.finalize_form_encoded_body body in
    Cohttp_lwt_unix.Client.call `POST uri ~headers ~body >>= fun (resp, body) ->
    Request.read_json_body_as (JsonSupport.unwrap Api_response.of_yojson) resp body
