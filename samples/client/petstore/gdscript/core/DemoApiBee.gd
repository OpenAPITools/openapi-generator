...
# These are constants.
# To set different values at runtime, use the bee_xxxx properties below.
...
var bee_port := BEE_DEFAULT_PORT_HTTP
...
var bee_polling_interval_ms := BEE_DEFAULT_POLLING_INTERVAL_MS  # milliseconds
...
func bee_enable_ssl():
	...
	self.bee_port = BEE_DEFAULT_PORT_HTTPS


func bee_disable_ssl():
	self.__bee_ssl_enabled = false
	self.bee_port = BEE_DEFAULT_PORT_HTTP
...
func bee_connect_client_if_needed(
	...
	while (
		...
		print("Connecting...")
		if self.bee_polling_interval_ms:
