...
# Required: False
# isArray: false
@export var category: DemoCategory:
	set(value):
...
@export var status: String = "":
	set(value):
		...
		if str(value) != "" and not (str(value) in __status__allowable__values):
			push_error("DemoPet: tried to set property `status` to a value that is not allowed." +
				"  Allowed values: `available`, `pending`, `sold`")
			return
...
static func bzz_denormalize_single(from_dict: Dictionary):
	...
	if from_dict.has("category"):
		me.category = DemoCategory.bzz_denormalize_single(from_dict["category"])
	...
	if from_dict.has("tags"):
		me.tags = DemoTag.bzz_denormalize_multiple(from_dict["tags"])
