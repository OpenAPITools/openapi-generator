...
module Petstore
  class Configuration
    ...
    def apply_auth!(headers : HTTP::Headers, params : Hash(String, String | Array(String)), auth_names : Array(String)) : Nil
      if auth_names.includes?("petstore_auth")
        if token = @access_token
          headers["Authorization"] = "Bearer #{token}"
        end
      end
      if auth_names.includes?("api_key")
        value = api_key_with_prefix(:"api_key")
        unless value.empty?
          headers["api_key"] = value
        end
      end
      if auth_names.includes?("api_key_query")
        value = api_key_with_prefix(:"api_key_query")
        unless value.empty?
          params["api_key_query"] = value
        end
      end
      if auth_names.includes?("http_basic_test")
        if @username || @password
          headers["Authorization"] = basic_auth_token
        end
      end
      if auth_names.includes?("bearer_test")
        if token = @access_token
          headers["Authorization"] = "Bearer #{token}"
        end
      end
