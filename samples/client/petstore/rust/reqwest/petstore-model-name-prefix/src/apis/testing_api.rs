...
pub fn tests_inline_enum_boxing_get(configuration: &configuration::Configuration, status: Option<&str>) -> Result<Vec<models::FooModelWithInlineEnum>, Error<TestsInlineEnumBoxingGetError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `Vec&lt;models::FooModelWithInlineEnum&gt;`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `Vec&lt;models::FooModelWithInlineEnum&gt;`")))),
        }
    } else {
        let content = resp.text()?;
...
pub fn tests_inline_enum_boxing_post(configuration: &configuration::Configuration, foo_model_with_inline_enum: models::FooModelWithInlineEnum) -> Result<models::FooModelWithInlineEnum, Error<TestsInlineEnumBoxingPostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_foo_model_with_inline_enum = foo_model_with_inline_enum;
    ...
    req_builder = req_builder.json(&p_body_foo_model_with_inline_enum);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::FooModelWithInlineEnum`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::FooModelWithInlineEnum`")))),
        }
    } else {
        let content = resp.text()?;
