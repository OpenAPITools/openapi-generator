...
use petstore_reqwest::apis::{configuration::Configuration, testing_api};
use petstore_reqwest::models::ModelWithInlineEnum;
...
async fn test_inline_enum_query_param_draft() {
    ...
    let result = testing_api::tests_inline_enum_boxing_get(&config, Some("draft")).await;
...
async fn test_all_inline_enum_values() {
    ...
    for status in ["draft", "published", "archived"] {
        let _ = testing_api::tests_inline_enum_boxing_get(&config, Some(status)).await;
    }
...
async fn test_optional_enum_none() {
    ...
    let result = testing_api::tests_inline_enum_boxing_get(&config, None).await;
...
async fn test_regression_exact_url_inspection() {
    ...
    let _ = testing_api::tests_inline_enum_boxing_get(&config, Some("published")).await;
