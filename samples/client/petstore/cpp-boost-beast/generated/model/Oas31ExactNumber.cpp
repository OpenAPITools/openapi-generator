...
ExactNumber::ExactNumber(Integer mantissa, Integer exponent10)
    : mantissa_(std::move(mantissa)), exponent10_(std::move(exponent10)) {
    normalize();
}

ExactNumber ExactNumber::fromInt(std::int64_t value) {
    return ExactNumber(Integer(value), 0);
}

ExactNumber ExactNumber::fromUint(std::uint64_t value) {
    return ExactNumber(Integer(value), 0);
}

ExactNumber::Integer ExactNumber::ipow10(std::size_t exponent) {
    Integer result = 1;
    Integer base = 10;
    while (exponent != 0) {
...
ExactNumber::Integer ExactNumber::gcd(Integer left, Integer right) {
    if (left < 0) left = -left;
    if (right < 0) right = -right;
    while (right != 0) {
        Integer const remainder = left % right;
        left = std::move(right);
        right = remainder;
    }
    return left;
...
std::size_t ExactNumber::boundedDistance(Integer const& distance) {
...
std::size_t ExactNumber::decimalDigits(Integer const& positive) {
    return positive.str().size();
}

void ExactNumber::normalize() {
    if (mantissa_ == 0) {
        exponent10_ = 0;
        return;
    }
...
ExactNumber ExactNumber::parseLexeme(std::string const& lexeme) {
    if (lexeme.size() > kMaxLexemeLength) {
        throw std::length_error(
    ...
    if (lexeme[index] == '0') {
        digits.push_back(lexeme[index++]);
        if (index < lexeme.size() && lexeme[index] >= '0' && lexeme[index] <= '9') {
            throw std::domain_error("ExactNumber::parseLexeme: leading zero");
        }
    } else if (lexeme[index] >= '1' && lexeme[index] <= '9') {
        do {
            digits.push_back(lexeme[index++]);
        } while (index < lexeme.size()
                 && lexeme[index] >= '0' && lexeme[index] <= '9');
    } else {
        throw std::domain_error("ExactNumber::parseLexeme: missing integer digit");
    }
    ...
    if (index < lexeme.size() && lexeme[index] == '.') {
        ++index;
        std::size_t const fractionStart = index;
        while (index < lexeme.size()
               && lexeme[index] >= '0' && lexeme[index] <= '9') {
            digits.push_back(lexeme[index++]);
            ++fractionDigits;
    ...
    if (index < lexeme.size()
            && (lexeme[index] == 'e' || lexeme[index] == 'E')) {
        ++index;
        bool exponentNegative = false;
        if (index < lexeme.size()
                && (lexeme[index] == '+' || lexeme[index] == '-')) {
            exponentNegative = lexeme[index] == '-';
            ++index;
        }
        std::size_t const exponentStart = index;
        while (index < lexeme.size()
               && lexeme[index] >= '0' && lexeme[index] <= '9') {
            explicitExponent *= 10;
            explicitExponent += static_cast<unsigned>(lexeme[index] - '0');
            ++index;
        }
    ...
    if (index != lexeme.size()) {
        throw std::domain_error(
            "ExactNumber::parseLexeme: trailing characters after lexeme");
    }
...
ExactNumber ExactNumber::fromDouble(double value) {
...
int ExactNumber::compare(ExactNumber const& other) const {
...
ExactNumber ExactNumber::add(ExactNumber const& other) const {
...
ExactNumber ExactNumber::mul(ExactNumber const& other) const {
    return ExactNumber(mantissa_ * other.mantissa_,
                       exponent10_ + other.exponent10_);
}

bool ExactNumber::isInteger() const {
    return isZero() || exponent10_ >= 0;
}

bool ExactNumber::isMultipleOf(ExactNumber const& divisor) const {
    if (divisor.isZero()) {
...
void ExactNumber::divmod(ExactNumber const& divisor,
    ...
    Integer const quotientValue = numerator / denominator;
    Integer const remainderValue = numerator % denominator;
    quotient = ExactNumber(quotientValue, 0);
    remainder = ExactNumber(remainderValue, commonExponent);
}

std::string ExactNumber::toString() const {
    if (isZero()) return "0";
...
bool isPositiveMultipleOf(ExactNumber const& value) {
    return !value.isZero() && value.mantissa() > 0;
}
