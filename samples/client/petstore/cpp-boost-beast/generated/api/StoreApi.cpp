...
StoreApi::deleteOrder(
    ...
    std::string path = operationServerPrefix(m_context, "http://petstore.swagger.io/v2") + "/store/order/{orderId}";
    std::map<std::string, std::string> headers;
    // Path parameters use their declared simple, label, or matrix style.
    replacePathParameter(path, "orderId", orderId, "simple", false);
...
StoreApi::getInventory(
    ...
    std::string serializedRequestBody;
    std::string path = operationServerPrefix(m_context, "http://petstore.swagger.io/v2") + "/store/inventory";
    std::map<std::string, std::string> headers;
    ...
    // Security alternatives are OR groups containing AND-required schemes.
    {
        ...
        applyOperationSecurity("getInventory", operationSecurity, path, headers);
    }
    auto statusCode = boost::beast::http::status::unknown;
    std::string responseBody;
    try {
...
StoreApi::placeOrder(
    ...
    std::string serializedRequestBody;
    std::string path = operationServerPrefix(m_context, "http://petstore.swagger.io/v2") + "/store/order";
    std::map<std::string, std::string> headers;
