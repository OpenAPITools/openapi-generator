...
/// Incremental Server-Sent Events framer (WHATWG HTML SSE).
/// Feeds raw body bytes and invokes onEvent once per complete event with the
/// event's data payload (multi-line data joined by LF). Incomplete events at
/// EOF are discarded. Comment lines and non-data fields are ignored for the
/// callback payload.
class SseEventFramer {
public:
    explicit SseEventFramer(std::function<void(const std::string &)> onEvent)
        : m_onEvent(std::move(onEvent)) {}

    void feed(const std::string_view chunk) {
...
private:
    bool consumeInitialByteOrderMark() {
        ...
        if (m_pending.size() < utf8ByteOrderMark.size()) {
            return false;
        }
    ...
    void drain(const bool endOfStream) {
        std::size_t cursor = 0;
        while (cursor < m_pending.size()) {
            std::size_t lineEnd = cursor;
            bool foundEol = false;
            while (lineEnd < m_pending.size()) {
                const char ch = m_pending[lineEnd];
                if (ch == '\n') {
                    foundEol = true;
                    break;
                }
                if (ch == '\r') {
                    // Hold a trailing CR until we know whether LF follows (CRLF).
                    if (lineEnd + 1 == m_pending.size() && !endOfStream) {
                        break;
                    }
                    foundEol = true;
                    break;
            ...
            const std::string line = m_pending.substr(cursor, lineEnd - cursor);
            cursor = lineEnd + 1;
...
streamBody(SyncReadStream &stream,
    ...
    parser.body_limit(responseBodyLimit);

    throwOperationError(
        runTimedOperation(ioContext, operationTimeout, "read header",
            [&stream, &readBuf, &parser](
    ...
    while (!parser.is_done()) {
        parser.get().body().data = bodyBuf.data();
        parser.get().body().size = bodyBuf.size();
        ...
        // end_of_stream is expected when the body is delimited by
        // connection close; the parser has consumed what was sent.
...
HttpClientImpl::executeStream(
    ...
    if (m_transport == Transport::Https) {
        return executeHttpsStream(request, std::move(onEvent));
    }
