...
# **updateImageFile**
```swift
    open class func updateImageFile( petId: Int64,  imageData: Data? = nil) -> Promise<ApiResponse>
    open class func updateImageFile(petId: Int64, imageData: Data? = nil) -> Observable<ApiResponse>
```
...
// updates an existing image
PetAPI.updateImageFile(petId: petId, imageData: imageData).then {
         // when the promise is fulfilled
     }.always {
         // regardless of whether the promise is fulfilled, or rejected
     }.catch { errorType in
