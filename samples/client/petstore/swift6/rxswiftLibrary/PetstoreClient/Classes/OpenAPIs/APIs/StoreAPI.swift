...
open class StoreAPI {
     ...
     */
    open class func deleteOrder(orderId: String, client: OpenAPIClient = OpenAPIClient.shared) -> Observable<Void> {
        return Observable.create { observer -> Disposable in
            let requestTask = self.deleteOrderWithRequestBuilder(orderId: orderId, client: client).execute { result in
                switch result {
                case .success:
                    observer.onNext(())
     ...
     */
    open class func getInventory(client: OpenAPIClient = OpenAPIClient.shared) -> Observable<[String: Int]> {
        return Observable.create { observer -> Disposable in
            let requestTask = self.getInventoryWithRequestBuilder(client: client).execute { result in
                switch result {
     ...
     */
    open class func getOrderById(orderId: Int64, client: OpenAPIClient = OpenAPIClient.shared) -> Observable<Order> {
        return Observable.create { observer -> Disposable in
            let requestTask = self.getOrderByIdWithRequestBuilder(orderId: orderId, client: client).execute { result in
                switch result {
                case let .success(response):
                    observer.onNext(response.body)
                case let .failure(error):
    ...
    /**
     ...
     - returns: Observable<Order>
     */
    open class func placeOrder(body: Order, client: OpenAPIClient = OpenAPIClient.shared) -> Observable<Order> {
        return Observable.create { observer -> Disposable in
            let requestTask = self.placeOrderWithRequestBuilder(body: body, client: client).execute { result in
                switch result {
                case let .success(response):
                    observer.onNext(response.body)
                case let .failure(error):
