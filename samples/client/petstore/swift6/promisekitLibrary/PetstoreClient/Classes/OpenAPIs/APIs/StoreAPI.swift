...
open class StoreAPI {
     ...
     */
    open class func deleteOrder(orderId: String, client: OpenAPIClient = OpenAPIClient.shared) -> Promise<Void> {
        let deferred = Promise<Void>.pending()
        deleteOrderWithRequestBuilder(orderId: orderId, client: client).execute { result in
            switch result {
            case .success:
     ...
     */
    open class func getInventory(client: OpenAPIClient = OpenAPIClient.shared) -> Promise<[String: Int]> {
        let deferred = Promise<[String: Int]>.pending()
        getInventoryWithRequestBuilder(client: client).execute { result in
            switch result {
     ...
     */
    open class func getOrderById(orderId: Int64, client: OpenAPIClient = OpenAPIClient.shared) -> Promise<Order> {
        let deferred = Promise<Order>.pending()
        getOrderByIdWithRequestBuilder(orderId: orderId, client: client).execute { result in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body)
            case let .failure(error):
    ...
    /**
     ...
     - returns: Promise<Order>
     */
    open class func placeOrder(body: Order, client: OpenAPIClient = OpenAPIClient.shared) -> Promise<Order> {
        let deferred = Promise<Order>.pending()
        placeOrderWithRequestBuilder(body: body, client: client).execute { result in
            switch result {
            case let .success(response):
                deferred.resolver.fulfill(response.body)
            case let .failure(error):
