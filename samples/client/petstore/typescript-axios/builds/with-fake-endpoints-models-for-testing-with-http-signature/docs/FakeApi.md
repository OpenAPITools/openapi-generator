...
let queryUnique: ApiModule.Set<string> = ; // Set<string> | 
let headerUnique: ApiModule.Set<string> = ; // Set<string> | 

apiInstance.testUniqueItemsHeaderAndQueryParameterCollectionFormat(queryUnique, headerUnique).then((data: any) => {
  console.log('API called successfully. Returned data: ', data);
}).catch((error: any) => {
  console.error(error);
});
