...
let pet: ApiModule.Pet = ; // Pet | Pet object that needs to be added to the store
...
let petId: number = 789; // number | ID of pet to update
let requiredFile: File = BINARY_DATA_HERE; // File | file to upload
const additionalMetadata: ApiModule.string = {
  // set here some attributes...
};  // string | Additional data to pass to server

apiInstance.uploadFileWithRequiredFile(petId, requiredFile, additionalMetadata).then((data: any) => {
  console.log('API called successfully. Returned data: ', data);
}).catch((error: any) => {
  console.error(error);
});
