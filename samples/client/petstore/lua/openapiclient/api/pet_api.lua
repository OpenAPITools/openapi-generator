...
-- model import
local openapiclient_api_response = require "openapiclient.model.api_response"
local openapiclient_pet = require "openapiclient.model.pet"
...
function pet_api:get_pet_by_id(pet_id)
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_pet.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
...
function pet_api:upload_file(pet_id, additional_metadata, file)
	...
	if http_status:sub(1,1) == "2" then
		...
		return openapiclient_api_response.cast(result), headers
	else
		local body, err, errno2 = stream:get_body_as_string()
		if not body then
			return nil, err, errno2
