...
function pet_api:find_pets_by_tags(tags, max_count)
	local req = http_request.new_from_uri({
		scheme = self.default_scheme;
		host = self.host;
		port = self.port;
		path = string.format("%s/pet/findByTags?tags=%s&maxCount=%s",
			self.basePath, http_util.encodeURIComponent(tags), http_util.encodeURIComponent(max_count));
	})
