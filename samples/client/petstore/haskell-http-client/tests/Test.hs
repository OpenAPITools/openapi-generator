...
main =
  hspec $ modifyMaxSize (const 10) $ do
    describe "JSON instances" $ do
      ...
      propMimeEq MimeJSON (Proxy :: Proxy TestEndpointParametersBody)
      propMimeEq MimeJSON (Proxy :: Proxy TestEnumParametersBody)
      propMimeEq MimeJSON (Proxy :: Proxy TestJsonFormDataBody)
      propMimeEq MimeJSON (Proxy :: Proxy TypeHolderDefault)
      propMimeEq MimeJSON (Proxy :: Proxy TypeHolderExample)
      propMimeEq MimeJSON (Proxy :: Proxy UpdatePetWithFormBody)
      propMimeEq MimeJSON (Proxy :: Proxy UploadFileBody)
      propMimeEq MimeJSON (Proxy :: Proxy UploadFileWithRequiredFileBody)
