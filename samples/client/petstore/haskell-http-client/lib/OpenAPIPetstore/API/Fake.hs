...
-- 
fakeHealthGet 
  :: OpenAPIPetstoreRequest FakeHealthGet MimeNoContent HealthCheckResult MimeJSON
fakeHealthGet =
  _mkRequest "GET" ["/fake/health"]

data FakeHealthGet  
-- | @application/json@
instance Produces FakeHealthGet MimeJSON
...
-- 
-- AuthMethod: 'AuthBasicHttpSignatureTest'
-- 
fakeHttpSignatureTest 
  :: (Consumes FakeHttpSignatureTest contentType, MimeRender contentType Pet)
  => ContentType contentType -- ^ request content-type ('MimeType')
  -> Pet -- ^ "pet" -  Pet object that needs to be added to the store
  -> OpenAPIPetstoreRequest FakeHttpSignatureTest contentType NoContent MimeNoContent
fakeHttpSignatureTest _ pet =
  _mkRequest "GET" ["/fake/http-signature-test"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthBasicHttpSignatureTest)
    `setBodyParam` pet

data FakeHttpSignatureTest 

-- | /Body Param/ "Pet" - Pet object that needs to be added to the store
instance HasBodyParam FakeHttpSignatureTest Pet 

-- | /Optional Param/ "query_1" - query parameter
instance HasOptionalParam FakeHttpSignatureTest Query1 where
  applyOptionalParam req (Query1 xs) =
    req `setQuery` toQuery ("query_1", Just xs)

-- | /Optional Param/ "header_1" - header parameter
instance HasOptionalParam FakeHttpSignatureTest Header1 where
  applyOptionalParam req (Header1 xs) =
    req `setHeader` toHeader ("header_1", xs)

-- | @application/xml@
instance Consumes FakeHttpSignatureTest MimeXML
-- | @application/json@
instance Consumes FakeHttpSignatureTest MimeJSON

instance Produces FakeHttpSignatureTest MimeNoContent
...
-- Fake endpoint to test group parameters (optional)
-- 
-- AuthMethod: 'AuthBasicBearerTest'
-- 
testGroupParameters 
...
testGroupParameters (RequiredStringGroup requiredStringGroup) (RequiredBooleanGroup requiredBooleanGroup) (RequiredInt64Group requiredInt64Group) =
  _mkRequest "DELETE" ["/fake"]
    `_hasAuthType` (P.Proxy :: P.Proxy AuthBasicBearerTest)
...
testQueryParameterCollectionFormat (Pipe pipe) (Ioutil ioutil) (Http http) (Url url) (Context context) =
  _mkRequest "PUT" ["/fake/test-query-paramters"]
    `setQuery` toQueryColl MultiParamArray ("pipe", Just pipe)
