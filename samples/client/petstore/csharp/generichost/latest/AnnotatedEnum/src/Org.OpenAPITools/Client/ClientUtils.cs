    ...
    public static partial class ClientUtils
    {
        ...
        public static string? ParameterToString(object? obj, string? format = ISO8601_DATETIME_FORMAT)
        {
            ...
            if (obj is Number number)
                return NumberValueConverter.ToJsonValue(number);
            if (obj is Number2 number2)
                return Number2ValueConverter.ToJsonValue(number2);
            if (obj is SingleAnyOfTest singleAnyOfTest)
                return SingleAnyOfTestValueConverter.ToJsonValue(singleAnyOfTest);
            if (obj is TypeIntegerWithOneOf typeIntegerWithOneOf)
                return TypeIntegerWithOneOfValueConverter.ToJsonValue(typeIntegerWithOneOf).ToString();
