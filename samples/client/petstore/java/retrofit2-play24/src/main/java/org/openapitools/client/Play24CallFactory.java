...
public class Play24CallFactory implements okhttp3.Call.Factory {
  ...
  public Play24CallFactory(WSClient wsClient) { this.wsClient = wsClient; }
  ...
  public Call newCall(Request request) {
    ...
    if (!this.extraQueryParams.isEmpty()) {
      ...
      for (Pair queryParam : this.extraQueryParams) {
        String param =
            String.format("%s=%s", queryParam.getName(), queryParam.getValue());
        if (newQuery == null) {
          newQuery = param;
        } else {
          newQuery += "&" + param;
      ...
      URI newUri;
      try {
        newUri = new URI(request.url().uri().getScheme(),
                         request.url().uri().getAuthority(),
                         request.url().uri().getPath(), newQuery,
  ...
  static class PlayWSCall implements Call {
      ...
      promise.onRedeem(new F.Callback<WSResponse>() {
        ...
        public void invoke(WSResponse wsResponse) throws Throwable {
          responseCallback.onResponse(call,
                                      PlayWSCall.this.toWSResponse(wsResponse));
        }
      });
    ...
    private Response toWSResponse(final WSResponse r) {
      ...
      builder.request(request).code(r.getStatus()).body(new ResponseBody() {
        @Override
      ...
      for (Map.Entry<String, List<String>> entry :
           r.getAllHeaders().entrySet()) {
        for (String value : entry.getValue()) {
          builder.addHeader(entry.getKey(), value);
        }
      }
      for (final WSCookie cookie : r.getCookies()) {
        builder.addHeader("Cookie", String.format("%s=%s", cookie.getName(),
                                                  cookie.getValue()));
      }
