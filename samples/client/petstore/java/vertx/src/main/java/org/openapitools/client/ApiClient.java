...
public class ApiClient {
  ...
  private static final OpenOptions FILE_DOWNLOAD_OPTIONS =
      new OpenOptions().setCreate(true).setTruncateExisting(true);
  ...
  public MultiMap getDefaultHeaders() { return defaultHeaders; }
  ...
  public MultiMap getDefaultCookies() { return defaultHeaders; }
  ...
  private boolean isJsonMime(String mime) {
    ...
    return mime != null &&
        (mime.matches(jsonMime) ||
         mime.equalsIgnoreCase("application/json-patch+json"));
  }
  ...
  public void
  sendBody(HttpRequest<Buffer> request,
   ...
   */
  public <T> void invokeAPI(String path, String method, List<Pair> queryParams,
                            Object body, MultiMap headerParams,
                            MultiMap cookieParams,
                            Map<String, Object> formParams, String[] accepts,
                            String[] contentTypes, String[] authNames,
    ...
    if (contentTypes != null) {
      headerParams.add(HttpHeaders.CONTENT_TYPE,
                       selectHeaderContentType(contentTypes));
    }

    HttpMethod httpMethod = HttpMethod.valueOf(method);
    HttpRequest<Buffer> request =
        getWebClient().requestAbs(httpMethod, basePath + path);
    ...
    final MultiMap cookies = MultiMap.caseInsensitiveMultiMap()
                                 .addAll(cookieParams)
                                 .addAll(defaultCookies);
    request.putHeader("Cookie", buildCookieHeader(cookies));

    Handler<AsyncResult<HttpResponse<Buffer>>> responseHandler =
        buildResponseHandler(returnType, resultHandler);
    if (body != null) {
      sendBody(request, responseHandler, body);
    } else if (formParams != null && !formParams.isEmpty()) {
      Map<String, String> formMap =
          formParams.entrySet().stream().collect(toMap(
              Map.Entry::getKey, entry -> parameterToString(entry.getValue())));
  ...
  private String buildCookieHeader(MultiMap cookies) {
    ...
    for (final Map.Entry<String, String> entry : cookies.entries()) {
      if (entry.getValue() != null) {
        cookieValue.append(String.format("%s%s=%s", delimiter, entry.getKey(),
                                         entry.getValue()));
        delimiter = "; ";
      }
    }
   ...
   * When the headers have no "Content-Disposition" information, a random UUID
   * name is generated.
   *
   ...
   */
  protected <T> void handleFileDownload(HttpResponse<Buffer> response,
   ...
   */
  protected <T> Handler<AsyncResult<HttpResponse<Buffer>>>
  buildResponseHandler(TypeReference<T> returnType,
                       Handler<AsyncResult<T>> handler) {
    return response -> {
        ...
        if (httpResponse.statusCode() / 100 == 2) {
          if (httpResponse.statusCode() == 204 || returnType == null) {
            ...
            if ("byte[]".equals(returnType.getType().toString())) {
              ...
              try {
                resultContent = this.objectMapper.readValue(
                    httpResponse.bodyAsString(), returnType);
                result = Future.succeededFuture(resultContent);
              } catch (Exception e) {
                result = ApiException.fail(new DecodeException(
                    "Failed to decode:" + e.getMessage(), e));
              }
            }
          }
        } else {
          result = ApiException.fail(
              httpResponse.statusMessage(), httpResponse.statusCode(),
              httpResponse.headers(), httpResponse.bodyAsString());
        }
      } else if (response.cause() instanceof ApiException) {
        result = Future.failedFuture(response.cause());
      } else {
        result = ApiException.fail(500, response.cause() != null
