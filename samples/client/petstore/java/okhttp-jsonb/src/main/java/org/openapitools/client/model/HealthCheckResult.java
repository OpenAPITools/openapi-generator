...
public class HealthCheckResult {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<HealthCheckResult> {
    @Override
    public void serialize(HealthCheckResult value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getNullableMessage() != null) {
        context.serialize("NullableMessage", value.getNullableMessage(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<HealthCheckResult> {
    @Override
    public HealthCheckResult deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      HealthCheckResult instance = new HealthCheckResult();
      if (jsonObj.containsKey("NullableMessage")) {
        instance.setNullableMessage(jsonObj.get("NullableMessage").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("NullableMessage").toString(), fieldType("nullableMessage")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = HealthCheckResult.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on HealthCheckResult");
    }
    ...
    private static void setField(HealthCheckResult instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on HealthCheckResult", e);
      }
    }
  }
