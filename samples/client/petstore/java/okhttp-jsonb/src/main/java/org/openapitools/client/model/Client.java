...
public class Client {
  ...
  @JsonbProperty(SERIALIZED_NAME_CLIENT)
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<Client> {
    @Override
    public void serialize(Client value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getClient() != null) {
        context.serialize("client", value.getClient(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<Client> {
    @Override
    public Client deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      if (jsonObj.containsKey("client")) {
        instance.setClient(jsonObj.get("client").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("client").toString(), fieldType("client")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = Client.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on Client");
    }
    ...
    private static void setField(Client instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on Client", e);
      }
    }
  }
}
