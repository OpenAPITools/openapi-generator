...
public class Apple {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<Apple> {
    @Override
    public void serialize(Apple value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getCultivar() != null) {
        context.serialize("cultivar", value.getCultivar(), generator);
      }
      if (value.getOrigin() != null) {
        context.serialize("origin", value.getOrigin(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<Apple> {
    @Override
    public Apple deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      if (jsonObj.containsKey("origin")) {
        instance.setOrigin(jsonObj.get("origin").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("origin").toString(), fieldType("origin")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = Apple.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on Apple");
    }
    ...
    private static void setField(Apple instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on Apple", e);
      }
    }
  }
}
