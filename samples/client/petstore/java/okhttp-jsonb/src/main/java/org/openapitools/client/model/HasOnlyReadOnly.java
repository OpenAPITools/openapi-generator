...
public class HasOnlyReadOnly {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<HasOnlyReadOnly> {
    @Override
    public void serialize(HasOnlyReadOnly value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getBar() != null) {
        context.serialize("bar", value.getBar(), generator);
      }
      if (value.getFoo() != null) {
        context.serialize("foo", value.getFoo(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<HasOnlyReadOnly> {
    @Override
    public HasOnlyReadOnly deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      HasOnlyReadOnly instance = new HasOnlyReadOnly();
      ...
      if (jsonObj.containsKey("foo")) {
        // readOnly properties have no setter; bind the declared field directly
        setField(instance, "foo", jsonObj.get("foo").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("foo").toString(), fieldType("foo")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = HasOnlyReadOnly.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on HasOnlyReadOnly");
    }
    ...
    private static void setField(HasOnlyReadOnly instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on HasOnlyReadOnly", e);
      }
    }
  }
