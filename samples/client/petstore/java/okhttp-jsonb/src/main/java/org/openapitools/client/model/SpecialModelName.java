...
public class SpecialModelName {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<SpecialModelName> {
    @Override
    public void serialize(SpecialModelName value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.get$SpecialPropertyName() != null) {
        context.serialize("$special[property.name]", value.get$SpecialPropertyName(), generator);
      }
      if (value.getSpecialModelName() != null) {
        context.serialize("_special_model.name_", value.getSpecialModelName(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<SpecialModelName> {
    @Override
    public SpecialModelName deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      SpecialModelName instance = new SpecialModelName();
      if (jsonObj.containsKey("$special[property.name]")) {
        instance.set$SpecialPropertyName(jsonObj.get("$special[property.name]").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("$special[property.name]").toString(), fieldType("$specialPropertyName")));
      }
      if (jsonObj.containsKey("_special_model.name_")) {
        instance.setSpecialModelName(jsonObj.get("_special_model.name_").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("_special_model.name_").toString(), fieldType("specialModelName")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = SpecialModelName.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on SpecialModelName");
    }
    ...
    private static void setField(SpecialModelName instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on SpecialModelName", e);
      }
    }
  }
}
