...
public class AllOfRefToFloat {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<AllOfRefToFloat> {
    @Override
    public void serialize(AllOfRefToFloat value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getWeight() != null) {
        context.serialize("weight", value.getWeight(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<AllOfRefToFloat> {
    @Override
    public AllOfRefToFloat deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      AllOfRefToFloat instance = new AllOfRefToFloat();
      if (jsonObj.containsKey("weight")) {
        instance.setWeight(jsonObj.get("weight").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("weight").toString(), fieldType("weight")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = AllOfRefToFloat.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on AllOfRefToFloat");
    }
    ...
    private static void setField(AllOfRefToFloat instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on AllOfRefToFloat", e);
      }
    }
  }
}
