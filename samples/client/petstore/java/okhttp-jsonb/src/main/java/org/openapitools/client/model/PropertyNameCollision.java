...
public class PropertyNameCollision {
  ...
  @JsonbProperty(SERIALIZED_NAME_TYPE_WITH_UNDERSCORE)
  @javax.annotation.Nullable
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<PropertyNameCollision> {
    @Override
    public void serialize(PropertyNameCollision value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getUnderscoreType() != null) {
        context.serialize("_type", value.getUnderscoreType(), generator);
      }
      if (value.getType() != null) {
        context.serialize("type", value.getType(), generator);
      }
      if (value.getTypeWithUnderscore() != null) {
        context.serialize("type_", value.getTypeWithUnderscore(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<PropertyNameCollision> {
    @Override
    public PropertyNameCollision deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      PropertyNameCollision instance = new PropertyNameCollision();
      if (jsonObj.containsKey("_type")) {
        instance.setUnderscoreType(jsonObj.get("_type").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("_type").toString(), fieldType("underscoreType")));
      }
      ...
      if (jsonObj.containsKey("type_")) {
        instance.setTypeWithUnderscore(jsonObj.get("type_").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("type_").toString(), fieldType("typeWithUnderscore")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = PropertyNameCollision.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on PropertyNameCollision");
    }
    ...
    private static void setField(PropertyNameCollision instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on PropertyNameCollision", e);
      }
    }
  }
}
