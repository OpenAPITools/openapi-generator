...
public class Whale {
   ...
   */
  public static class CustomJsonbSerializer implements JsonbSerializer<Whale> {
    @Override
    public void serialize(Whale value, JsonGenerator generator, SerializationContext context) {
      generator.writeStartObject();
      if (value.getHasBaleen() != null) {
        context.serialize("hasBaleen", value.getHasBaleen(), generator);
      }
      if (value.getHasTeeth() != null) {
        context.serialize("hasTeeth", value.getHasTeeth(), generator);
      }
   ...
   */
  public static class CustomJsonbDeserializer implements JsonbDeserializer<Whale> {
    @Override
    public Whale deserialize(JsonParser parser, DeserializationContext context, Type rtType) {
      ...
      if (jsonObj.containsKey("hasBaleen")) {
        instance.setHasBaleen(jsonObj.get("hasBaleen").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("hasBaleen").toString(), fieldType("hasBaleen")));
      }
      if (jsonObj.containsKey("hasTeeth")) {
        instance.setHasTeeth(jsonObj.get("hasTeeth").getValueType() == JsonValue.ValueType.NULL
            ? null
            : jsonb.fromJson(jsonObj.get("hasTeeth").toString(), fieldType("hasTeeth")));
      }
    ...
    private static java.lang.reflect.Field declaredField(String fieldName) {
      ...
      for (Class<?> clazz = Whale.class; clazz != null; clazz = clazz.getSuperclass()) {
        try {
      ...
      throw new IllegalArgumentException("Field " + fieldName + " not found on Whale");
    }
    ...
    private static void setField(Whale instance, String fieldName, Object value) {
      try {
        java.lang.reflect.Field field = declaredField(fieldName);
        field.setAccessible(true);
        field.set(instance, value);
      } catch (IllegalAccessException e) {
        throw new JsonbException("Unable to bind the field " + fieldName + " on Whale", e);
      }
    }
  }
}
