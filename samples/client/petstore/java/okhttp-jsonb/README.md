...
<dependency>
  ...
  <artifactId>petstore-okhttp-jsonb</artifactId>
...
```groovy
  repositories {
    mavenCentral()     // Needed if the 'petstore-okhttp-jsonb' jar has been published to maven central.
    mavenLocal()       // Needed if the 'petstore-okhttp-jsonb' jar has been published to the local maven repo.
  }

  dependencies {
     implementation "org.openapitools:petstore-okhttp-jsonb:1.0.0"
  }
```
...
* `target/petstore-okhttp-jsonb-1.0.0.jar`
* `target/lib/*.jar`
