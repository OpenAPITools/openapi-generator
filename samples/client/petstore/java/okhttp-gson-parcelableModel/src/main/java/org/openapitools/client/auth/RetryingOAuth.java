...
public class RetryingOAuth extends OAuth implements Interceptor {
  ...
  public RetryingOAuth(OkHttpClient client,
  ...
  @param tokenUrl The token URL to be used for this OAuth2 flow.
      Applicable to the following OAuth2 flows: "password", "clientCredentials"
  and "authorizationCode". The value must be an absolute URL.
  ...
  */
  public RetryingOAuth(String tokenUrl, String clientId, OAuthFlow flow,
                       String clientSecret, Map<String, String> parameters) {
  ...
  retryingIntercept(Chain chain,
    ...
    // If the request already has an authorization (e.g. Basic auth), proceed
    ...
    if (getAccessToken() != null) {
      ...
      // 401/403 response codes most likely indicate an expired access token,
      // unless it happens two times in a row
      if (response != null &&
