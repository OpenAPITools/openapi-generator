...
public class Play26CallFactory implements okhttp3.Call.Factory {
  ...
  public Play26CallFactory(WSClient wsClient) { this.wsClient = wsClient; }
  ...
  static class PlayWSCall implements Call {
    ...
    public PlayWSCall(WSClient wsClient, Executor executor,
                      List<WSRequestFilter> filters, Request request) {
      this.wsClient = wsClient;
    ...
    CompletionStage<WSResponse> executeAsync() {
      try {
        ...
        wsRequest = wsClient.url(url.scheme() + "://" + url.host() + ":" +
                                 url.port() + url.encodedPath());
        url.queryParameterNames().forEach(queryParam -> {
          wsRequest.addQueryParameter(queryParam,
                                      url.queryParameter(queryParam));
        });
    ...
    List<WSCookie> getCookies() {
      ...
      for (final String cookieString : request.headers("Cookie")) {
        for (String cookie : cookieString.split(";")) {
          ...
          cookies.add(new WSCookieBuilder()
