...
/**
 * Creates {@link CallAdapter} instances that convert {@link Call} into {@link
...
public class Play26CallAdapterFactory extends CallAdapter.Factory {
  ...
  private CallAdapter<?, CompletionStage<?>>
  createAdapter(ParameterizedType returnType) {
   ...
   */
  private static final class ValueAdapter<R>
      implements CallAdapter<R, CompletionStage<R>> {
    ...
    ValueAdapter(
        Type responseType, boolean includeResponse,
        Function<RuntimeException, RuntimeException> exceptionConverter) {
        ...
        public void onResponse(Call<R> call, Response<R> response) {
          if (response.isSuccessful()) {
            ...
            promise.completeExceptionally(
                exceptionConverter.apply(new HttpException(response)));
          }
        }
