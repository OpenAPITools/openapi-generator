...
public class Pig extends AbstractOpenApiSchema {
            ...
            return (TypeAdapter<T>) new TypeAdapter<Pig>() {
                ...
                public Pig read(JsonReader in) throws IOException {
                    ...
                    if (jsonObject.get("className") == null) {
                        ...
                        switch (jsonObject.get("className").getAsString()) {
                            ...
                            case "DanishPig":
                                ...
                                return newPig;
                            default:
                                log.log(Level.WARNING, String.format(Locale.ROOT, "Failed to lookup discriminator value `%s` for Pig. Possible values: BasquePig DanishPig", jsonObject.get("className").getAsString()));
                        }
                    }
                    ...
                    try {
                        ...
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for BasquePig failed with `%s`.", e.getMessage()));
                    ...
                    try {
                        ...
                        // deserialization failed, continue
                        errorMessages.add(String.format(Locale.ROOT, "Deserialization for DanishPig failed with `%s`.", e.getMessage()));
                    ...
                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for Pig: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for Pig with oneOf schemas: BasquePig, DanishPig. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }
