...
public class Shape extends AbstractOpenApiSchema {
        ...
        public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
            ...
            return (TypeAdapter<T>) new TypeAdapter<Shape>() {
                ...
                public Shape read(JsonReader in) throws IOException {
                    ...
                    if (jsonObject.get("shapeType") == null) {
                        ...
                        switch (jsonObject.get("shapeType").getAsString()) {
                            ...
                            case "Triangle":
                                ...
                                return newShape;
                            default:
                                log.log(Level.WARNING, String.format(Locale.ROOT, "Failed to lookup discriminator value `%s` for Shape. Possible values: Quadrilateral Triangle", jsonObject.get("shapeType").getAsString()));
                        }
                    }
                    ...
                    throw new IOException(String.format(Locale.ROOT, "Failed deserialization for Shape: %d classes match result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", match, errorMessages, jsonElement.toString()));
                }
            }.nullSafe();
        }
    }
    ...
    public static void validateJsonElement(JsonElement jsonElement) throws IOException {
        ...
        if (validCount != 1) {
            throw new IOException(String.format(Locale.ROOT, "The JSON string is invalid for Shape with oneOf schemas: Quadrilateral, Triangle. %d class(es) match the result, expected 1. Detailed failure message for oneOf schemas: %s. JSON: %s", validCount, errorMessages, jsonElement.toString()));
        }
    }
