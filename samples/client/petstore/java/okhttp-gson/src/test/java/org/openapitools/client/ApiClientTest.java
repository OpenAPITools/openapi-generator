...
public class ApiClientTest {
  ...
  public void testSelectHeaderAccept() {
    ...
    assertEquals("application/json; charset=UTF8",
  ...
  */
  ...
  public void testParameterToPairsWhenValueIsEmptyStrings() throws Exception {
    ...
    List<Pair> concatStrings =
        apiClient.parameterToPairs("csv", "param-a", strs);

    assertEquals(1, concatStrings.size());
    assertFalse(concatStrings.get(0)
                    .getValue()
  ...
  public void testParameterToPairsWhenValueIsCollection() throws Exception {
    ...
    collectionFormatMap.put("unknown",
                            ","); // all other formats, must default to csv
    ...
    for (String collectionFormat : collectionFormatMap.keySet()) {
      List<Pair> pairs =
          apiClient.parameterToPairs(collectionFormat, name, values);
  ...
  public void testSanitizeFilename() {
    ...
    assertEquals("sun.gif",
                 apiClient.sanitizeFilename("c:\\var\\tmp\\sun.gif"));
    assertEquals("sun.gif", apiClient.sanitizeFilename(".\\sun.gif"));
  }
  ...
  /**
   * Tests the invariant that the HttpClient for the ApiClient must never be
   * null
   */
