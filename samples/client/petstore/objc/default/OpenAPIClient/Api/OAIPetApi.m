...
///  @param xHeaderTest A header to test if headers work. 
///
...
-(NSURLSessionTask*) findPetsByStatusWithXHeaderTest: (NSString*) xHeaderTest
    ...
    // verify the required parameter 'xHeaderTest' is set
    if (xHeaderTest == nil) {
        NSParameterAssert(xHeaderTest);
        if(handler) {
            NSDictionary * userInfo = @{NSLocalizedDescriptionKey : [NSString stringWithFormat:NSLocalizedString(@"Missing required parameter '%@'", nil),@"xHeaderTest"] };
            NSError* error = [NSError errorWithDomain:kOAIPetApiErrorDomain code:kOAIPetApiMissingParamErrorCode userInfo:userInfo];
            handler(nil, error);
        }
        return nil;
    ...
    [headerParams addEntriesFromDictionary:self.defaultHeaders];
    if (xHeaderTest != nil) {
        headerParams[@"X-Header-Test"] = xHeaderTest;
    }
    // HTTP header `Accept`
