...
@implementation OAIPetApi

NSString* kOAIPetApiErrorDomain = @"OAIPetApiErrorDomain";
NSInteger kOAIPetApiMissingParamErrorCode = 234513;
...
-(NSURLSessionTask*) deletePetWithPetId: (NSNumber*) petId
    ...
    if (petId == nil) {
        ...
        if(handler) {
            ...
            NSError* error = [NSError errorWithDomain:kOAIPetApiErrorDomain code:kOAIPetApiMissingParamErrorCode userInfo:userInfo];
            handler(error);
        }
        return nil;
    }
...
-(NSURLSessionTask*) findPetsByStatusWithStatus: (NSArray<NSString*>*) status
    ...
    NSMutableDictionary* queryParams = [[NSMutableDictionary alloc] init];
    if (status != nil) {
        queryParams[@"status"] = [[OAIQueryParamCollection alloc] initWithValuesAndFormat: status format: @"multi"];
    }
    NSMutableDictionary* headerParams = [NSMutableDictionary dictionaryWithDictionary:self.apiClient.configuration.defaultHeaders];
    [headerParams addEntriesFromDictionary:self.defaultHeaders];
    // HTTP header `Accept`
