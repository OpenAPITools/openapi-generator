...
ApiClient <- R6::R6Class(
  ...
  public = list(
    ...
    deserializeObj = function(obj, return_type, pkg_env) {
      ...
      # "Date"/"POSIXct" are temporal primitives but NOT added to primitive_types so
      # that a package-defined model of the same name (e.g. the petstore `Date` model)
      # takes precedence in deserialization. A bare date/date-time payload is always a
      # character string (never a list), so is.character(obj) disambiguates the two.
      temporal_types <- c("Date", "POSIXct")
      ...
      if (startsWith(return_type, "map(")) {
      ...
      } else if (exists(return_type, pkg_env) &&
                 !(c(return_type) %in% primitive_types) &&
                 !(c(return_type) %in% temporal_types && is.character(obj))) {
        # To handle model objects which are not array or map containers. Ex:"Pet"
        # A temporal token (Date/POSIXct) with a character payload is a bare
        # date/date-time value, not a model — fall through to the primitive branch.
        ...
        if (identical(return_type, "Date")) {
          ...
          return_obj <- if (is.null(obj)) NULL else .parse_datetime(obj)
        } else {
          return_obj <- obj
        }
      }
