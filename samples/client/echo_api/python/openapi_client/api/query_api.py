...
class QueryApi:
    ...
    def _test_query_style_form_explode_true_array_string_serialize(
        ...
        if query_object is not None:
            ...
            # properties to explode, so it stays a single parameter; one holding a list
            # repeats the parameter name per item. A null property or item is left out.
            ...
            elif isinstance(_serialized, (list, tuple)):
                _query_params.extend(('query_object', _item) for _item in _serialized if _item is not None)
            else:
                _query_params.append(('query_object', _serialized))
