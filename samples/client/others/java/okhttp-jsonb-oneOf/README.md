...
<dependency>
  ...
  <artifactId>okhttp-jsonb-oneOf</artifactId>
...
```groovy
  repositories {
    mavenCentral()     // Needed if the 'okhttp-jsonb-oneOf' jar has been published to maven central.
    mavenLocal()       // Needed if the 'okhttp-jsonb-oneOf' jar has been published to the local maven repo.
  }

  dependencies {
     implementation "org.openapitools:okhttp-jsonb-oneOf:1.0.0"
  }
```
...
* `target/okhttp-jsonb-oneOf-1.0.0.jar`
* `target/lib/*.jar`
