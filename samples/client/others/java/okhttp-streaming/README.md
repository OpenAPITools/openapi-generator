...
<dependency>
  ...
  <artifactId>petstore-okhttp</artifactId>
  <version>1.0</version>
  <scope>compile</scope>
</dependency>
```
...
```groovy
  repositories {
    mavenCentral()     // Needed if the 'petstore-okhttp' jar has been published to maven central.
    mavenLocal()       // Needed if the 'petstore-okhttp' jar has been published to the local maven repo.
  }

  dependencies {
     implementation "org.openapitools:petstore-okhttp:1.0"
  }
```
...
* `target/petstore-okhttp-1.0.jar`
* `target/lib/*.jar`
