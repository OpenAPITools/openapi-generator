...
module Qdrant::Api
  ...
  # WithVector (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class WithVector
    ...
    def self.from_json_any(data : JSON::Any) : self
      ...
      begin
        return new(Bool.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in WithVector (anyOf)", 0, 0)
    end
    ...
    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      ...
      begin
        return new(Bool.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in WithVector (anyOf)", 0, 0)
    end
