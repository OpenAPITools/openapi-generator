...
module Qdrant::Api
  # StartFrom (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class StartFrom
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(Float64.from_json(json))
      ...
      begin
        return new(Time.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in StartFrom (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(Float64.new(ctx, node))
      ...
      begin
        return new(Time.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in StartFrom (anyOf)", 0, 0)
    end
