...
module Qdrant::Api
  # WithLookupInterface (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class WithLookupInterface
    getter value

    def initialize(@value : String)
    end
    def initialize(@value : WithLookup)
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      ...
      begin
        return new(WithLookup.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in WithLookupInterface (anyOf)", 0, 0)
    end
    ...
    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      ...
      begin
        return new(WithLookup.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in WithLookupInterface (anyOf)", 0, 0)
    end
