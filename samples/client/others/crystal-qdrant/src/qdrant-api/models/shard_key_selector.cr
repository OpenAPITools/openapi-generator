...
module Qdrant::Api
  # ShardKeySelector (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class ShardKeySelector
    ...
    def initialize(@value : Array(ShardKey))
    end
    def initialize(@value : ShardKey)
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(Array(ShardKey).from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(ShardKey.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in ShardKeySelector (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(Array(ShardKey).new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(ShardKey.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in ShardKeySelector (anyOf)", 0, 0)
    end
