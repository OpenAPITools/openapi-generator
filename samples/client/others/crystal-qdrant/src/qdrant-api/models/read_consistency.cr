...
module Qdrant::Api
  ...
  class ReadConsistency
    ...
    def initialize(@value : ReadConsistencyType)
    end

    # List of classes defined in anyOf (OpenAPI v3)
    def self.openapi_any_of
      [
        Int32, ReadConsistencyType
      ]
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      ...
      begin
        return new(ReadConsistencyType.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in ReadConsistency (anyOf)", 0, 0)
    end
    ...
    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      ...
      begin
        return new(ReadConsistencyType.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in ReadConsistency (anyOf)", 0, 0)
    end
