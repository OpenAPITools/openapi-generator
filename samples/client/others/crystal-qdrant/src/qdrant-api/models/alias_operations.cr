...
module Qdrant::Api
  ...
  # AliasOperations (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class AliasOperations
    ...
    def initialize(@value : CreateAliasOperation)
    end
    def initialize(@value : DeleteAliasOperation)
    end
    def initialize(@value : RenameAliasOperation)
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(CreateAliasOperation.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(DeleteAliasOperation.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(RenameAliasOperation.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in AliasOperations (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(CreateAliasOperation.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(DeleteAliasOperation.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(RenameAliasOperation.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in AliasOperations (anyOf)", 0, 0)
    end
