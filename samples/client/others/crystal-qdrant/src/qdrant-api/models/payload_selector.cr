...
module Qdrant::Api
  ...
  # PayloadSelector (OpenAPI anyOf): a value matching at least one of the listed schemas.
  ...
  class PayloadSelector
    ...
    def initialize(@value : PayloadSelectorExclude)
    end
    def initialize(@value : PayloadSelectorInclude)
    end

    # List of classes defined in anyOf (OpenAPI v3)
    def self.openapi_any_of
      [
        PayloadSelectorExclude, PayloadSelectorInclude
      ]
    end
    ...
    def self.from_json_any(data : JSON::Any) : self
      json = data.to_json
      begin
        return new(PayloadSelectorExclude.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(PayloadSelectorInclude.from_json(json))
      rescue JSON::ParseException | ArgumentError | TypeCastError
      end
      raise JSON::ParseException.new("`#{json}` doesn't match any schema listed in PayloadSelector (anyOf)", 0, 0)
    end

    def self.new(ctx : YAML::ParseContext, node : YAML::Nodes::Node)
      begin
        return new(PayloadSelectorExclude.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      begin
        return new(PayloadSelectorInclude.new(ctx, node))
      rescue YAML::ParseException | ArgumentError | TypeCastError
      end
      raise YAML::ParseException.new("doesn't match any schema listed in PayloadSelector (anyOf)", 0, 0)
    end
