...
allowed_methods(Req, State = #state{operation_id = 'TestFormIntegerBooleanString'}) ->
    {[<<"POST">>], Req, State};

allowed_methods(Req, State = #state{operation_id = 'TestFormObjectMultipart'}) ->
    {[<<"POST">>], Req, State};

allowed_methods(Req, State = #state{operation_id = 'TestFormOneof'}) ->
    {[<<"POST">>], Req, State};
...
valid_content_headers(Req0, #state{operation_id = 'TestFormIntegerBooleanString'} = State) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0, #state{operation_id = 'TestFormObjectMultipart'} = State) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0, #state{operation_id = 'TestFormOneof'} = State) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};
