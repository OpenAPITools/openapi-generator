...
allowed_methods(Req, State = #state{operation_id = 'TestAuthHttpBasic'}) ->
    {[<<"POST">>], Req, State};

allowed_methods(Req, State = #state{operation_id = 'TestAuthHttpBearer'}) ->
    {[<<"POST">>], Req, State};
...
is_authorized(Req0, #state{operation_id = 'TestAuthHttpBasic' = OperationID,
...
is_authorized(Req0, #state{operation_id = 'TestAuthHttpBearer' = OperationID,
                           logic_handler = LogicHandler} = State) ->
    Result = openapi_auth:authorize_api_key(
                              LogicHandler,
                              OperationID,
...
valid_content_headers(Req0, #state{operation_id = 'TestAuthHttpBasic'} = State) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};

valid_content_headers(Req0, #state{operation_id = 'TestAuthHttpBearer'} = State) ->
    Headers = [],
    {Result, Req} = validate_headers(Headers, Req0),
    {Result, Req, State};
