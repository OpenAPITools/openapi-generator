...
public interface StoreApi {
    ...
    default CompletableFuture<ResponseEntity<Map<String, Integer>>> getInventory() throws IOException {
        // do some magic!
    ...
    @RequestMapping(value = "/store/order/{order_id}",
        produces = { "application/xml", "application/json" }, 
        method = RequestMethod.GET)
    default CompletableFuture<ResponseEntity<Order>> getOrderById( @Min(1) @Max(5)@ApiParam(value = "ID of pet that needs to be fetched",required=true ) @PathVariable("order_id") Long orderId) throws IOException {
        // do some magic!
    ...
    default CompletableFuture<ResponseEntity<Order>> placeOrder(@ApiParam(value = "order placed for purchasing the pet" ,required=true )  @Valid @RequestBody Order body) throws IOException {
        // do some magic!
        return CompletableFuture.completedFuture(new ResponseEntity<Order>(HttpStatus.OK));
    }
