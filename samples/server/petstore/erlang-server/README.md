...
3. Go into the http_server project folder
    ...
    _NOTE: The following generated files are now in the folder "http_server":_
...
```erlang
openapi_server:start(http_server, #{transport_opts=>[{ip,{127,0,0,1}},{port,8080}]})
```
...
start(_StartType, _StartArgs) ->
    openapi_server:start(http_server, 
                        #{transport_opts=>[{ip,{127,0,0,1}},{port,8080}]}),
