...
class OpenApiDataMockerTest extends TestCase
{
    ...
    public function testMockIntegerWithCorrectArguments(
        ...
        foreach ($matchingInternalTypes as $matchType) {
            $this->assertInternalType($matchType, $integer);
        }
        ...
        foreach ($notMatchingInternalTypes as $notMatchType) {
            $this->assertNotInternalType($notMatchType, $integer);
        }

        if ($minimum !== null) {
            if ($exclusiveMinimum) {
                $this->assertGreaterThan($minimum, $integer);
            } else {
                $this->assertGreaterThanOrEqual($minimum, $integer);
            }
        }

        if ($maximum !== null) {
            if ($exclusiveMaximum) {
                $this->assertLessThan($maximum, $integer);
            } else {
                $this->assertLessThanOrEqual($maximum, $integer);
            }
        }
    }
    ...
    public function provideMockIntegerCorrectArguments()
    {
        return [
            ...
            [null, -100, null, false, false],
            [null, null, 100, false, false],
            [null, -99.5, null, true, false],
            [null, null, 99.5, false, true],
            [null, -99.5, 99.5, true, true],
        ];
    }
