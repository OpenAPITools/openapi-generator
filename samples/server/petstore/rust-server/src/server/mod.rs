...
impl RequestParser for ApiRequestParser {
    fn parse_operation_id(request: &Request) -> Result<&'static str, ()> {
        ...
        match request.method() {
            ...
            &hyper::Method::Get if path.matched(paths::ID_FAKE_XMLFEATURES) => Ok("GetXmlFeatures"),

            // PostPlainText - POST /fake/plaintext
            &hyper::Method::Post if path.matched(paths::ID_FAKE_PLAINTEXT) => Ok("PostPlainText"),

            // PostUrlEncodedForm - POST /fake/formUrlEncoded
            &hyper::Method::Post if path.matched(paths::ID_FAKE_FORMURLENCODED) => Ok("PostUrlEncodedForm"),

            // PostXmlFeatures - POST /fake/xmlFeatures
            &hyper::Method::Post if path.matched(paths::ID_FAKE_XMLFEATURES) => Ok("PostXmlFeatures"),
