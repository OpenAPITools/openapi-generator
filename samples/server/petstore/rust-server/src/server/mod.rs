...
mod paths {
    ...
    lazy_static! {
        pub static ref GLOBAL_REGEX_SET: regex::RegexSet = regex::RegexSet::new(vec![
            ...
            r"^/v2/fake$",
            r"^/v2/fake/body-mixedPropertiesAndAdditionalProperties$",
            r"^/v2/fake/body-with-file-schema$",
            r"^/v2/fake/body-with-query-params$",
            r"^/v2/fake/create_xml_item$",
            ...
            r"^/v2/fake/test-query-paramters$",
            r"^/v2/fake/(?P<petId>[^/?#]*)/uploadImageWithRequiredFile$",
    ...
    pub static ID_FAKE_BODY_MIXEDPROPERTIESANDADDITIONALPROPERTIES: usize = 2;
    pub static ID_FAKE_BODY_WITH_FILE_SCHEMA: usize = 3;
    pub static ID_FAKE_BODY_WITH_QUERY_PARAMS: usize = 4;
    pub static ID_FAKE_CREATE_XML_ITEM: usize = 5;
    pub static ID_FAKE_INLINE_ADDITIONALPROPERTIES: usize = 6;
    pub static ID_FAKE_JSONFORMDATA: usize = 7;
    pub static ID_FAKE_OUTER_BOOLEAN: usize = 8;
    pub static ID_FAKE_OUTER_COMPOSITE: usize = 9;
    pub static ID_FAKE_OUTER_NUMBER: usize = 10;
    pub static ID_FAKE_OUTER_STRING: usize = 11;
    pub static ID_FAKE_TEST_QUERY_PARAMTERS: usize = 12;
    pub static ID_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE: usize = 13;
    lazy_static! {
        pub static ref REGEX_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE: regex::Regex = regex::Regex::new(r"^/v2/fake/(?P<petId>[^/?#]*)/uploadImageWithRequiredFile$").unwrap();
    }
    pub static ID_FAKE_CLASSNAME_TEST: usize = 14;
    pub static ID_PET: usize = 15;
    pub static ID_PET_FINDBYSTATUS: usize = 16;
    pub static ID_PET_FINDBYTAGS: usize = 17;
    pub static ID_PET_PETID: usize = 18;
    lazy_static! {
        pub static ref REGEX_PET_PETID: regex::Regex = regex::Regex::new(r"^/v2/pet/(?P<petId>[^/?#]*)$").unwrap();
    }
    pub static ID_PET_PETID_UPLOADIMAGE: usize = 19;
    lazy_static! {
        pub static ref REGEX_PET_PETID_UPLOADIMAGE: regex::Regex = regex::Regex::new(r"^/v2/pet/(?P<petId>[^/?#]*)/uploadImage$").unwrap();
    }
    pub static ID_STORE_INVENTORY: usize = 20;
    pub static ID_STORE_ORDER: usize = 21;
    pub static ID_STORE_ORDER_ORDER_ID: usize = 22;
    lazy_static! {
        pub static ref REGEX_STORE_ORDER_ORDER_ID: regex::Regex = regex::Regex::new(r"^/v2/store/order/(?P<order_id>[^/?#]*)$").unwrap();
    }
    pub static ID_USER: usize = 23;
    pub static ID_USER_CREATEWITHARRAY: usize = 24;
    pub static ID_USER_CREATEWITHLIST: usize = 25;
    pub static ID_USER_LOGIN: usize = 26;
    pub static ID_USER_LOGOUT: usize = 27;
    pub static ID_USER_USERNAME: usize = 28;
    lazy_static! {
    ...
    C: Has<XSpanIdString> + Has<Option<Authorization>> + 'static
{
    ...
    fn call(&self, (req, mut context): Self::Request) -> Self::Future {
        ...
        match &method {
            ...
            // 123Test@$%SpecialTags - PATCH /another-fake/dummy
                    ...
                    .then(move |result| -> Box<Future<Item=Response, Error=Error>> {
                        match result {
                            Ok(body) => {
                                ...
                                Box::new(api_impl.123_test@__%special_tags(param_body, &context)
                                    .then(move |result| {
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                123Test@$%SpecialTagsResponse::SuccessfulOperation
                                                ...
                                                => {
                                                    ...
                                                    response.headers_mut().set(ContentType(mimetypes::responses::123_TEST@__%SPECIAL_TAGS_SUCCESSFUL_OPERATION.clone()));
            ...
            // CreateXmlItem - POST /fake/create_xml_item
            &hyper::Method::Post if path.matched(paths::ID_FAKE_CREATE_XML_ITEM) => {
                // Body parameters (note that non-required body parameters will ignore garbage
                // values, rather than causing a 400 response). Produce warning header and logs for
                // any unused fields.
                Box::new(body.concat2()
                    .then(move |result| -> Box<Future<Item=Response, Error=Error>> {
                        match result {
                            Ok(body) => {
                                let mut unused_elements = Vec::new();
                                let param_xml_item: Option<models::XmlItem> = if !body.is_empty() {
                                    let deserializer = &mut serde_xml_rs::de::Deserializer::new_from_reader(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                    }) {
                                        Ok(param_xml_item) => param_xml_item,
                                        Err(e) => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body(format!("Couldn't parse body parameter XmlItem - doesn't match schema: {}", e)))),
                                    }
                                } else {
                                    None
                                };
                                let param_xml_item = match param_xml_item {
                                    Some(param_xml_item) => param_xml_item,
                                    None => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body("Missing required body parameter XmlItem"))),
                                };
                                Box::new(api_impl.create_xml_item(param_xml_item, &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
                            ...
                            Err(e) => Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body(format!("Couldn't read body parameter XmlItem: {}", e)))),
                        }
                    })
                ) as Box<Future<Item=Response, Error=Error>>
            },
            ...
            &hyper::Method::Put if path.matched(paths::ID_FAKE_BODY_WITH_FILE_SCHEMA) => {
                // Body parameters (note that non-required body parameters will ignore garbage
                // values, rather than causing a 400 response). Produce warning header and logs for
                // any unused fields.
                Box::new(body.concat2()
                    .then(move |result| -> Box<Future<Item=Response, Error=Error>> {
                        match result {
                            Ok(body) => {
                                let mut unused_elements = Vec::new();
                                let param_body: Option<models::FileSchemaTestClass> = if !body.is_empty() {
                                    let deserializer = &mut serde_json::Deserializer::from_slice(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                ...
                                Box::new(api_impl.test_body_with_file_schema(param_body, &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
            ...
            // TestGroupParameters - DELETE /fake
            &hyper::Method::Delete if path.matched(paths::ID_FAKE) => {
                // Header parameters
                header! { (RequestRequiredBooleanGroup, "required_boolean_group") => [bool] }
                let param_required_boolean_group = match headers.get::<RequestRequiredBooleanGroup>() {
                    Some(param_required_boolean_group) => param_required_boolean_group.0.clone(),
                    None => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body("Missing or invalid required header required_boolean_group"))),
                };
                header! { (RequestBooleanGroup, "boolean_group") => [bool] }
                let param_boolean_group = headers.get::<RequestBooleanGroup>().map(|header| header.0.clone());
                // Query parameters (note that non-required or collection query parameters will ignore garbage values, rather than causing a 400 response)
                let query_params = form_urlencoded::parse(uri.query().unwrap_or_default().as_bytes()).collect::<Vec<_>>();
                let param_required_string_group = query_params.iter().filter(|e| e.0 == "required_string_group").map(|e| e.1.to_owned())
                    .nth(0);
                let param_required_string_group = match param_required_string_group {
                    Some(param_required_string_group) => match param_required_string_group.parse::<i32>() {
                        Ok(param_required_string_group) => param_required_string_group,
                        Err(e) => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body(format!("Couldn't parse query parameter required_string_group - doesn't match schema: {}", e)))),
                    },
                    None => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body("Missing required query parameter required_string_group"))),
                };
                let param_required_int64_group = query_params.iter().filter(|e| e.0 == "required_int64_group").map(|e| e.1.to_owned())
                    .nth(0);
                let param_required_int64_group = match param_required_int64_group {
                    Some(param_required_int64_group) => match param_required_int64_group.parse::<i64>() {
                        Ok(param_required_int64_group) => param_required_int64_group,
                        Err(e) => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body(format!("Couldn't parse query parameter required_int64_group - doesn't match schema: {}", e)))),
                    },
                    None => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body("Missing required query parameter required_int64_group"))),
                };
                let param_string_group = query_params.iter().filter(|e| e.0 == "string_group").map(|e| e.1.to_owned())
                    .nth(0);

                let param_string_group = param_string_group.and_then(|param_string_group| param_string_group.parse::<>().ok());
                let param_int64_group = query_params.iter().filter(|e| e.0 == "int64_group").map(|e| e.1.to_owned())
                    .nth(0);

                let param_int64_group = param_int64_group.and_then(|param_int64_group| param_int64_group.parse::<>().ok());
                Box::new({
                        {{
                                Box::new(api_impl.test_group_parameters(param_required_string_group, param_required_boolean_group, param_required_int64_group, param_string_group, param_boolean_group, param_int64_group, &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
            ...
            // TestMixedPropertiesAndAdditionalProperties - POST /fake/body-mixedPropertiesAndAdditionalProperties
            &hyper::Method::Post if path.matched(paths::ID_FAKE_BODY_MIXEDPROPERTIESANDADDITIONALPROPERTIES) => {
                // Body parameters (note that non-required body parameters will ignore garbage
                // values, rather than causing a 400 response). Produce warning header and logs for
                // any unused fields.
                Box::new(body.concat2()
                    .then(move |result| -> Box<Future<Item=Response, Error=Error>> {
                        match result {
                            Ok(body) => {
                                let mut unused_elements = Vec::new();
                                let param_param: Option<models::MixedPropertiesAndAdditionalPropertiesClass> = if !body.is_empty() {
                                    let deserializer = &mut serde_json::Deserializer::from_slice(&*body);
                                    match serde_ignored::deserialize(deserializer, |path| {
                                            warn!("Ignoring unknown field in body: {}", path);
                                            unused_elements.push(path.to_string());
                                ...
                                Box::new(api_impl.test_mixed_properties_and_additional_properties(param_param, &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                TestMixedPropertiesAndAdditionalPropertiesResponse::SuccessfulOperation
                                                ...
                                                => {
                                                    ...
                                                    response.headers_mut().set(ContentType(mimetypes::responses::TEST_MIXED_PROPERTIES_AND_ADDITIONAL_PROPERTIES_SUCCESSFUL_OPERATION.clone()));
            ...
            &hyper::Method::Put if path.matched(paths::ID_FAKE_TEST_QUERY_PARAMTERS) => {
                // Query parameters (note that non-required or collection query parameters will ignore garbage values, rather than causing a 400 response)
                let query_params = form_urlencoded::parse(uri.query().unwrap_or_default().as_bytes()).collect::<Vec<_>>();
                let param_pipe = query_params.iter().filter(|e| e.0 == "pipe").map(|e| e.1.to_owned())
                    .filter_map(|param_pipe| param_pipe.parse::<String>().ok())
                    .collect::<Vec<_>>();
                let param_ioutil = query_params.iter().filter(|e| e.0 == "ioutil").map(|e| e.1.to_owned())
                    .filter_map(|param_ioutil| param_ioutil.parse::<String>().ok())
                    .collect::<Vec<_>>();
                let param_http = query_params.iter().filter(|e| e.0 == "http").map(|e| e.1.to_owned())
                    .filter_map(|param_http| param_http.parse::<String>().ok())
                    .collect::<Vec<_>>();
                let param_url = query_params.iter().filter(|e| e.0 == "url").map(|e| e.1.to_owned())
                    .filter_map(|param_url| param_url.parse::<String>().ok())
                    .collect::<Vec<_>>();
                let param_context = query_params.iter().filter(|e| e.0 == "context").map(|e| e.1.to_owned())
                    .filter_map(|param_context| param_context.parse::<String>().ok())
                    .collect::<Vec<_>>();
                Box::new({
                        {{
                                Box::new(api_impl.test_query_parameter_collection_format(param_pipe.as_ref(), param_ioutil.as_ref(), param_http.as_ref(), param_url.as_ref(), param_context.as_ref(), &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
            ...
            // UploadFileWithRequiredFile - POST /fake/{petId}/uploadImageWithRequiredFile
            &hyper::Method::Post if path.matched(paths::ID_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE) => {
                {
                    let authorization = match (&context as &Has<Option<Authorization>>).get() {
                        &Some(ref authorization) => authorization,
                        &None => return Box::new(future::ok(Response::new()
                ...
                let path = uri.path().to_string();
                let path_params =
                    paths::REGEX_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE
                    .captures(&path)
                    .unwrap_or_else(||
                        panic!("Path {} matched RE FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE in set but failed match against \"{}\"", path, paths::REGEX_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE.as_str())
                    );
                let param_pet_id = match percent_encoding::percent_decode(path_params["petId"].as_bytes()).decode_utf8() {
                    Ok(param_pet_id) => match param_pet_id.parse::<i64>() {
                        Ok(param_pet_id) => param_pet_id,
                    ...
                    .then(move |result| -> Box<Future<Item=Response, Error=Error>> {
                        match result {
                            Ok(body) => {
                                ...
                                let file_required_file = entries.files.remove("required_file");
                                let param_required_file = match file_required_file {
                                    Some(file) => {
                                        let path = &file[0].path;
                                        let required_file_str = fs::read_to_string(path).expect("Reading saved String should never fail");
                                        let required_file_model: swagger::ByteArray = match serde_json::from_str(&required_file_str) {
                                            Ok(model) => model,
                                            Err(e) => {
                                                return Box::new(future::ok(
                                                    Response::new()
                                                    .with_status(StatusCode::BadRequest)
                                                    .with_body(format!("required_file data does not match API definition: {}", e))))
                                            }
                                        };
                                        required_file_model
                                    }
                                    None => return Box::new(future::ok(Response::new().with_status(StatusCode::BadRequest).with_body(format!("Missing required form parameter required_file")))),
                                };
                                
                                Box::new(api_impl.upload_file_with_required_file(param_pet_id, param_required_file, param_additional_metadata, &context)
                                    .then(move |result| {
                                        let mut response = Response::new();
                                        response.headers_mut().set(XSpanId((&context as &Has<XSpanIdString>).get().0.to_string()));
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                UploadFileWithRequiredFileResponse::SuccessfulOperation
                                                ...
                                                => {
                                                    ...
                                                    response.headers_mut().set(ContentType(mimetypes::responses::UPLOAD_FILE_WITH_REQUIRED_FILE_SUCCESSFUL_OPERATION.clone()));
...
impl RequestParser for ApiRequestParser {
    fn parse_operation_id(request: &Request) -> Result<&'static str, ()> {
        ...
        match request.method() {
            ...
            // 123Test@$%SpecialTags - PATCH /another-fake/dummy
            &hyper::Method::Patch if path.matched(paths::ID_ANOTHER_FAKE_DUMMY) => Ok("123Test@$%SpecialTags"),

            // CreateXmlItem - POST /fake/create_xml_item
            &hyper::Method::Post if path.matched(paths::ID_FAKE_CREATE_XML_ITEM) => Ok("CreateXmlItem"),
            ...
            // TestBodyWithFileSchema - PUT /fake/body-with-file-schema
            &hyper::Method::Put if path.matched(paths::ID_FAKE_BODY_WITH_FILE_SCHEMA) => Ok("TestBodyWithFileSchema"),
            ...
            // TestGroupParameters - DELETE /fake
            &hyper::Method::Delete if path.matched(paths::ID_FAKE) => Ok("TestGroupParameters"),
            ...
            // TestMixedPropertiesAndAdditionalProperties - POST /fake/body-mixedPropertiesAndAdditionalProperties
            &hyper::Method::Post if path.matched(paths::ID_FAKE_BODY_MIXEDPROPERTIESANDADDITIONALPROPERTIES) => Ok("TestMixedPropertiesAndAdditionalProperties"),

            // TestQueryParameterCollectionFormat - PUT /fake/test-query-paramters
            &hyper::Method::Put if path.matched(paths::ID_FAKE_TEST_QUERY_PARAMTERS) => Ok("TestQueryParameterCollectionFormat"),
            ...
            // UploadFileWithRequiredFile - POST /fake/{petId}/uploadImageWithRequiredFile
            &hyper::Method::Post if path.matched(paths::ID_FAKE_PETID_UPLOADIMAGEWITHREQUIREDFILE) => Ok("UploadFileWithRequiredFile"),
