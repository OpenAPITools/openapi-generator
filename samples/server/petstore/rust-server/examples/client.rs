...
fn main() {
    ...
    match matches.value_of("operation") {
        ...
        // Some("123Test@$%SpecialTags") => {
        //     let result = core.run(client.123_test@__%special_tags(???));
        //     println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
        //  },

        // Disabled because there's no example.
        // Some("CreateXmlItem") => {
        //     let result = core.run(client.create_xml_item(???));
        //     println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
        //  },
        ...
        // Disabled because there's no example.
        // Some("TestBodyWithFileSchema") => {
        //     let result = core.run(client.test_body_with_file_schema(???));
        //     println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
        //  },
        ...
        Some("TestEnumParameters") => {
            let result = core.run(client.test_enum_parameters(Some(&Vec::new()), Some("enum_header_string_example".to_string()), Some(&Vec::new()), Some("enum_query_string_example".to_string()), Some(56), Some(1.2), Some("enum_form_string_array_example".to_string()), Some("enum_form_string_example".to_string())));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },

        Some("TestGroupParameters") => {
            let result = core.run(client.test_group_parameters(56, true, 789, Some(56), Some(true), Some(789)));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
        ...
        // Disabled because there's no example.
        // Some("TestMixedPropertiesAndAdditionalProperties") => {
        //     let result = core.run(client.test_mixed_properties_and_additional_properties(???));
        //     println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
        //  },

        Some("TestQueryParameterCollectionFormat") => {
            let result = core.run(client.test_query_parameter_collection_format(&Vec::new(), &Vec::new(), &Vec::new(), &Vec::new(), &Vec::new()));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
        ...
        Some("UploadFileWithRequiredFile") => {
            let result = core.run(client.upload_file_with_required_file(789, swagger::ByteArray(Vec::from("BINARY_DATA_HERE")), Some("additional_metadata_example".to_string())));
            println!("{:?} (X-Span-ID: {:?})", result, (client.context() as &Has<XSpanIdString>).get().clone());
         },
