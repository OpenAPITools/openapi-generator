...
import mo.del.pack.age.ApiResponse
import java.io.File
import mo.del.pack.age.Pet
...
class PetRoutes(service: PetService) extends cask.Routes {
  ...
  def addPet(request: cask.Request) = {
    ...
    val result = for {
      petData <- Parsed
        .eval(PetData.fromJsonString(request.bodyAsString))
        .mapError(e =>
          s"Error parsing json as Pet from >${request.bodyAsString}< : ${e}"
        ) /* not array or map */
