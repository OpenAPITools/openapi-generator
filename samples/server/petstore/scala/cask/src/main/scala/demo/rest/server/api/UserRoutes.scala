...
class UserRoutes(service: UserService) extends cask.Routes {
  ...
  def createUser(request: cask.Request) = {
    ...
    val result = for {
      userData <- Parsed
        .eval(UserData.fromJsonString(request.bodyAsString))
        .mapError(e =>
          s"Error parsing json as User from >${request.bodyAsString}< : ${e}"
        ) /* not array or map */
  ...
  def createUsersWithArrayInput(request: cask.Request) = {
    ...
    val result = for {
      user <- Parsed
        .fromTry(UserData.manyFromJsonStringValidated(request.bodyAsString))
        .mapError(e =>
          s"Error parsing json as an array of User from >${request.bodyAsString}< : ${e}"
        ) /* array */
