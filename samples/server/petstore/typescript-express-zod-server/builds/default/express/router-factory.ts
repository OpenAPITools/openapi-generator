...
export function getRouter({
  ...
  router
    .route("/books/:bookId/latest-event")
    .get(
      handlersFor(
        "getLatestBookEvent",
        handlers.handleGetLatestBookEvent(getCatalogService),
      ),
    )
    .options((_, res) => {
  ...
  router
    ...
    .get(
      handlersFor(
        "getAuthor",
        handlers.handleGetAuthor(getAuthorsService),
      ),
    )
    .options((_, res) => {
      res.set("Allow", "GET, HEAD, OPTIONS").sendStatus(204);
  ...
  router
    ...
    .delete(
      handlersFor(
        "deleteBook",
        handlers.handleDeleteBook(getCatalogService),
      ),
    )
    .get(
      handlersFor(
        "getBook",
        handlers.handleGetBook(getCatalogService),
      ),
    )
    .options((_, res) => {
  ...
  router
    ...
    .post(
      handlersFor(
        "createAuthor",
        handlers.handleCreateAuthor(getAuthorsService),
      ),
    )
    .options((_, res) => {
      res.set("Allow", "OPTIONS, POST").sendStatus(204);
    })
  ...
  router
    ...
    .post(
      handlersFor(
        "createBook",
        handlers.handleCreateBook(getCatalogService),
      ),
    )
    .options((_, res) => {
      res.set("Allow", "OPTIONS, POST").sendStatus(204);
    ...
    .get(...getMiddleware(middleware, "_onlySwaggerUI"), (_, res) => {
      ...
      <html>
        ...
        <body>
          ...
          <script>
            const spec = ${JSON.stringify(schema).replace(/</g, "\\u003c")};
