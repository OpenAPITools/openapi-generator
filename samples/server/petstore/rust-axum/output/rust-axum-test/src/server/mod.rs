    ...
    
{
    ...
    Router::new()
        .route("/allOf",
            get(all_of_get::<I, A, E>)
        )
        .route("/dummy",
            get(dummy_get::<I, A, E>).put(dummy_put::<I, A, E>)
        )
        .route("/file_response",
            get(file_response_get::<I, A, E>)
        )
        .route("/get-structured-yaml",
            get(get_structured_yaml::<I, A, E>)
        )
        .route("/html",
            post(html_post::<I, A, E>)
        )
        .route("/post-yaml",
            post(post_yaml::<I, A, E>)
        )
        .route("/raw_json",
            get(raw_json_get::<I, A, E>)
        )
        .route("/solo-object",
            post(solo_object_post::<I, A, E>)
        )
        .with_state(api_impl)
}
...
where
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::AllOfGetResponse::Status200_OK
                                                    (body)
                                                => {
...
where
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::FileResponseGetResponse::Status200_Success
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
...
where
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::GetStructuredYamlResponse::Status200_OK
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
...
where
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::HtmlPostResponse::Status200_Success
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
...
where
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::RawJsonGetResponse::Status200_Success
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
