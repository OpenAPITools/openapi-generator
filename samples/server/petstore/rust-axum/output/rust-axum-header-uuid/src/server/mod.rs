...
where
    ...
    let header_params = {
                ...
                let header_some_uid = match header_some_uid {
                    Some(v) => match header::IntoHeaderValue::<uuid::Uuid>::try_from((*v).clone()) {
                        ...
                        Err(err) => {
                            return Response::builder()
                                        ...
                                        .body(Body::from(format!("Invalid header some_uid - {err}"))).map_err(|e| { error!(error = ?e); StatusCode::INTERNAL_SERVER_ERROR });
                    ...
                    None => {
                        return Response::builder()
                                        .status(StatusCode::BAD_REQUEST)
                                        .body(Body::from("Missing required header some_uid")).map_err(|e| { error!(error = ?e); StatusCode::INTERNAL_SERVER_ERROR });
                    }
                };
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::default::UsersPostResponse::Status201_AddedRowToTable
                                                    (body)
                                                => {
