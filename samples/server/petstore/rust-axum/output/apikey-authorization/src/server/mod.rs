...
where
    ...
    A: apis::EventDispatcher + apis::payments::Payments<E, Claims = C> + apis::payments::PaymentsAuthorization<Claims = C> + apis::ApiAuthBasic<Claims = C> + apis::ApiAuthBasic<Claims = C> + apis::ApiKeyAuthHeader<Claims = C> + apis::CookieAuthentication<Claims = C> + Send + Sync + 'static,
...
where
    I: AsRef<A> + Send + Sync,
    A: apis::EventDispatcher + apis::payments::Payments<E, Claims = C> + apis::payments::PaymentsAuthorization<Claims = C>+ apis::ApiAuthBasic<Claims = C> + Send + Sync,
    E: std::fmt::Debug + Send + Sync + 'static,
        {
let start_at = chrono::Utc::now();
  ...
  let Ok((
    ...
    return Response::builder()
      ...
      // Authorization
      let authorization = api_impl.as_ref().get_payment_method_by_id_authorize(
          &method,
          &host,
          &cookies,
          &claims,
...
let mut event = apis::event::Event::default();
  ...
  let resp = match result {
                                        ...
                                        if let Ok(resp) = resp.as_ref() && !event.is_empty() {
                                             ...
                                             event.insert(apis::event::convention::EVENT_ACTION.to_string(), "get_payment_method_by_id".to_string());
...
let start_at = chrono::Utc::now();
  ...
  let Ok((
    ...
    return Response::builder()
      ...
      // Authorization
      let authorization = api_impl.as_ref().get_payment_methods_authorize(
          &method,
          &host,
          &cookies,
          &claims,
...
let mut event = apis::event::Event::default();
  ...
  let resp = match result {
                                        ...
                                        if let Ok(resp) = resp.as_ref() && !event.is_empty() {
                                             ...
                                             event.insert(apis::event::convention::EVENT_SERVICE.to_string(), api_impl.as_ref().service_name());
                                             event.insert(apis::event::convention::EVENT_STATUS_CODE.to_string(), resp.status().as_u16().to_string());
                                             event.insert(apis::event::convention::EVENT_ACTION.to_string(), "get_payment_methods".to_string());
                                             event.insert(apis::event::convention::EVENT_LATENCY_SECS.to_string(), format!("{:.6}", chrono::Utc::now().signed_duration_since(start_at).as_seconds_f64()));
                                             api_impl.as_ref().dispatch(event).await;
                                        }
...
where
    I: AsRef<A> + Send + Sync,
    A: apis::EventDispatcher + apis::payments::Payments<E, Claims = C> + apis::payments::PaymentsAuthorization<Claims = C>+ apis::CookieAuthentication<Claims = C>+ apis::ApiAuthBasic<Claims = C> + Send + Sync,
    E: std::fmt::Debug + Send + Sync + 'static,
        {
let start_at = chrono::Utc::now();
  ...
  let Ok((
    ...
    return Response::builder()
      ...
      // Authorization
      let authorization = api_impl.as_ref().post_make_payment_authorize(
          &method,
          &host,
          &cookies,
          &claims,
...
let mut event = apis::event::Event::default();
  ...
  let resp = match result {
                                        ...
                                        if let Ok(resp) = resp.as_ref() && !event.is_empty() {
                                             ...
                                             event.insert(apis::event::convention::EVENT_SERVICE.to_string(), api_impl.as_ref().service_name());
                                             event.insert(apis::event::convention::EVENT_STATUS_CODE.to_string(), resp.status().as_u16().to_string());
                                             event.insert(apis::event::convention::EVENT_ACTION.to_string(), "post_make_payment".to_string());
                                             event.insert(apis::event::convention::EVENT_LATENCY_SECS.to_string(), format!("{:.6}", chrono::Utc::now().signed_duration_since(start_at).as_seconds_f64()));
                                             api_impl.as_ref().dispatch(event).await;
                                        }
