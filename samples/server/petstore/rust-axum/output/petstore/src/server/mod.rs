    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::pet::AddPetResponse::Status200_SuccessfulOperation_PlainText(body) => {
            ...
            apis::pet::AddPetResponse::Status200_SuccessfulOperation_Json(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers
    ...
    E: std::fmt::Debug + Send + Sync + 'static,
{
    ...
    let resp = match result {
        Ok(rsp) => match rsp {
            apis::pet::UpdatePetResponse::Status200_SuccessfulOperation_PlainText(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers.insert(CONTENT_TYPE, HeaderValue::from_static("text/plain"));
            ...
            apis::pet::UpdatePetResponse::Status200_SuccessfulOperation_Json(body) => {
                let mut response = response.status(200);
                {
                    let mut response_headers = response.headers_mut().unwrap();
                    response_headers
