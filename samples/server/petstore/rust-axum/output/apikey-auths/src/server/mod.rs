...
where
    ...
    A: apis::payments::Payments<E, Claims = C> + apis::ApiAuthBasic<Claims = C> + apis::ApiAuthBasic<Claims = C> + apis::ApiKeyAuthHeader<Claims = C> + apis::CookieAuthentication<Claims = C> + Send + Sync + 'static,
    E: std::fmt::Debug + Send + Sync + 'static,
    C: Send + Sync + 'static,
{
    // build our application with a route
    Router::new()
        .route("/v71/paymentMethods",
            get(get_payment_methods::<I, A, E, C>)
        )
        .route("/v71/paymentMethods/{id}",
            get(get_payment_method_by_id::<I, A, E, C>)
        )
        .route("/v71/payments",
            post(post_make_payment::<I, A, E, C>)
        )
        .with_state(api_impl)
}
...
fn get_payment_method_by_id_validation(
  ...
  models::GetPaymentMethodByIdPathParams,
), ValidationErrors>
{
...
where
    ...
    get_payment_method_by_id_validation(
        path_params,
    )
  ).await.unwrap();
  ...
  let result = api_impl.as_ref().get_payment_method_by_id(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::payments::GetPaymentMethodByIdResponse::Status200_OK
                                                    (body)
                                                => {
                                                ...
                                                apis::payments::GetPaymentMethodByIdResponse::Status422_UnprocessableEntity
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
...
#[tracing::instrument(skip_all)]
fn get_payment_methods_validation(
) -> std::result::Result<(
), ValidationErrors>
{
...
where
    ...
    let Some(claims) = claims else {
      ...
      #[allow(clippy::redundant_closure)]
      let validation = tokio::task::spawn_blocking(move ||
    get_payment_methods_validation(
    )
  ).await.unwrap();
  ...
  let result = api_impl.as_ref().get_payment_methods(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::payments::GetPaymentMethodsResponse::Status200_OK
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
...
where
    I: AsRef<A> + Send + Sync,
    A: apis::payments::Payments<E, Claims = C>+ apis::CookieAuthentication<Claims = C>+ apis::ApiAuthBasic<Claims = C> + Send + Sync,
    E: std::fmt::Debug + Send + Sync + 'static,
        {


    // Authentication
    let claims_in_cookie = api_impl.as_ref().extract_claims_from_cookie(&cookies, "X-API-Key").await;
    let claims_in_auth_header = api_impl.as_ref().extract_claims_from_auth_header(apis::BasicAuthKind::Bearer, &headers, "authorization").await;
    let claims = None
             .or(claims_in_cookie)
             .or(claims_in_auth_header)
  ...
  let result = api_impl.as_ref().post_make_payment(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::payments::PostMakePaymentResponse::Status200_OK
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(200);
                                                ...
                                                apis::payments::PostMakePaymentResponse::Status422_UnprocessableEntity
                                                    (body)
                                                => {
                                                let mut response = Response::builder();
                                                  let mut response = response.status(422);
