...
class PetApiValidationTest {
    ...
    // ── @PageableDefault — size default from allOf $ref ───────────────────────
    // Endpoint: GET /pet/findWithDefaultFromAllOfRef  @PageableDefault(size = 7)
    // PetApiController asserts size == 7; returns 200 on success, throws on mismatch.

    @Test
    void pageableDefault_absentSizeParamResolvesToSizeSevenDefault() throws Exception {
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_DEFAULT_FROM_ALL_OF_REF))
                .andExpect(status().isOk());
    }

    // ── @ValidPageable — minSize constraint from allOf $ref ───────────────────
    // Endpoint: GET /pet/findWithMinSizeConstraintFromAllOfRef  minSize = 5

    @Test
    void validPageable_sizeAboveMinimumReturns200() throws Exception {
    ...
    @Test
    void validPageable_sizeAtMinimumReturns200() throws Exception {
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_MIN_SIZE_CONSTRAINT_FROM_ALL_OF_REF)
                        .param("size", "5"))
                .andExpect(status().isOk());
    }
    ...
    @Test
    void validPageable_sizeBelowMinimumReturns400() throws Exception {
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_MIN_SIZE_CONSTRAINT_FROM_ALL_OF_REF)
                        .param("size", "4"))
                .andExpect(status().isBadRequest());
    }
    ...
    @Test
    void validPageable_unpagedPageableIsAllowedForMinConstraint() throws Exception {
        // Unpaged Pageable (no params, no @PageableDefault) bypasses the validator per
        // PageableConstraintValidator#isValid which returns true immediately for !isPaged().
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_MIN_SIZE_CONSTRAINT_FROM_ALL_OF_REF))
                .andExpect(status().isOk());
    }

    // ── @ValidPageable — maxSize constraint from allOf $ref ───────────────────
    // Endpoint: GET /pet/findWithSizeConstraintFromAllOfRef  maxSize = 75

    @Test
    void validPageable_sizeBelowMaximumReturns200_forSizeConstraintFromAllOfRef() throws Exception {
    ...
    @Test
    void validPageable_sizeAtMaximumReturns200_forSizeConstraintFromAllOfRef() throws Exception {
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_SIZE_CONSTRAINT_FROM_ALL_OF_REF)
                        .param("size", "75"))
                .andExpect(status().isOk());
    }
    ...
    @Test
    void validPageable_sizeExceedsMaximumReturns400_forSizeConstraintFromAllOfRef() throws Exception {
        mockMvc.perform(get(PetApi.PATH_FIND_PETS_WITH_SIZE_CONSTRAINT_FROM_ALL_OF_REF)
                        .param("size", "76"))
                .andExpect(status().isBadRequest());
    }
