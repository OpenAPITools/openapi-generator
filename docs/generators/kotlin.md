...
|enumUnknownDefaultCase|Add an `unknown_default_open_api` enum case as a fallback for unrecognized values. Only `moshi`, `jackson` and `kotlinx_serialization`(serializationLibrary) decode unknown values to it; `gson` and `multiplatform` add the case but still fail on unknown values.|<dl><dt>**false**</dt><dd>No changes to the enums are made, this is the default option.</dd><dt>**true**</dt><dd>Each enum gains an `unknown_default_open_api` case that unrecognized values decode to.</dd></dl>|false|
