...
|generateJsonIncludeAnnotations|Whether to generate policy @JsonInclude annotations on model properties. When false, all automatic @JsonInclude annotations (required-field protection and the optional non-nullable policy) are omitted so the global ObjectMapper owns inclusion. A per-property override set via the `x-jackson-json-include-policy` vendor extension is still honored.| |true|
