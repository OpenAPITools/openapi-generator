...
|x-inner-validation|Custom annotation to be placed on the type argument of a collection (List, Set), set on the array/set `items` schema, enabling per-element bean validation constraints (e.g. `@NotNull`)|FIELD|null
