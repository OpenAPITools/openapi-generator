...
public class OptionsTest extends AbstractOptionsTest {
    ...
    protected void verifyOptions() {
        ...
        verify(clientCodegen)
                .setNonPublicApi(
                        Boolean.parseBoolean(XojoClientOptionsProvider.NON_PUBLIC_API_VALUE));
        verify(clientCodegen)
                .setPrependFormOrBodyParameters(
                        Boolean.valueOf(
                                XojoClientOptionsProvider.PREPEND_FORM_OR_BODY_PARAMETERS_VALUE));
        verify(clientCodegen)
                .setEnumUnknownDefaultCase(
                        Boolean.parseBoolean(
                                XojoClientOptionsProvider.ENUM_UNKNOWN_DEFAULT_CASE_VALUE));
        verify(clientCodegen)
                .setSerializationLibrary(XojoClientOptionsProvider.SERIALIZATION_LIBRARY_VALUE);
