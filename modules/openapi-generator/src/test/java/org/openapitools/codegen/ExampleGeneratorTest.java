...
public class ExampleGeneratorTest {
    ...
    public void generateFromResponseSchemaWithPrimitiveType() {
        OpenAPI openAPI =
                TestUtils.parseFlattenSpec("src/test/resources/3_0/example_generator_test.yaml");

        new InlineModelResolver().flatten(openAPI);

        ExampleGenerator exampleGenerator =
                new ExampleGenerator(openAPI.getComponents().getSchemas(), openAPI);
        Set<String> mediaTypeKeys = new TreeSet<>();
        mediaTypeKeys.add("application/json");
        List<Map<String, String>> examples =
                exampleGenerator.generateFromResponseSchema(
                        "200",
                        openAPI.getPaths()
    ...
    public void generateFromResponseSchemaWithArrayOfModel() {
        ...
        assertEquals("application/json", examples.get(0).get("contentType"));
        assertEquals(
                "[ \"string schema example value\", \"string schema example value\" ]",
    ...
    public void generateFromResponseSchemaWithArrayOfPrimitiveTypes() {
        ...
        assertEquals("application/json", examples.get(0).get("contentType"));
        assertEquals(
                "[ \"primitive types example value\", \"primitive types example value\" ]",
                examples.get(0).get("example"));
        assertEquals("200", examples.get(0).get("statusCode"));
    }
    ...
    public void generateFromResponseSchemaWithModel() {
        ...
        assertEquals(
                String.format(
                        Locale.ROOT,
                        "{%n  \"example_schema_property\" : \"example schema property value\"%n}"),
                examples.get(0).get("example"));
        assertEquals("200", examples.get(0).get("statusCode"));
    }
}
