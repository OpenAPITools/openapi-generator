...
public class PlantumlDocumentationCodegenTest {
    private PlantumlDocumentationCodegen plantumlDocumentationCodegen =
            new PlantumlDocumentationCodegen();
    ...
    public void inheritedEntitiesTest() {
        ...
        final Schema childAllOfInlineSchema =
                new Schema()
        ...
        final CodegenModel parentModel =
                plantumlDocumentationCodegen.fromModel("parent", parentSchema);
        final CodegenModel childAllOfModel =
                plantumlDocumentationCodegen.fromModel("child_allOf", childAllOfInlineSchema);
        final CodegenModel childComposedModel =
                plantumlDocumentationCodegen.fromModel("child", composedSchema);

        Map<String, Object> objs =
                createObjectsMapFor(parentModel, childComposedModel, childAllOfModel);
    ...
    public void aggregatedEntitiesTest() {
        OpenAPI openAPI = TestUtils.createOpenAPI();
        final Schema simpleDataTypeSchema =
                new Schema()
        ...
        final Schema tagDataTypeSchema =
                new Schema().description("a tag model").addProperties("name", new StringSchema());
        ...
        plantumlDocumentationCodegen.setOpenAPI(openAPI);
        final CodegenModel simpleModel =
                plantumlDocumentationCodegen.fromModel("simple", simpleDataTypeSchema);
        final CodegenModel tagModel =
                plantumlDocumentationCodegen.fromModel("tag", tagDataTypeSchema);
        final CodegenModel parentModel =
                plantumlDocumentationCodegen.fromModel("parent", parentSchema);
    ...
    public void sharedIdenticalInlineAllOfTest() {
        OpenAPI openAPI = TestUtils.createOpenAPI();
        final Schema parentSchema =
                new Schema().description("a parent model").addProperties("id", new StringSchema());
        ...
        final Schema anotherAllOfInlineSchema =
                new Schema()
        ...
        final CodegenModel parentModel =
                plantumlDocumentationCodegen.fromModel("parent", parentSchema);
        final CodegenModel childAllOfModel =
                plantumlDocumentationCodegen.fromModel("another_allOf", anotherAllOfInlineSchema);
        final CodegenModel childComposedModel =
                plantumlDocumentationCodegen.fromModel("child", composedSchema);
    ...
    private Map<String, Object> getEntityFromList(String name, List<?> entityList) {
        Optional<Map<String, Object>> entity =
                entityList.stream()
                        .map(entityItem -> toMap(entityItem))
                        .filter(entityMap -> hasName(name, entityMap))
                        .findFirst();
    ...
    private Map<String, Object> getFieldFromEntity(String name, Map<String, Object> entity) {
        List<Object> fieldList = (List<Object>) entity.get("fields");
        Optional<Map<String, Object>> field =
                fieldList.stream()
                        .map(fieldItem -> toMap(fieldItem))
                        .filter(fieldMap -> hasName(name, fieldMap))
                        .findFirst();
    ...
    private List<Object> getList(Map<String, Object> objs, String listName) {
        ...
        Assert.assertTrue(
                ...
                "object with name '" + listName + "' in objs map is a list");
