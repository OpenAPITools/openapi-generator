...
public class RClientCodegenTest {
    ...
    public void testDateAndDateTimeModelGeneration() throws IOException {
        ...
        // validateJSON checks are generated by constructValidateJSONCheck in Java
        Assert.assertTrue(content.contains("is.character(input_json$`password`)"),
                "validateJSON should check is.character for the password field");
        Assert.assertTrue(content.contains("is.character(input_json$`date`)"),
                "validateJSON should check is.character for the date field (JSON dates are strings)");

        // container item checks are generated by constructItemCheck in Java
        var petModel = generatedFiles.stream()
                .filter(file -> "pet.R".equals(file.getName())).findFirst();
        if (petModel.isEmpty()) {
            Assert.fail("`pet.R` has not been generated");
        }
        String petContent = String.join("\n", Files.readAllLines(Paths.get(petModel.get().getAbsolutePath())));
        Assert.assertTrue(petContent.contains("stopifnot(is.character(x)))"),
                "container item check for array<string> should use is.character(x)");
    }
