...
public class ConstructorAssert extends AbstractAssert<ConstructorAssert, ConstructorDeclaration> {
    ...
    ConstructorAssert(
            final JavaFileAssert fileAssert, final ConstructorDeclaration constructorDeclaration) {
    ...
    public ParameterAssert hasParameter(final String paramName) {
        ...
        Assertions.assertThat(parameter)
                .withFailMessage(
                        "Constructor %s should have parameter %s, but it doesn't",
                        signature, paramName)
                .isPresent();
    ...
    public ConstructorAssert doesNotHaveParameter(final String paramName) {
        Assertions.assertThat(actual.getParameterByName(paramName))
                .withFailMessage(
                        "Constructor %s shouldn't have parameter %s, but it does",
                        signature, paramName)
                .isEmpty();
        return this;
    }
    ...
    public ConstructorAssert doesNotHaveComment() {
        Assertions.assertThat(actual.getJavadocComment())
                .withFailMessage(
                        "Constructor %s shouldn't contains comment, but it does", signature)
                .isEmpty();
        return this;
    }
    ...
    public ConstructorAssert commentContainsLines(final String... lines) {
        Assertions.assertThat(actual.getJavadocComment())
                .withFailMessage(
                        "Constructor %s should contains comment, but it doesn't", signature)
                .isPresent();
