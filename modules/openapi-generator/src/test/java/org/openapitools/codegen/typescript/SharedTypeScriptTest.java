...
public class SharedTypeScriptTest {
    ...
    private void checkAPIFile(List<File> files, String apiFileName) throws IOException {
        File apiFile =
                files.stream()
                        .filter(file -> file.getName().contains(apiFileName))
                        .findFirst()
                        .get();
        String apiFileContent = FileUtils.readFileToString(apiFile, StandardCharsets.UTF_8);
        Assert.assertTrue(
                !apiFileContent.contains("import { GetCustomer200Response | PersonWrapper }"));
        Assert.assertEquals(
                StringUtils.countMatches(apiFileContent, "import { PersonWrapper }"), 1);
        Assert.assertEquals(
                StringUtils.countMatches(apiFileContent, "import { GetCustomer200Response }"), 1);
    }
    ...
    public void oldImportsStillPresentTest() throws IOException {
        ...
        File pets =
                files.stream().filter(file -> file.getName().contains("pet.ts")).findFirst().get();
    ...
    */
    ...
    public void toModelImportMapTest() {
        ...
        for (Map.Entry<String, String[]> entry : types.entrySet()) {
            String[] mapped =
                    codegen.toModelImportMap(entry.getKey()).values().toArray(new String[0]);
            Arrays.sort(mapped);
