...
public class KtormSchemaCodegenTest {
    ...
    private Map<String, Object> getColumnDefinition(Map<String, Object> schema) {
        return (Map<String, Object>) schema.get("columnDefinition");
    }

    private Map<String, Object> getRelationDefinition(Map<String, Object> schema) {
        return (Map<String, Object>) schema.get("relationDefinition");
    }
    ...
    public void testMatchedColType() {
        ...
        Assert.assertEquals(
                getMatchedColType(new IntegerSchema().type(SchemaTypeUtil.BYTE_FORMAT)), "int");
        Assert.assertEquals(
                getMatchedColType(new IntegerSchema().format(SchemaTypeUtil.BYTE_FORMAT)), "int");
        ...
        Assert.assertEquals(
                getMatchedColType(new IntegerSchema().format(SchemaTypeUtil.INTEGER32_FORMAT)),
                "int");
        Assert.assertEquals(getMatchedColType(new IntegerSchema().type("long")), "long");
        Assert.assertEquals(
                getMatchedColType(new IntegerSchema().format(SchemaTypeUtil.INTEGER64_FORMAT)),
                "long");
        Assert.assertEquals(
                getMatchedColType(new ObjectSchema().type(SchemaTypeUtil.FLOAT_FORMAT)), "float");
        Assert.assertEquals(
                getMatchedColType(new NumberSchema().format(SchemaTypeUtil.FLOAT_FORMAT)), "float");
        Assert.assertEquals(
                getMatchedColType(new ObjectSchema().type(SchemaTypeUtil.DOUBLE_FORMAT)), "double");
        Assert.assertEquals(
                getMatchedColType(new NumberSchema().format(SchemaTypeUtil.DOUBLE_FORMAT)),
                "double");
        Assert.assertEquals(
                getMatchedColType(
                        new ObjectSchema()
                                .type(SchemaTypeUtil.FLOAT_FORMAT)
                                .format(SchemaTypeUtil.DOUBLE_FORMAT)),
                "float");
        Assert.assertEquals(
                getMatchedColType(
                        new ObjectSchema()
                                .type(SchemaTypeUtil.DOUBLE_FORMAT)
                                .format(SchemaTypeUtil.FLOAT_FORMAT)),
                "double");
        ...
        Assert.assertEquals(
                getMatchedColType(new NumberSchema().type(SchemaTypeUtil.NUMBER_TYPE)), "decimal");
        ...
        Assert.assertEquals(
                getMatchedColType(
                        new ArraySchema()
                                .items(new IntegerSchema().type(SchemaTypeUtil.BYTE_FORMAT))),
                "blob");
        Assert.assertEquals(
                getMatchedColType(
                        new ArraySchema()
                                .items(new IntegerSchema().format(SchemaTypeUtil.BYTE_FORMAT))),
                "blob");
    ...
    public void testMatchedColKotlinType() {
        ...
        Assert.assertEquals(
                getMatchedKotlinType(new StringSchema().format("char")), "kotlin.String");
        Assert.assertEquals(getMatchedKotlinType(new BooleanSchema()), "kotlin.Boolean");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().type(SchemaTypeUtil.BYTE_FORMAT)),
                "kotlin.Byte");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().format(SchemaTypeUtil.BYTE_FORMAT)),
                "kotlin.Int"); // *1
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().type("short")), "kotlin.Short");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().format("short")), "kotlin.Int"); // *1
        Assert.assertEquals(getMatchedKotlinType(new IntegerSchema()), "kotlin.Int");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().type("integer")), "kotlin.Int");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().format("integer")), "kotlin.Int"); // *1
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().format(SchemaTypeUtil.INTEGER32_FORMAT)),
                "kotlin.Int");
        Assert.assertEquals(getMatchedKotlinType(new IntegerSchema().type("long")), "kotlin.Long");
        Assert.assertEquals(
                getMatchedKotlinType(new IntegerSchema().format(SchemaTypeUtil.INTEGER64_FORMAT)),
                "kotlin.Long");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type(SchemaTypeUtil.FLOAT_FORMAT)),
                "kotlin.Float");
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().format(SchemaTypeUtil.FLOAT_FORMAT)),
                "kotlin.Float");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type(SchemaTypeUtil.DOUBLE_FORMAT)),
                "kotlin.Double");
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().format(SchemaTypeUtil.DOUBLE_FORMAT)),
                "kotlin.Double");
        Assert.assertEquals(
                getMatchedKotlinType(
                        new ObjectSchema()
                                .type(SchemaTypeUtil.FLOAT_FORMAT)
                                .format(SchemaTypeUtil.DOUBLE_FORMAT)),
                "kotlin.Float"); // *1
        ...
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().format("real")),
                "java.math.BigDecimal"); // *1
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().type(SchemaTypeUtil.NUMBER_TYPE)),
                "java.math.BigDecimal");
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().type("decimal")), "java.math.BigDecimal");
        Assert.assertEquals(
                getMatchedKotlinType(new NumberSchema().type("BigDecimal")),
                "java.math.BigDecimal");
        ...
        Assert.assertEquals(
                getMatchedKotlinType(
                        new ArraySchema()
                                .items(new IntegerSchema().format(SchemaTypeUtil.BYTE_FORMAT))),
                "kotlin.Array<kotlin.Int>"); // *1
        Assert.assertEquals(getMatchedKotlinType(new ArraySchema()), "kotlin.Array<kotlin.String>");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type("list")), "kotlin.collections.List");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type("set")), "kotlin.collections.Set");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type("map")), "kotlin.collections.Map");
        Assert.assertEquals(getMatchedKotlinType(new ObjectSchema()), "kotlin.Any");
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type("binary")), "kotlin.ByteArray");
        Assert.assertEquals(getMatchedKotlinType(new ObjectSchema().type("AnyType")), "kotlin.Any");
        Assert.assertEquals(
                getMatchedKotlinType(new BinarySchema()), "java.io.File"); // looks like a bug
        ...
        Assert.assertEquals(
                getMatchedKotlinType(new ObjectSchema().type("UUID")), "java.util.UUID");
        ...
        Assert.assertEquals(
                getMatchedKotlinType(new StringSchema().format("password")), "kotlin.String");
        Assert.assertEquals(
                getMatchedKotlinType(new StringSchema().type("password")), "kotlin.String");
    }
    ...
    public void testNonMatchedRelation() {
        ...
        Assert.assertEquals(
                getMatchedRelation(new IntegerSchema().type(SchemaTypeUtil.BYTE_FORMAT)), null);
        Assert.assertEquals(
                getMatchedRelation(new IntegerSchema().format(SchemaTypeUtil.BYTE_FORMAT)), null);
        ...
        Assert.assertEquals(
                getMatchedRelation(new IntegerSchema().format(SchemaTypeUtil.INTEGER32_FORMAT)),
                null);
        Assert.assertEquals(getMatchedRelation(new IntegerSchema().type("long")), null);
        Assert.assertEquals(
                getMatchedRelation(new IntegerSchema().format(SchemaTypeUtil.INTEGER64_FORMAT)),
                null);
        Assert.assertEquals(
                getMatchedRelation(new ObjectSchema().type(SchemaTypeUtil.FLOAT_FORMAT)), null);
        Assert.assertEquals(
                getMatchedRelation(new NumberSchema().format(SchemaTypeUtil.FLOAT_FORMAT)), null);
        Assert.assertEquals(
                getMatchedRelation(new ObjectSchema().type(SchemaTypeUtil.DOUBLE_FORMAT)), null);
        Assert.assertEquals(
                getMatchedRelation(new NumberSchema().format(SchemaTypeUtil.DOUBLE_FORMAT)), null);
        Assert.assertEquals(
                getMatchedRelation(
                        new ObjectSchema()
                                .type(SchemaTypeUtil.FLOAT_FORMAT)
        ...
        Assert.assertEquals(
                getMatchedRelation(new NumberSchema().type(SchemaTypeUtil.NUMBER_TYPE)), null);
    ...
    public void testMatchedRelation() {
        ...
        Assert.assertEquals(
                getMatchedRelation(new ObjectSchema().type("UserNamespace.UserClass")),
                "userNamespaceUserClass");
        Assert.assertEquals(
                getMatchedColType(new ObjectSchema().type("UserNamespace.UserClass")), "long");
        // arrays are special case, we convert them to 1:N relations
        Assert.assertEquals(getMatchedRelation(new ArraySchema()), "key");
        Assert.assertEquals(
                getMatchedRelation(new ArraySchema().items(new ObjectSchema().type("Something"))),
                "something");
        Assert.assertEquals(
                getMatchedRelation(
                        new ArraySchema()
                                .items(new ObjectSchema().type("UserNamespace.UserClass"))),
                "userNamespaceUserClass");
        Assert.assertEquals(
                getMatchedRelation(
                        new ArraySchema()
        ...
        Assert.assertEquals(getMatchedColType(new ArraySchema()), "blob");
        Assert.assertEquals(
                getMatchedColType(new ArraySchema().items(new ObjectSchema().type("Something"))),
                "blob");
        Assert.assertEquals(
                getMatchedColType(
                        new ArraySchema()
