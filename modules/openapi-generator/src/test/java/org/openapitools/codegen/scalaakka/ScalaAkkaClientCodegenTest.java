...
public class ScalaAkkaClientCodegenTest {
    ...
    public void arrayAsSetModelTest() {
        ...
        Assert.assertEquals(cm.imports.size(), 2);
        Assert.assertEquals(
                Sets.intersection(cm.imports, Sets.newHashSet("Set", "Children")).size(), 2);
    }
    ...
    public void mapModelTest() {
        ...
        Assert.assertEquals(
                Sets.intersection(cm.imports, Sets.newHashSet("Map", "Children")).size(), 1);
    }
    ...
    public void codeGenerationTest() throws Exception {
        ...
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/model/SomeObj.scala");

        File someObj =
                files.stream().filter(f -> f.getName().equals("SomeObj.scala")).findFirst().get();

        byte[] fileContents = Files.readAllBytes(someObj.toPath());
        Assert.assertEquals(
                new String(fileContents, StandardCharsets.UTF_8),
                Resources.toString(
                        Resources.getResource("codegen/scala/SomeObj.scala.txt"),
    ...
    public void codeGenerationJava8Test() throws Exception {
        ...
        Assert.assertEquals(
                new String(fileContents, StandardCharsets.UTF_8),
                Resources.toString(
                        Resources.getResource("codegen/scala/JavaTimeObj.scala.txt"),
                        StandardCharsets.UTF_8));
    }
    ...
    public void mainPackageTest() throws Exception {
        ...
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/model/SomeObj.scala");
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/core/ApiSettings.scala");
    ...
    public void overridePackagesTest() throws Exception {
        ...
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/model/package/SomeObj.scala");
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/package/invoker/ApiSettings.scala");
        TestUtils.ensureContainsFile(
                files, output, "src/main/scala/hello/world/api/package/PingApi.scala");
    }
}
