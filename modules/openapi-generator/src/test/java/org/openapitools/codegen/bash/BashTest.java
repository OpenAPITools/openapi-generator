...
public class BashTest {
    ...
    public void petstoreOperationTest() {
        ...
        final CodegenOperation op =
                codegen.fromOperation("/pet/findByStatus", "GET", findPetsByStatusOperation, null);
    ...
    public void petstoreParameterExampleTest() {
        ...
        final CodegenOperation op = codegen.fromOperation("/pet", "POST", addPetOperation, null);

        Assert.assertEquals(op.bodyParams.size(), 1);

        CodegenParameter p = op.bodyParams.get(0);

        Assert.assertTrue(p.vendorExtensions.containsKey("x-codegen-body-example"));
    ...
    public void escapeTextTest() {
        ...
        Assert.assertEquals(
                codegen.escapeText("__Bold text__"), "$(tput bold) Bold text $(tput sgr0)");

        Assert.assertEquals(
                codegen.escapeText("**Bold text**"), "$(tput bold) Bold text $(tput sgr0)");

        Assert.assertEquals(
                codegen.escapeText("*Italic text*"), "$(tput dim) Italic text $(tput sgr0)");

        Assert.assertEquals(
                codegen.escapeText("_Italic text_"), "$(tput dim) Italic text $(tput sgr0)");

        Assert.assertEquals(
                codegen.escapeText("# SECTION NAME"),
