...
public class IncludeMarkupFilterTest extends LambdaTest {
    ...
    public void testIncludeMarkupFilterDoesNotIncludeMissingFile() {
        ...
        final Map<String, Object> ctx =
                context(
                        "specinclude",
                        generator.new IncludeMarkupLambda("specDir", "DOES_NOT_EXIST"));

        final String result =
                execute("{{#specinclude}}not.an.existing.file.adoc{{/specinclude}}", ctx);
        Assert.assertTrue(
                result.contains(
                        "// markup not found, no include::{specDir}not.an.existing.file.adoc[opts=optional]"),
    ...
    public void testIncludeMarkupFilterFoundFileOk() throws IOException {
        ...
        final String result =
                execute("{{#snippetinclude}}" + tempFile.getName() + "{{/snippetinclude}}", ctx);
        Assert.assertTrue(
                result.contains("include::{specDir}" + tempFile.getName() + "[opts=optional]"),
                "unexpected filtered: " + result);
    }

    @Test
    public void testIncludeMarkupFilterEscapeCurlyBracketsInOrderToBeParsedByAsciidoc()
            throws IOException {
