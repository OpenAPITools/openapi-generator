...
public class JavaFileAssert extends AbstractAssert<JavaFileAssert, CompilationUnit> {
    ...
    public JavaFileAssert isInterface() {
        Assertions.assertThat(actual.getType(0).asClassOrInterfaceDeclaration())
                .withFailMessage(
                        "Expected type %s to be an interface",
                        actual.getType(0).getName().asString())
    ...
    public JavaFileAssert isNormalClass() {
        Assertions.assertThat(actual.getType(0).asClassOrInterfaceDeclaration())
                .withFailMessage(
                        "Expected type %s to be a normal class(non-abstract)",
                        actual.getType(0).getName().asString())
                .extracting(
                        ClassOrInterfaceDeclaration::isInterface,
                        NodeWithAbstractModifier::isAbstract)
    ...
    public JavaFileAssert isAbstractClass() {
        Assertions.assertThat(actual.getType(0).asClassOrInterfaceDeclaration())
                .withFailMessage(
                        "Expected type %s to be an abstract class",
                        actual.getType(0).getName().asString())
                .extracting(
                        ClassOrInterfaceDeclaration::isInterface,
                        NodeWithAbstractModifier::isAbstract)
    ...
    public MethodAssert assertMethod(final String methodName, final String... paramTypes) {
        ...
        String message =
                paramTypes.length == 0
                        ...
                        : "Expected to be a single method %s with parameter(s) %s, but found "
                                + methods.size();
    ...
    public ConstructorAssert assertConstructor(final String... paramTypes) {
        Optional<ConstructorDeclaration> constructorDeclaration =
                actual.getType(0).getConstructorByParameterTypes(paramTypes);
    ...
    public JavaFileAssert assertNoConstructor(final String... paramTypes) {
        ...
        Assertions.assertThat(constructorDeclaration)
                .withFailMessage(
                        "Found constructor with parameter(s) %s", Arrays.toString(paramTypes))
                .isEmpty();
    ...
    public PropertyAssert hasProperty(final String propertyName) {
        Optional<FieldDeclaration> fieldOptional =
                actual.getType(0).getMembers().stream()
