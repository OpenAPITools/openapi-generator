...
public class JavaHelidonCommonCodegenPackagePrefixTest {
    ...
    private static final String INPUT_FILE =
            "src/test/resources/3_0/helidon/petstore-for-testing.yaml";
    ...
    private static final List<List<String>> GENERATOR_LIBRARY_PAIRS =
    ...
    public void checkValidCombinations(
            String explicitHelidonVersion,
    ...
    public void checkInvalidCombinations(
            String explicitHelidonVersion,
            String explicitPrefix,
            String generatorName,
            String libraryName) {
    ...
    private Object[][] prepareTestData(Object[][] settingsForEachRun) {
        Object[][] result =
                new Object[GENERATOR_LIBRARY_PAIRS.size() * settingsForEachRun.length][];
        int resultSlot = 0;
    ...
    private List<File> runTest(
            String explicitHelidonVersion,
            String explicitPackagePrefix,
            String generatorName,
            String libraryName) {
        Map<String, Object> additionalProperties = new HashMap<>();
        CodegenConfigurator clientConfigurator =
    ...
    private void checkFileForPackagePrefix(
            List<File> files, String generatorName, String libraryName, String expectedPrefix) {
        ...
        if (libraryName.equals("mp")) {
            TestUtils.ensureContainsFile(
                    files,
                    Paths.get(outputDir).toFile(),
                    generatedFilePath(generatorName, libraryName));
            TestUtils.assertFileContains(
                    Paths.get(outputDir + "/" + generatedFilePath(generatorName, libraryName)),
                    "import " + expectedPrefix + ".ws.rs.*;");
            return;
        }

        // The SE server generates 'import {{rootJavaEEPackage}}.json.stream.JsonParser;' in POJOs
        // for JSON-B seriolization.
        TestUtils.ensureContainsFile(
                files,
                Paths.get(outputDir).toFile(),
                generatedFilePath(generatorName, libraryName));
    ...
    private String generatedFilePath(String generatorName, String libraryName) {
        ...
        // The file to check depends on the generator: e.g., PetApi for client, PetService for
        // server.
        ...
        String filePath =
                (libraryName.equals("mp") ? "api/Pet" + apiFileNameSuffix : "model/Pet") + ".java";

        return "src/main/java/org/openapitools/" + serverOrClient + "/" + filePath;
    }
}
