...
public class JavaJAXRSSpecServerCodegen extends AbstractJavaJAXRSServerCodegen {
    ...
    public void processOpts() {
        ...
        if (useRecords && withXml) {
            throw new IllegalArgumentException(
                    "Flag '" + USE_RECORDS + "' cannot be combined with '" + WITH_XML
                            + "': JAXB binding requires mutable JavaBeans with a no-argument "
                            + "constructor, which records cannot provide.");
        }
    ...
    /**
     * A model is rendered as a record when it implements one of the interfaces generated by the
     * useOneOfInterfaces feature and is structurally record-compatible: it is not an enum, has no Java
     * parent and no subclasses (records can neither extend nor be extended), carries no additional
     * (undeclared) properties (those require a mutable map-backed pojo), and is not itself a oneOf/anyOf
     * container. Models outside a oneOf interface hierarchy - ordinary pojos, inheritance participants
     * and map-backed models - are deliberately left untouched.
     *
    ...
    private boolean isOneOfInterfaceRecordCandidate(CodegenModel model, Set<String> oneOfInterfaceNames) {
        if (model.isEnum
                ...
                || model.hasChildren
                || model.getAdditionalPropertiesType() != null
