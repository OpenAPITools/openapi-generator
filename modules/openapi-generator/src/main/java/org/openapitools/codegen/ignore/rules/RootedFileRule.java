...
public class RootedFileRule extends Rule {
    ...
    private String getFilenamePart(final String input, int stopIndex) {
        return input.substring(
                '/' == input.charAt(0) ? 1 : 0, stopIndex > 0 ? stopIndex : input.length());
    }
        ...
        if (isSingleFile) {
            ...
            boolean extensionMatches =
                    definedExtension.equals(extension)
                            || definedExtension.equals(
                                    IgnoreLineParser.Token.MATCH_ANY.getPattern());
                ...
                Pattern regex =
                        Pattern.compile(
                                definedFilename
                                        ...
                                        .replaceAll(
                                                Pattern.quote("*"),
                                                ".*?") // non-greedy match on 0+ any character
                                );
