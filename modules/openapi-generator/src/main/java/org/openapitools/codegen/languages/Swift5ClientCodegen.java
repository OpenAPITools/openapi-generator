...
public class Swift5ClientCodegen extends DefaultCodegen implements CodegenConfig {
    ...
    public Swift5ClientCodegen() {
        ...
        objcReservedWords =
                new HashSet<>(
                        Arrays.asList(
                                ...
                                "NSError",
                                "NSDictionary",
                                // 'Property 'hash' with type 'String' cannot override a property
                                // with type 'Int' (when objcCompatible=true)
                                "hash",
        ...
        cliOptions.add(
                new CliOption(
                        ...
                        CodegenConstants.NON_PUBLIC_API_DESC + "(default: false)"));
        cliOptions.add(
                new CliOption(
                        OBJC_COMPATIBLE,
        ...
        cliOptions.add(
                new CliOption(
                        READONLY_PROPERTIES, "Make properties " + "readonly (default: false)"));
        cliOptions.add(
                new CliOption(
        ...
        cliOptions.add(
                new CliOption(
                        CodegenConstants.API_NAME_PREFIX, CodegenConstants.API_NAME_PREFIX_DESC));
        cliOptions.add(
                new CliOption(
                        USE_SPM_FILE_STRUCTURE,
                        "Use SPM file structure"
                                + " and set the source path to Sources"
                                + File.separator
                                + "{{projectName}} (default: false)."));
        cliOptions.add(
                new CliOption(
                        SWIFT_PACKAGE_PATH,
                        "Set a custom source path instead of "
                                + projectName
                                + File.separator
                                + "Classes"
                                + File.separator
        ...
        cliOptions.add(
                new CliOption(HASHABLE_MODELS, "Make hashable models (default: true)")
                        .defaultValue(Boolean.TRUE.toString()));
    ...
    protected void addAdditionPropertiesToCodeGenModel(CodegenModel codegenModel, Schema schema) {
        ...
        if (additionalProperties != null) {
            ...
            codegenModel.additionalPropertiesType =
                    inner != null ? getTypeDeclaration(inner) : getSchemaType(additionalProperties);
        }
    }
    ...
    public void processOpts() {
        ...
        if (additionalProperties.containsKey(USE_CUSTOM_DATE_WITHOUT_TIME)) {
            setUseCustomDateWithoutTime(
                    convertPropertyToBooleanAndWriteBack(USE_CUSTOM_DATE_WITHOUT_TIME));
        }
        ...
        if (!getLibrary().equals(LIBRARY_VAPOR)) {
            supportingFiles.add(
                    new SupportingFile("Podspec.mustache", "", projectName + ".podspec"));
            supportingFiles.add(new SupportingFile("Cartfile.mustache", "", "Cartfile"));
            supportingFiles.add(
                    new SupportingFile(
                            "CodableHelper.mustache", sourceFolder, "CodableHelper.swift"));
            supportingFiles.add(
                    new SupportingFile(
                            "JSONDataEncoding.mustache", sourceFolder, "JSONDataEncoding.swift"));
            supportingFiles.add(
                    new SupportingFile(
            ...
            supportingFiles.add(
                    new SupportingFile("APIHelper.mustache", sourceFolder, "APIHelper.swift"));
            supportingFiles.add(
                    new SupportingFile("Models.mustache", sourceFolder, "Models.swift"));
        }
        supportingFiles.add(new SupportingFile("Package.swift.mustache", "", "Package.swift"));
        supportingFiles.add(
                new SupportingFile("Configuration.mustache", sourceFolder, "Configuration.swift"));
        supportingFiles.add(
                new SupportingFile("Extensions.mustache", sourceFolder, "Extensions.swift"));
        supportingFiles.add(
                new SupportingFile(
        ...
        if (validatable) {
            supportingFiles.add(
                    new SupportingFile("Validation.mustache", sourceFolder, "Validation.swift"));
        }
        supportingFiles.add(new SupportingFile("gitignore.mustache", "", ".gitignore"));
        supportingFiles.add(new SupportingFile("README.mustache", "", "README.md"));
        supportingFiles.add(new SupportingFile("swiftformat.mustache", "", ".swiftformat"));
    ...
    public String getTypeDeclaration(Schema p) {
        if (ModelUtils.isArraySchema(p)) {
            ...
            return ModelUtils.isSet(p)
                    ? "Set<" + getTypeDeclaration(inner) + ">"
    ...
    public ModelsMap postProcessModels(ModelsMap objs) {
        ...
        //
        // Also, if any property in the model has x-codegen-escaped-property-name=true, then we
        // mark:
        //
                    ...
                    if (prop.vendorExtensions.get("x-null-encodable").toString().equals("true")) {
                        if (prop.defaultValue == null || prop.defaultValue.equals("null")) {
                            prop.vendorExtensions.put(
                                    "x-null-encodable-default-value", ".encodeNull");
                        } else {
                            prop.vendorExtensions.put(
                                    "x-null-encodable-default-value",
        ...
        for (CodegenOperation operation : operations) {
            for (CodegenParameter cp : operation.allParams) {
                cp.vendorExtensions.put(
                        "x-swift-example", constructExampleCode(cp, modelMaps, new HashSet<>()));
            }
        }
        return objs;
    }
    ...
    public String constructExampleCode(
        ...
        if (codegenParameter.isArray) { // array
            ...
            if ("String".equals(codegenParameter.dataType)
                    || "Character".equals(codegenParameter.dataType)) {
            ...
            if (modelMaps.containsKey(codegenParameter.dataType)) {
                if (visitedModels.contains(codegenParameter.dataType)) {
                    ...
                    return constructExampleCode(
                            modelMaps.get(codegenParameter.dataType), modelMaps, visitedModels);
                }
            } else {
    ...
    public String constructExampleCode(
        ...
        if (codegenProperty.isArray) { // array
            ...
            if ("String".equals(codegenProperty.dataType)
                    || "Character".equals(codegenProperty.dataType)) {
            ...
            if (modelMaps.containsKey(codegenProperty.dataType)) {
                if (visitedModels.contains(codegenProperty.dataType)) {
                    ...
                    return constructExampleCode(
                            modelMaps.get(codegenProperty.dataType), modelMaps, visitedModels);
                }
            } else {
                // LOGGER.error("Error in constructing examples. Failed to look up the model " +
        ...
        for (CodegenProperty codegenProperty : codegenModel.vars) {
            propertyExamples.add(
                    codegenProperty.name
                            + ": "
                            + constructExampleCode(codegenProperty, modelMaps, visitedModels));
        }
    ...
    public void postProcess() {
        ...
        System.out.println(
                "#                                                                              #");
        System.out.println(
                "# swift5 generator is contributed by Bruno Coelho (https://github.com/4brunu). #");
        System.out.println(
                "# Please support his work directly via https://paypal.com/paypalme/4brunu \uD83D\uDE4F   #");
        System.out.println(
                "################################################################################");
    }
