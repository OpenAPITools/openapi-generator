...
public abstract class CppBoostBeastModelCodegen extends AbstractCppCodegen {
    ...
    protected CppBoostBeastModelCodegen() {
    ...
    protected static final String FORMAT_ASSERTION_POLICY_ANNOTATION = "annotation";
    /** Controls composition-branch validation during model decoding. */
    protected boolean validateOnDecode = true;
    /** Retains undeclared JSON object members in generated object models. */
    protected boolean preserveAdditionalProperties = false;
    ...
    protected Map<String, String> componentSchemaIdsByName = Collections.emptyMap();
    /** Starts an isolated state set for one generator invocation. */
    protected void beginGeneration(OpenAPI openApi) {
     ...
     */
    protected boolean detectExplicitRootServers() {
    ...
    protected static boolean isSchemaValidationSupportingFile(SupportingFile file) {
     ...
     */
    protected void codegenParameterStyled(Parameter parameter,
    ...
    protected String queryCollectionDelimiter(Parameter.StyleEnum style) {
    ...
    protected void addContainerPropertyNames(List<CodegenProperty> properties) {
    ...
    protected void reserveExtraJsonPropertyIdentifiers(CodegenModel codegenModel) {
     ...
     */
    protected String lowerInlineComposedSchema(Schema p) {
    ...
    protected String explicitScalarDefaultValue(CodegenProperty property, Schema schema) {
        ...
        // Genuine containers lower to std::vector / std::map / std::set, where
        // a scalar initializer cannot even compile — OpenAI's
        // Eval.testing_criteria declares `default: eval` on an array of
        // graders. JSON Schema keeps such a default as an annotation the
        // decode path cannot honour; drop it so the member value-initializes
        // instead of emitting broken code. Composed carriers keep their
        // scalar default even when an alternative is a container: the final
        // member is a variant alias, and postProcessAllModels rewrites the
        // default to fromJsonValue_<alias>(...).
        if (property.isContainer) {
            return null;
        }
        if (nullableInner instanceof String) {
            String inner = (String) nullableInner;
            if (inner.startsWith("std::vector<")
                    || inner.startsWith("std::map<")
                    || inner.startsWith("std::set<")) {
                return null;
            }
        }
    ...
    protected static String explicitNumericDefault(
    ...
    protected void refreshComponentSchemaIds(OpenAPI openApi) {
    ...
    /** Shared namespace/validation options common to client and server generators. */
    protected void applySharedCppOptions() {
        ...
        if (!validateOnDecode) {
            supportingFiles.removeIf(CppBoostBeastModelCodegen::isSchemaValidationSupportingFile);
        }
    ...
    public Map<String, ModelsMap> updateAllModels(Map<String, ModelsMap> objs)  {
        ...
        // Stamp every model with its component schema IR id after upstream
        // model updates complete (mirrors the pre-refactor client pipeline).
        refreshComponentSchemaIds(openAPI);
        for (Map.Entry<String, ModelsMap> entry : objs.entrySet()) {
            for (ModelMap modelMap : entry.getValue().getModels()) {
                CodegenModel model = modelMap.getModel();
                String schemaName = model.schemaName != null
                        ? model.schemaName : entry.getKey();
