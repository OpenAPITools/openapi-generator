...
/**
 * A standalone instance for evaluating rule and recommendations related to OAS {@link
...
class OpenApiOperationValidations extends GenericValidator<OperationWrapper> {
     ...
     *
     * <p>RFC7231 describes this behavior as:
     *
     * <p>A payload within a GET request message has no defined semantics; sending a payload body on
     * a GET request might cause some existing implementations to reject the request.
     *
     * <p>See https://tools.ietf.org/html/rfc7231#section-4.3.1
     *
     * <p>Because there are no defined semantics, and because some client and server implementations
     * may silently ignore the entire body (see https://xhr.spec.whatwg.org/#the-send()-method) or
     * throw an error (see https://fetch.spec.whatwg.org/#ref-for-dfn-throw%E2%91%A1%E2%91%A1), we
     * maintain that the existence of a body for this operation is most likely programmer error and
     * raise awareness.
     *
     ...
     *     ValidationRule.Fail}
     */
    private static ValidationRule.Result checkAntipatternGetOrHeadWithBody(
        ...
        if (wrapper.getHttpMethod() == PathItem.HttpMethod.GET
                || wrapper.getHttpMethod() == PathItem.HttpMethod.HEAD) {
            RequestBody body = wrapper.getOperation().getRequestBody();

            if (body != null) {
                if (StringUtils.isNotEmpty(body.get$ref())
                        || (body.getContent() != null && body.getContent().size() > 0)) {
