...
public class CppHttplibServerCodegen extends AbstractCppCodegen {
    ...
    public OperationsMap postProcessOperationsWithModels(OperationsMap objs, List<ModelMap> allModels) {
        ...
        for (CodegenOperation op : operationList) {
            ...
            op.vendorExtensions.put("handlerFunctionRequest", toHandlerFunctionRequest(op.path.toString()));
            ...
            List<Map<String, String>> successCodeToTypes = new ArrayList<>();
            List<Map<String, String>> errorCodeToTypes = new ArrayList<>();
            for (CodegenResponse resp : op.responses) {
                ...
                if (resp.code != null) {
                    ...
                    if (hasAnyResponseSchema) {
                        // Add status code const for successType only once per unique type/const pair
                        if(successType != null && !successType.isEmpty()) {
                            String successTypeName = successType.replace("models::", "").replaceAll("::", "_");
                            String successConstName = HTTP_RESPONSE_PREFIX + StringUtils.underscore(successTypeName).toUpperCase(Locale.ROOT);
                            String uniqueKey = successType + ":" + successConstName;
                            // if (!seenSuccessTypes.contains(uniqueKey)) {
                                // seenSuccessTypes.add(uniqueKey);
                                boolean successConstExists = opStatusCodeConsts.stream()
                                        .anyMatch(listitem -> successConstName.equals(listitem.get("constName")));
                                if (!successConstName.isEmpty() && !successConstExists) {
                                    Map<String, String> item = new HashMap<>();
                                    item.put("constName", successConstName);
                                    item.put("statusCode", resp.code);
                                    opStatusCodeConsts.add(item);
                                }
                                Map<String, String> successItem = new HashMap<>();
                        ...
                        // Add to errorTypes list (can be multiple error types)
                        if(errorType != null && !errorType.isEmpty()) {
                            String errorTypeName = errorType.replace("models::", "").replaceAll("::", "_");
                            String errorConstName =  HTTP_RESPONSE_PREFIX + StringUtils.underscore(errorTypeName).toUpperCase(Locale.ROOT);
                            boolean errorConstExists = opStatusCodeConsts.stream()
                                    .anyMatch(listitem -> errorConstName.equals(listitem.get("constName")));
    ...
    public String toHandlerFunctionRequest(String path) {
        ...
        return toPascalCase(className + "RequestParams");
    }
     ...
     */
    private void setCppTypeFlags(CodegenParameter param) {
        // Prefer baseType if available, otherwise fall back to dataType
        String type = param.baseType != null && !param.baseType.isEmpty() ? param.baseType : param.dataType;
        if (type == null) {
            type = "";
        }
        boolean isString = type.equals("std::string") || type.equals("string");
        boolean isInt = type.equals("int") || type.equals("int32_t") || type.equals("int64_t") || type.equals("long") || type.equals("integer");
        boolean isBool = type.equals("bool") || type.equals("boolean");
        boolean isDouble = type.equals("double") || type.equals("float") || type.equals("number");
        boolean isPrimitive = isInt || isBool || isDouble;

        param.vendorExtensions.put("isString", isString);
        param.vendorExtensions.put("isInt", isInt);
        param.vendorExtensions.put("isBool", isBool);
        param.vendorExtensions.put("isDouble", isDouble);
        param.vendorExtensions.put("isPrimitive", isPrimitive);
    }
}