...
/**
 * An interface for querying and mutating a JSON object graph. Clients indicate the location within
 * the graph at which a value is to be read or written using a <a
 * href="https://tools.ietf.org/html/rfc6901">JSON Pointer</a> as either a string or a <code>
 * JsonPointer</code> instance. The latter is more efficient for repeated access to the same
 * location. For the <code>set(ptr|path, value)</code> and <code>getXxx(ptr|path, defaultValue)
 * </code>methods, the cache automatically creates missing container elements in the path as either
 * object or array nodes according to whether the corresponding path segment is a property name or
 * an integer index respectively, and creates the leaf element as a JSON value node of the
 * appropriate type. Existing tree elements identified by a JSON pointer must match the pointer
 * segments by type; that is, an existing parent of an integer path segment <b>must</b> be an array
 * node and existing parent of a property name path segment <b>must</b> be an object node: if this
 * condition is not met, the method throws a {@linkplain CacheException}. Note that it is possible
 * to store the same object at multiple locations within the tree (thus converting the tree to a
 * <i>graph</i>), in which case the same object may be retrieved using any of the corresponding
 * paths. However, this shared object becomes multiple independent objects when serialised to JSON
 * and if subsequently loaded into a cache instance. Such graphs <b>must</b> be acyclic: storing a
 * cyclic graph in the tree will cause a stack overflow when the cache is saved. Instances are not
 * guaranteed threadsafe and require external synchronisation if mutator methods may be called
 * concurrently from multiple threads. Sparse arrays are not supported - all array elements have a
 * value, even if it is null.
 *
 * <p><em>N.B. some <code>getXxx()</code> methods can return mutable objects, mutations to which
 * will be unobserved by the cache instance, thus compromising the reliability of the <code>flush*()
 * </code> methods. Callers relying on these methods should either not mutate such objects or they
 * should call the appropriate <code>set()</code> method afterwards to ensure that the cache's
 * 'modified' flag is set.</em>
 *
...
public interface JsonCache {
    ...
    interface Factory {
        ...
        /**
         * Returns the singleton cache root instance for the specified key. The same instance is
         * returned every time the method is called with the same <code>key</code> value, being
         * lazily created on the first such call.
         *
    ...
    interface Root extends JsonCache {
        /**
         * Describes cache behaviour when a load method is called for a cache instance that is
         * already loaded.
         */
        enum MergePolicy {
            /**
            ...
            NO_MERGE,
            /**
             * Recursively merges the incoming tree into the existing tree, retaining existing leaf
             * properties.
             */
            MERGE_RECURSIVE,
         ...
         * @throws CacheException if the root node does not exist or if unable to create or write
         *     the file.
        ...
        /**
         * Makes a best-effort attempt to ensure that the cache gets flushed to a disk file if dirty
         * on shutdown. The call has no additional effect if the shutdown hook has already been
         * registered.
         *
         * @param file The output file.
         * @return The receiver, to allow chaining.
        ...
        /**
         * Makes a best-effort attempt to ensure that the cache gets flushed to an output stream if
         * dirty on shutdown. The call has no additional effect if the shutdown hook has already
         * been registered.
         *
        ...
        /**
         * Returns the mapper used for JSON-object marshalling and serialisation operations. Callers
         * may configure the mapper to achieve the desired JSON serialisation format.
         *
         * @return The mapper.
        ...
        /**
         * Returns the merge policy that applies when <code>load()</code> is called on a cache that
         * is already loaded. The default is {@link MergePolicy#MERGE_RECURSIVE};
         *
        ...
        /**
         * Loads the cache from the specified file. If the cache is already loaded, merges the
         * incoming tree according to the current {@link #mergePolicy(MergePolicy) merge policy}.
         * The call has no effect if the file does not exist.
         *
         * @param file The JSON file to load.
         * @return The receiver, to allow chaining.
         * @throws CacheException if the file exists but could not be read or its content is not
         *     valid JSON.
         * @throws NullPointerException if <code>file</code> is <code>null</code>.
         * @see #save(File)
         * @see #save(OutputStream)
        ...
        /**
         * Loads the cache from the specified stream. If the cache is already loaded, merges the
         * incoming tree according to the current {@link #mergePolicy(MergePolicy) merge policy}.
         *
        ...
        /**
         * Sets the merge policy that applies when <code>load()</code> is called for a cache that is
         * already loaded.
         *
    ...
    /**
     * Adds a <code>BigDecimal</code> to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
     *     otherwise, it is appended to the end of the array.
     * @param value The value to add.
     * @return The receiver, to support chaining.
     * @throws CacheException if <code>ptr</code> is not a valid path within the object graph
     *     managed by the receiver.
     */
    JsonCache add(JsonPointer ptr, BigDecimal value) throws CacheException;

    /**
     * Adds a <code>BigInteger</code> to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
    ...
    /**
     * Adds a <code>double</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
    ...
    /**
     * Adds a <code>float</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
    ...
    /**
     * Adds an <code>int</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
    ...
    /**
     * Adds a <code>long</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
    ...
    /**
     * Adds a <code>short</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param ptr A pointer to the property to set. If the last segment is a positive integer less
     *     than the current array size, <code>value</code> is inserted at the specified index;
     ...
     * exist.
     *
     * @param path A JSON Pointer expression for the property to set. If the last segment is a
     *     positive integer less than the current array size, <code>value</code> is inserted at the
     *     specified index; otherwise, it is appended to the end of the array.
     * @param value The value to add.
     * @return The receiver, to support chaining.
     * @throws IllegalArgumentException if <code>path</code> is not a valid JSON path expression.
     * @throws CacheException if <code>path</code> is not a valid path within the object graph
     *     managed by the receiver.
     */
    ...
    /**
     * Adds a <code>boolean</code> value to an array. The array is created if it does not already
     * exist.
     *
     * @param path A JSON Pointer expression for the property to set. If the last segment is a
     *     positive integer less than the current array size, <code>value</code> is inserted at the
     ...
     * Returns a child cache rooted at the given location relative to the receiver's base pointer.
     *
     * @param path A JSON Pointer expression for the subtree managed by the new cache. Note that the
     *     expression must start with a forward slash character.
     * @return The child cache.
     * @throws IllegalArgumentException if <code>path</code> is not a valid JSON path expression.
     */
    ...
    /**
     * Tests for the existence of the specified path within the object graph managed by the
     * receiver.
     *
    ...
    /**
     * Retrieves an <code>Object</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A pointer to the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
     *     graph.
     ...
     * @param defaultValue The default value to return if <code>path</code> is not present in the
     *     graph.
     * @return the property value or <code>defaultValue</code> if not present.
     * @throws CacheException if <code>path</code> is not a valid path within the object graph
     *     managed by the receiver.
    ...
    /**
     * Retrieves a <code>BigDecimal</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A pointer to the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>BigInteger</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>byte[]</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>boolean</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>double</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>float</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves an <code>int</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>long</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>Number</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
     ...
     * @return the property value or <code>null</code> if not present.
     * @throws CacheException if <code>ptr</code> is not a valid path within the object graph
     *     managed by the receiver or if the node could not be converted to the requested type.
     */
     ...
     * @throws CacheException if <code>ptr</code> is not a valid path within the object graph
     *     managed by the receiver or if the nodes could not be converted to the requested type.
     */
    <T> List<T> getObjects(JsonPointer ptr, Class<T> type) throws CacheException;

    /**
     * Retrieves a typed list of objects from within the graph. If the values are not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param <T> The list element type.
     * @param ptr A pointer to the values to return.
     * @param type The list element type.
     * @param defaultValue The default values to return if <code>ptr</code> is not present in the
     *     graph.
     * @return the property values.
     * @throws CacheException if <code>ptr</code> is not a valid path within the object graph
     *     managed by the receiver or if the nodes could not be converted to the requested type.
     */
    <T> List<T> getObjects(JsonPointer ptr, Class<T> type, List<T> defaultValue)
     ...
     * @param path A JSON Pointer expression for the values to return.
     * @param type The list element type.
     * @param defaultValue The default values to return if <code>path</code> is not present in the
     *     graph.
     * @return the property values.
     * @throws CacheException if <code>ptr</code> is not a valid path within the object graph
     *     managed by the receiver or if the nodes could not be converted to the requested type.
    ...
    /**
     * Retrieves a <code>short</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A JSON Pointer expression for the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Retrieves a <code>String</code> value from within the graph. If the value is not present,
     * stores <code>defaultValue</code> at the specified location and returns this value.
     *
     * @param ptr A pointer to the value to return.
     * @param defaultValue The default value to return if <code>ptr</code> is not present in the
    ...
    /**
     * Returns the size of a node within the object graph managed by the receiver. For an array
     * node, size is the number of elements; for an object node, size is the number of properties;
     * for other node types, size is <code>0</code>.
     *
