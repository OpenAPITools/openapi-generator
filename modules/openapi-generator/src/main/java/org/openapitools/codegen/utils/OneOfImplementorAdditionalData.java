 ...
 *
 * <p>Foo: properties: x: oneOf: - $ref: "#/components/schemas/One - $ref: "#/components/schemas/Two
 * y: type: string One: properties: z: type: string Two: properties: a: type: string
 *
 * <p>In codegens that use this mechanism, `Foo` will become an interface and `One` will become its
 * implementing class. This class carries all data necessary to properly modify the implementing
 * class model. Specifically:
 *
 * <p>* Interfaces that the implementing classes have to implement (in the example above, `One` and
 * `Two` will implement `Foo`) * Properties that need to be added to implementing classes (as `Foo`
 * is interface, the `y` property will get pushed to implementing classes `One` and `Two`) * Imports
 * that need to be added to implementing classes (e.g. if type of property `y` needs a specific
 * import, it needs to be added to `One` and `Two` because of the above point)
 */
public class OneOfImplementorAdditionalData {
     ...
     * @param addInterfaceImports whether or not to add the interface model as import (will vary by
     *     language)
     */
    @SuppressWarnings("unchecked")
    public void addToImplementor(
            CodegenConfig cc,
            CodegenModel implcm,
        ...
        // Add oneOf-containing models properties - we need to properly set the hasMore values to
        // make rendering correct
        implcm.vars.addAll(additionalProps);
        implcm.hasVars = !implcm.vars.isEmpty();

        // Add imports
        for (Map<String, String> oneImport : additionalImports) {
            // exclude imports from this package - these are imports that only the oneOf interface
            // needs
            if (!implImports.contains(oneImport)
                    && !oneImport.getOrDefault("import", "").startsWith(cc.modelPackage())) {
