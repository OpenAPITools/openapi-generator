...
class OpenApiSchemaValidations extends GenericValidator<SchemaWrapper> {
    ...
    private static ValidationRule.Result checkOneOfWithProperties(SchemaWrapper schemaWrapper) {
        ...
        if (schema instanceof ComposedSchema) {
            ...
            // This is a recommendation because the 3.0.x spec is not clear enough on usage of
            // oneOf.
            // see https://json-schema.org/draft/2019-09/json-schema-core.html#rfc.section.9.2.1.3
            // and the OAS section on 'Composition and Inheritance'.
            if (composed.getOneOf() != null && composed.getOneOf().size() > 0) {
                if (composed.getProperties() != null
                        && composed.getProperties().size() >= 1
                        && composed.getProperties().get("discriminator") == null) {
                    // not necessarily "invalid" here, but we trigger the recommendation which
                    // requires the method to return false.
     ...
     *
     * <p>'null' type is supported in OpenAPI Specification 3.1 and above. It is not supported in
     * OpenAPI 3.0.x. Note: the validator invokes checkNullType() for every top-level schema in the
     * OAS document. The method is not called for nested schemas that are defined inline.
     *
                ...
                if (ModelUtils.isNullType(schema)) {
                    ...
                    result.setDetails(
                            String.format(
                                    ...
                                    name,
                                    schemaWrapper.getOpenAPI().getOpenapi()));
                    return result;
                }
            }
        }
     ...
     *
     * <p>The 'nullable' attribute is supported in OpenAPI Specification 3.0.x, but it is deprecated
     * in OpenAPI 3.1 and above.
     *
     * @param schema An input schema, regardless of the type of schema
     * @return {@link ValidationRule.Pass} if the check succeeds, otherwise {@link
     *     ValidationRule.Fail}
     ...
     *
     * <p>The type must be one of the following values: null, boolean, object, array, number,
     * string, integer.
     *
     * @param schema An input schema, regardless of the type of schema
     * @return {@link ValidationRule.Pass} if the check succeeds, otherwise {@link
