...
public class PythonExperimentalClientCodegen extends PythonLegacyClientCodegen {
    ...
    public void processOpts() {
        ...
        supportingFiles.add(new SupportingFile("enums.mustache", packagePath(), "enums.py"));
    ...
    public CodegenModel fromModel(String name, Schema sc) {
        ...
        while (queue.size() > 0) {
            ...
            if (schema.getRequiredVars() != null && schema.getRequiredVars().size() > 0) {
                for (IJsonSchemaValidationProperties prop: schema.getRequiredVars()) {
                    prop.setIndent(currentIndent);
                    queue.add(prop);
                }
            }
            if (schema.getComposedSchemas() != null) {
                String composedIndent = currentIndent + oneIndentationLevel;
                if (schema.getComposedSchemas().allOf != null) {
                    List<IJsonSchemaValidationProperties> items = schema.getComposedSchemas().allOf;
                    for (IJsonSchemaValidationProperties item: items) {
                        item.setIndent(composedIndent);
                        queue.add(item);
                    }
                }
                if (schema.getComposedSchemas().oneOf != null) {
                    List<IJsonSchemaValidationProperties> items = schema.getComposedSchemas().oneOf;
                    for (IJsonSchemaValidationProperties item: items) {
                        item.setIndent(composedIndent);
                        queue.add(item);
                    }
                }
                if (schema.getComposedSchemas().anyOf != null) {
                    List<IJsonSchemaValidationProperties> items = schema.getComposedSchemas().anyOf;
                    for (IJsonSchemaValidationProperties item: items) {
                        item.setIndent(composedIndent);
                        queue.add(item);
                    }
