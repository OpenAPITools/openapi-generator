...
/**
 * Provides calling code a way to log important messages only once, regardless of how many times the
 * invocation has occurred. This can be used, for instance, to log a warning like "One or more
 * schemas aren't declared" without logging that message for every time the schema is mentioned in a
 * document.
 *
 * <p>This implementation currently only supports single-argument string literal log methods (e.g.
 * {@link Logger#debug(String)}).
 */
@SuppressWarnings("FieldCanBeLocal")
public class OnceLogger extends LoggerWrapper {
    /**
     * Allow advanced users to modify cache size of the OnceLogger (more for performance tuning in
     * hosted environments)
     */
    static final String CACHE_SIZE_PROPERTY = "org.openapitools.codegen.utils.oncelogger.cachesize";

    /**
     * Allow advanced users to disable the OnceLogger (more for performance tuning in hosted
     * environments). This is really only useful or necessary if this implementation causes issues.
     */
    static final String ENABLE_ONCE_LOGGER_PROPERTY =
            "org.openapitools.codegen.utils.oncelogger.enabled";

    /**
     * Allow advanced users to modify cache expiration of the OnceLogger (more for performance
     * tuning in hosted environments)
     */
    ...
    /**
     * The fully qualified class name of the <b>logger instance</b>, typically the logger class,
     * logger bridge or a logger wrapper.
     */
    private static final String FQCN = OnceLogger.class.getName();

    /**
     * Gets the marker instance. This can be used by supported log implementations to filter/manage
     * logs coming from this implementation differently than others (i.e. make them stand out since
     * they're to be logged once).
     */
    private static final Marker MARKER = MarkerFactory.getMarker("ONCE");

    /** The allowed size of the cache. */
    private static int maxCacheSize =
            Integer.parseInt(GlobalSettings.getProperty(CACHE_SIZE_PROPERTY, "200"));

    /** The millis to expire a cached log message. */
    private static int expireMillis =
            Integer.parseInt(GlobalSettings.getProperty(EXPIRY_PROPERTY, "2000"));
    ...
    static void caffeineCache(Ticker ticker, int expireMillis) {
        ...
        // The intent is to debounce log messages such that they occur at most [maxRepetitions] per
        // [expireMillis].
        messageCountCache =
                Caffeine.newBuilder()
    ...
    public static Logger once(Logger logger) {
        try {
            if (Boolean.parseBoolean(
                    GlobalSettings.getProperty(ENABLE_ONCE_LOGGER_PROPERTY, "true"))) {
                return new OnceLogger(logger);
            }
        } catch (Exception ex) {
            logger.warn(
                    "Unable to wrap logger instance in OnceLogger. Falling back to non-decorated implementation, which may be noisy.");
        }
        return logger;
    }
     ...
     * Delegate to the appropriate method of the underlying logger.
     *
     * <p>Use this method sparingly. If you're limiting error messages, ask yourself whether your
     * log fits better as a warning.
     *
     * @param msg The log message.
     */
    @Override
