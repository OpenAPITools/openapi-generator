...
public class JavaMicronautServerCodegen extends JavaMicronautAbstractCodegen {
    ...
    public JavaMicronautServerCodegen() {
        ...
        cliOptions.add(
                new CliOption(
                                ...
                                "The package in which controllers will be generated")
        ...
        cliOptions.add(
                CliOption.newBoolean(
                        ...
                        "Generate an abstract class for controller to be extended. ("
                                + CodegenConstants.API_PACKAGE
                                + " is then used for the abstract class, and "
                                + OPT_CONTROLLER_PACKAGE
        ...
        cliOptions.add(
                CliOption.newBoolean(
                        OPT_USE_AUTH,
                        "Whether to import authorization and to annotate controller methods accordingly",
                        useAuth));
    ...
    public void processOpts() {
        ...
        if (additionalProperties.containsKey(OPT_GENERATE_CONTROLLER_AS_ABSTRACT)) {
            generateControllerAsAbstract =
                    convertPropertyToBoolean(OPT_GENERATE_CONTROLLER_AS_ABSTRACT);
        }
        ...
        if (additionalProperties.containsKey(OPT_GENERATE_OPERATIONS_TO_RETURN_NOT_IMPLEMENTED)) {
            generateOperationsToReturnNotImplemented =
                    convertPropertyToBoolean(OPT_GENERATE_OPERATIONS_TO_RETURN_NOT_IMPLEMENTED);
        }
        writePropertyBack(
                OPT_GENERATE_OPERATIONS_TO_RETURN_NOT_IMPLEMENTED,
                generateOperationsToReturnNotImplemented);
        ...
        } else if (!generateControllerAsAbstract
                && additionalProperties.containsKey(CodegenConstants.API_PACKAGE)) {
        ...
        if (additionalProperties.containsKey(OPT_GENERATE_CONTROLLER_FROM_EXAMPLES)) {
            generateControllerFromExamples =
                    convertPropertyToBoolean(OPT_GENERATE_CONTROLLER_FROM_EXAMPLES);
        }
        ...
        supportingFiles.add(
                new SupportingFile("server/doc/README.mustache", "", "README.md").doNotOverwrite());
        ...
        supportingFiles.add(
                new SupportingFile(
                                "common/configuration/Application.mustache",
                                invokerFolder,
                                "Application.java")
                        .doNotOverwrite());
    }
        ...
        if (generateControllerAsAbstract && templateName.contains("controllerImplementation")) {
            ...
            return (implementationFolder
                            ...
                            + StringUtils.camelize(
                                    controllerPrefix + "_" + tag + "_" + controllerSuffix)
                            + ".java")
            ...
            for (CodegenOperation operation : allOperations) {
                if (!operation.vendorExtensions.containsKey(EXTENSION_ROLES)) {
                    ...
                    operation.vendorExtensions.put(
                            EXTENSION_ROLES, Collections.singletonList(role));
                } else {
                    List<String> roles =
                            (List<String>) operation.vendorExtensions.get(EXTENSION_ROLES);
                    roles =
                            roles.stream()
                                    .map(
                                            role -> {
