...
public abstract class AbstractJavaCodegen extends DefaultCodegen implements CodegenConfig,
    ...
    public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> objs) {
        ...
        */
        ...
        // Drop the lombok.Getter/lombok.Data annotations so explicit getters are generated.
        if (optionalGettersForNullableFieldsOnly && lombokAnnotations != null && !lombokAnnotations.isEmpty()) {
            boolean removed = false;
            if (lombokAnnotations.containsKey("Getter")) {
                lombokAnnotations.remove("Getter");
                removed = true;
            }
            if (lombokAnnotations.containsKey("Data")) {
                lombokAnnotations.remove("Data");
                removed = true;
            }
            if (removed) {
                writePropertyBack(LOMBOK, new HashMap<>(lombokAnnotations));
                LOGGER.warn("{} is enabled: Lombok Getter/Data annotations have been disabled so that explicit " +
                        "Optional getters can be generated. Re-enable Lombok by disabling this option.",
