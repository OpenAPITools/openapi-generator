...
public class WsdlSchemaCodegen extends DefaultCodegen implements CodegenConfig {
    ...
    public WsdlSchemaCodegen() {
        ...
        supportingFiles.add(
                new SupportingFile("jaxb-customization.mustache", "", "jaxb-customization.xml"));
    }
        ...
        if (additionalProperties.containsKey("useSpecifiedOperationId")) {
            this.setUseSpecifiedOperationId(
                    Boolean.parseBoolean(
                            additionalProperties.get("useSpecifiedOperationId").toString()));
        }
    }
    ...
    private String escapeTitle(String title) {
        ...
        final String normalizedTitle =
                Normalizer.normalize(title, Normalizer.Form.NFD).replaceAll("[^\\p{ASCII}]", "");
    ...
    public OperationsMap postProcessOperationsWithModels(
        ...
        for (CodegenOperation op : objs.getOperations().getOperation()) {
            ...
            if (this.contentTypeVersion != null) {
                ...
                // use content type data to change dataType/baseType variable depending on specified
                // version
                                ...
                                for (Map.Entry<String, CodegenMediaType> ite2 :
                                    ...
                                    if (!ite2.getKey().startsWith(this.contentTypeVersion)) {
                                        if (codegenParameter.isArray) {
                                            ...
                                            unusedModels.add(
                                                    ite2.getValue().getSchema().getDataType());
                                        }
                                    }
                                }
                                break;
            ...
            for (CodegenParameter param : op.allParams) {
                ...
                if (param.isEnum) {
                    param.baseName =
                            param.baseName.substring(0, 1).toUpperCase(Locale.getDefault())
            ...
            for (CodegenResponse response : op.responses) {
                if (response.isModel) {
                    response.dataType =
                            response.dataType.substring(0, 1).toUpperCase(Locale.getDefault())
                                    + response.dataType.substring(1);
                }

                if (response.isArray) {
                    response.baseType =
                            response.baseType.substring(0, 1).toUpperCase(Locale.getDefault())
        ...
        for (ModelMap mo : objs.getModels()) {
            ...
            for (CodegenProperty var : model.vars) {
                ...
                if ("string".equals(var.openApiType)) {
                    var.baseType =
                            var.baseType.substring(0, 1).toLowerCase(Locale.getDefault())
                                    + var.baseType.substring(1);
                }
                // if string enum, uppercase 'name' to have a reference to wsdl simpletype
                if (var.isEnum) {
                    var.name =
                            var.name.substring(0, 1).toUpperCase(Locale.getDefault())
                ...
                if ("array".equals(var.openApiType) && var.items.dataType.startsWith("oneOf<")) {
                    ...
                    List<String> oneofSchemas =
                            new ArrayList<String>(
                                    Arrays.asList(schemaNamesString.split("\\s*,\\s*")));
    ...
    public String generateOperationId(CodegenOperation op) {
        ...
        for (int i = 0; i < pathElements.length; i++) {
            ...
            if (pathElements[i].length() > 0) {
                newOperationid =
                        newOperationid + this.lowerCaseStringExceptFirstLetter(pathElements[i]);
            }
        }
        ...
        if (pathParameters.size() > 0) {
            for (int i = 0; i < pathParameters.size(); i++) {
                ...
                pathParameter =
                        this.lowerCaseStringExceptFirstLetter(
                                pathParameter.substring(1, pathParameter.length() - 1));
                if (i == 0) {
