...
public class ScalaGatlingCodegen extends AbstractScalaCodegen implements CodegenConfig {
    ...
    protected String resourceFolder =
            "src" + File.separator + "gatling" + File.separator + "resources";
    ...
    public ScalaGatlingCodegen() {
        ...
        // Although the generator supports authorization, it's done via manual header modification
        // and it's done
        // globally. This means it doesn't _technically_ support auth per OpenAPI Spec (which would
        // allow, for example, a different API key per operation),
        ...
        supportingFiles.add(
                new SupportingFile("default.conf.mustache", confFolder, "default.conf"));
        supportingFiles.add(new SupportingFile("default.conf.mustache", confFolder, "CI.conf"));
        supportingFiles.add(new SupportingFile("default.conf.mustache", confFolder, "CD.conf"));
        supportingFiles.add(new SupportingFile("default.conf.mustache", confFolder, "stress.conf"));
        supportingFiles.add(
                new SupportingFile("default.conf.mustache", confFolder, "baseline.conf"));
        supportingFiles.add(
                new SupportingFile("default.conf.mustache", confFolder, "longevity.conf"));
    ...
    /**
     * Gatling does not need the models to have escaped words as it builds models dynamically
     * instead of through an instance of the object.
     *
    ...
    private void prepareGatlingData(
            Operation operation, Set<Parameter> parameters, String parameterType) {
        if (parameters.size() > 0) {
            ...
            operation.addExtension(
                    "x-gatling-" + parameterType.toLowerCase(Locale.ROOT) + "-params", vendorList);
            operation.addExtension(
                    "x-gatling-" + parameterType.toLowerCase(Locale.ROOT) + "-feeder",
            ...
            try {
                ...
                LOGGER.error(
                        "Could not create feeder file for operationId" + operation.getOperationId(),
                        ioe);
            }
        }
    }
