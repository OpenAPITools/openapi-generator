...
public class JavaCXFExtServerCodegen extends JavaCXFServerCodegen implements CXFExtServerFeatures {
    class CodegenVariable {
        ...
        private CodegenVariable(
            ...
            items =
                    param.items == null
                            ? null
                            : new CodegenVariable(this, param.items, null, models);
        }
        ...
        private void appendPath(StringBuilder path, boolean includeIndexes) {
            if (parent == null) path.append(testDataPath);
            else parent.appendPath(path, includeIndexes);
            if (!isListItem()) path.append('/').append(name);
            if (includeIndexes && isIndexed()) path.append('/').append(index);
        }

        String getComponentType() {
            return isArray
                    ? dataType.substring(0, dataType.length() - 2)
                    : (isContainer ? items : this).dataType;
        }

        private JsonPointer getPointer(
                String suffix, boolean includeIndexes, boolean includeLastIndex) {
            StringBuilder path = new StringBuilder();
            appendPath(path, includeIndexes);
            if (includeIndexes && !includeLastIndex && isIndexed())
                path.setLength(path.lastIndexOf("/"));
            if (suffix != null) path.append('/').append(suffix);
            return JsonPointer.compile(path.toString());
        }

        private void init(
                CodegenVariable parent, String testDataPath, Map<String, CodegenModel> models) {
            this.parent = parent;
            ...
            try {
                if ((isArray || isContainer) && testDataControlCache != null)
                    this.itemCount =
                            testDataControlCache.getInt(
                                    getPointer("testItemCount", false, false), 1);
            } catch (CacheException e) {
    ...
    // SimpleDateFormat is not thread-safe, and may not be stored in a static field unless stored by
    // ThreadLocal.
    // It's not enough to add a ThreadLocal at the usage site.
    @SuppressWarnings("squid:S5164")
    private static final ThreadLocal<SimpleDateFormat> ISO8601_DATE_FORMAT =
    ...
    // It's not enough to add a ThreadLocal at the usage site.
    @SuppressWarnings("squid:S5164")
    private static final ThreadLocal<SimpleDateFormat> ISO8601_DATETIME_FORMAT =
            ThreadLocal.withInitial(
                    () -> {
                        SimpleDateFormat f =
    ...
    private static final Collection<String> DATE_TYPES =
            Arrays.asList("Date", "DateTime", "OffsetDateTime", "LocalDateTime", "LocalDate");
    ...
    public JavaCXFExtServerCodegen() {
        ...
        embeddedTemplateDir =
                templateDir = JAXRS_TEMPLATE_DIRECTORY_NAME + File.separator + "cxf-ext";
        cliOptions.add(
                CliOption.newBoolean(
                        SUPPORT_MULTIPLE_SPRING_SERVICES,
                        "Support generation of Spring services from multiple specifications"));
        cliOptions.add(
                CliOption.newBoolean(
                        GENERATE_OPERATION_BODY, "Generate fully functional operation bodies"));
        cliOptions.add(
                CliOption.newBoolean(
                        LOAD_TEST_DATA_FROM_FILE, "Load test data from a generated JSON file"));
        cliOptions.add(
                CliOption.newString(TEST_DATA_FILE, "JSON file to contain generated test data"));
        cliOptions.add(
                CliOption.newString(
                        TEST_DATA_CONTROL_FILE, "JSON file to control test data generation"));
    }
    ...
    private void appendArrayValue(
        ...
        if (var.dataType.equals("byte[]")) {
            ...
            int itemCount =
                    Math.max(var.itemCount, var.minItems == null ? 1 : Math.max(1, var.minItems));
            if (!loadTestDataFromFile) {
                buffer.append("new ").append(var.getComponentType());
                if (isPrimitiveType) buffer.append("[] {");
                else buffer.append('[').append(itemCount).append("];");
            }
            var.index = var.size();
            for (int i = var.index; i < itemCount; i++) {
                if (isPrimitiveType) {
                    ...
                    if (i < itemCount - 1 && !loadTestDataFromFile) buffer.append(", ");
                } else {
                    String itemVar =
                            appendLocalVariable(buffer, indent, op, var.items, localVars, models);
                    if (!loadTestDataFromFile) {
                        buffer.append(NL)
                                .append(indent)
                                .append(localVar)
            ...
            var.index = 0;
            if (isPrimitiveType && !loadTestDataFromFile) buffer.append("};");
        }
    }

    private void appendByteArrayValue(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        if (!loadTestDataFromFile) buffer.append('"');
        short min =
                var == null || var.minimum == null ? Byte.MIN_VALUE : Byte.parseByte(var.minimum);
        short max =
                var == null || var.maximum == null ? Byte.MAX_VALUE : Byte.parseByte(var.maximum);
        ...
        if (loadTestDataFromFile && var != null) var.addTestData(randomBytesBase64);
        else buffer.append('"');
    }

    private void appendListValue(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        if (!loadTestDataFromFile) {
            ...
            if (supportJava6) buffer.append(var.dataType);
            buffer.append(">();");
        }
        var.index = var.size();
        int itemCount =
     ...
     */
    private String appendLocalVariable(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        // Ensure that we're using a unique local variable name (to avoid typing and overwriting
        // conflicts).
        String localVar = var.name;
        for (int i = 2; localVars.contains(localVar); i++) localVar = var.name + i;
        ...
        if (!loadTestDataFromFile) appendSemicolon(buffer);
    ...
    private void appendMapValue(
        ...
        if (!loadTestDataFromFile) {
            ...
            if (supportJava6) buffer.append("String, ").append(var.dataType);
            buffer.append(">();");
        }
        var.index = var.size();
        int itemCount =
    ...
    private void appendObjectValue(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        if ("Object".equals(var.dataType)) {
            // Jackson can't serialize java.lang.Object, so we'll provide an empty JSON ObjectNode
            // instead.
            if (loadTestDataFromFile) var.addTestData(JsonNodeFactory.instance.objectNode());
            else buffer.append("JsonNodeFactory.instance.objectNode();");
        } else {
            if (needToImport(var.dataType)) op.imports.add(var.dataType);
            if (!loadTestDataFromFile) buffer.append("new ").append(var.dataType).append("();");
            appendPropertyAssignments(buffer, indent, op, var, localVar, localVars, models);
        }
    }

    private void appendPropertyAssignments(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable parent,
        ...
        if (cm != null) { // TODO: handle isArrayModel and isMap
            for (CodegenProperty cp : cm.allVars) {
                ...
                if (var.isContainer || !var.isPrimitiveType) {
                    String containerVar =
                            appendLocalVariable(buffer, indent, op, var, localVars, models);
                    if (!loadTestDataFromFile) {
                        buffer.append(NL)
                                .append(indent)
                                .append(localVar)
                    ...
                    // No need to use a local variable for types which can be initialised with a
                    // single expression.
                    if (!loadTestDataFromFile)
                        buffer.append(NL)
                                .append(indent)
    ...
    private void appendRandomBoolean(
            StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        boolean randomBoolean = Math.random() > 0.5;

        if (loadTestDataFromFile) var.addTestData(randomBoolean);
        else buffer.append(randomBoolean);
    }
    ...
    private void appendRandomByte(StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            short min =
                    var == null || var.minimum == null
                            ? Byte.MIN_VALUE
                            : Byte.parseByte(var.minimum);
            short max =
                    var == null || var.maximum == null
                            ? Byte.MAX_VALUE
                            : Byte.parseByte(var.maximum);
            short exclusiveMin = (short) (var != null && var.exclusiveMinimum ? 1 : 0);
            short inclusiveMax = (short) (var == null || !var.exclusiveMaximum ? 1 : 0);
            byte randomByte =
                    (byte)
    ...
    private void appendRandomDate(StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            if (var != null) {
                DateFormat df =
                        var.dataFormat.equals("date-time")
                                ? ISO8601_DATETIME_FORMAT.get()
                                : ISO8601_DATE_FORMAT.get();
                ...
                if (var.minimum != null) {
                    try {
                        ...
                        // Ignore, use MIN_DATE.
                        LOGGER.warn(
                                "Could not parse minimum {} value for '{}/{}' as an ISO-8601 {}: {}",
                                var.dataFormat,
                                op.operationId,
                                var.name,
                                isoFormat,
                ...
                if (var.maximum != null) {
                    try {
                        ...
                        // Ignore, use MAX_DATE.
                        LOGGER.warn(
                                "Could not parse maximum {} value for '{}/{}' as an ISO-8601 {}: {}",
                                var.dataFormat,
                                op.operationId,
                                var.name,
                                isoFormat,
            ...
            BigDecimal exclusiveMinLong =
                    new BigDecimal(var != null && var.exclusiveMinimum ? 1 : 0);
            BigDecimal inclusiveMaxLong =
                    new BigDecimal(var == null || !var.exclusiveMaximum ? 1 : 0);
            long randomDateLong =
                    minLong.add(exclusiveMinLong)
                            .add(
                                    maxLong.add(inclusiveMaxLong)
                                            .subtract(minLong)
                                            .subtract(exclusiveMinLong)
            ...
            // NOTE: By default Jackson serializes Date as long milliseconds since epoch date, but
            // that conflicts with
            // the OpenAPI 2.0/3.0 specs, which mandates the ISO-8601 full-date or date-time
            // formats. Accordingly, date
            // and date-time fields are annotated with @JsonFormat to specify the appropriate ISO
            // format.
            if (loadTestDataFromFile) {
                if (var != null) {
    ...
    private void appendRandomDouble(
            StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            BigDecimal randomBigDecimal =
                    min.add(exclusiveMin)
                            .add(
                                    max.add(inclusiveMax)
                                            .subtract(min)
                                            .subtract(exclusiveMin)
                                            .multiply(
                                                    new BigDecimal(String.valueOf(Math.random()))));
    ...
    private boolean appendRandomEnum(
            StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (var != null && var.allowableValues != null) {
            ...
            if (definingClass != null) {
                ...
                List<Map<String, Object>> enumVars =
                        (List<Map<String, Object>>) var.allowableValues.get("enumVars");
                if (enumVars != null) {
                    if (!loadTestDataFromFile) {
                        Map<String, Object> randomEnumVar = enumVars.get(i);
                        // NOTE: to disambiguate identically named inner enums, qualify enum name
                        // with defining class.
                        buffer.append(definingClass)
                                .append('.')
                                .append(var.enumName)
                                .append('.')
    ...
    private void appendRandomFile(StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        // For code generation purposes we'll just supply a random string as the contents of a text
        // 'file'.
    ...
    private void appendRandomInt(StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            long min =
                    var == null || var.minimum == null
                            ? Integer.MIN_VALUE
                            : Integer.parseInt(var.minimum);
            long max =
                    var == null || var.maximum == null
                            ? Integer.MAX_VALUE
                            : Integer.parseInt(var.maximum);
        ...
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            BigDecimal min =
                    new BigDecimal(
                            var == null || var.minimum == null
                                    ? Long.MIN_VALUE
                                    : Long.parseLong(var.minimum));
            BigDecimal max =
                    new BigDecimal(
                            var == null || var.maximum == null
                                    ? Long.MAX_VALUE
                                    : Long.parseLong(var.maximum));
            BigDecimal exclusiveMin = new BigDecimal(var != null && var.exclusiveMinimum ? 1 : 0);
            BigDecimal inclusiveMax = new BigDecimal(var == null || !var.exclusiveMaximum ? 1 : 0);
            long randomLong =
                    min.add(exclusiveMin)
    ...
    private void appendRandomShort(StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            ...
            int min =
                    var == null || var.minimum == null
                            ? Short.MIN_VALUE
                            : Short.parseShort(var.minimum);
            int max =
                    var == null || var.maximum == null
                            ? Short.MAX_VALUE
                            : Short.parseShort(var.maximum);
    ...
    private void appendRandomString(
            StringBuilder buffer, CodegenOperation op, CodegenVariable var) {
        if (!appendRandomEnum(buffer, op, var)) {
            String randomString = generateRandomString(var);
    ...
    /**
     * Appends a sample value for a scalar type - that is, one which is neither a list nor a map
     * container.
     *
     ...
     */
    private void appendScalarValue(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        if (!var.isPrimitiveType && !DATE_TYPES.contains(var.dataType)
                || var.dataType.equals("Object")) {
            ...
            // NOTE: this loop is only iterated once unless var.isArray is true and var.type is not
            // byte[].
            int itemCount =
                    Math.max(var.itemCount, var.minItems == null ? 1 : Math.max(1, var.minItems));
            for (int i = 0; i < itemCount; i++) {
                    ...
                    default:
                        LOGGER.warn(
                                "Unrecognized component type '{}"
                                        + "' in '{}' property for '{}'operation",
                                var.getComponentType(),
                                var.name,
                                op.operationId);
                }
                if (!loadTestDataFromFile && i < itemCount - 1) buffer.append(", ");
            }
    ...
    private StringBuilder appendValue(
            StringBuilder buffer,
            String indent,
            CodegenOperation op,
            CodegenVariable var,
        ...
        else if (var.isMap) appendMapValue(buffer, indent, op, var, localVar, localVars, models);
        else if (var.isArray)
            appendArrayValue(buffer, indent, op, var, localVar, localVars, models);
        else appendScalarValue(buffer, indent, op, var, localVar, localVars, models);

        return buffer;
    }

    /**
     * Applies appropriate default consumes and/or produces content type(s). Affects operations
     * which do not specify a content type for consumes and/or produces and respectively have a body
     * parameter and/or a return value. This is necessary since none of the configured CXF or
     * Jackson JAX-RS providers support the inferred default content type of <code>&ast;/*</code>.
     *
     * @param op
     */
    private void applyDefaultContentTypes(CodegenOperation op) {
        if (op.bodyParam != null && !op.hasConsumes) {
            ...
            else if (bodyParam.isBinary || bodyParam.isFile) mediaType = "application/octet-stream";
            else mediaType = "text/plain";
            Map<String, String> contentType = new HashMap<>();
            contentType.put("mediaType", mediaType);
            if (op.consumes == null) op.consumes = new ArrayList<>();
        ...
        if (!"void".equals(op.returnType) && !op.hasProduces) {
            ...
            Map<String, String> contentType = new HashMap<>();
            contentType.put("mediaType", mediaType);
            if (op.produces == null) op.produces = new ArrayList<>();
    ...
    private void closeValueBuffer(StringBuilder buffer, CodegenVariable var) {
        if (var.isArray && !loadTestDataFromFile) buffer.append('}');
    }
    ...
    private String patternFor(CodegenVariable var) {
        ...
        if (var != null) {
            if (var.pattern != null) {
                ...
                // string-type values can be constrained by a format, one of: {date-time, email,
                // hostname, ipv4, ipv6,
                // uri}. Custom formats are allowed. The OpenAPI Specification also mentions binary,
                // byte, date,
                // password, uuid.
                switch (var.dataFormat) {
                    case "binary":
                    ...
                    case "date":
                        ...
                        pattern =
                                "20\\d{2}-(?:" // YY- (20yy)
                                        ...
                                        + "(?:04|06|09|11)-(?:0[1-9]|[1-2][0-9]|30)|" // MM-DD (30
                                        // days)
                                        + "02-(?:0[1-9]|1[0-9]|2[0-8]))"; // MM-DD (28 days)
                        break;
                    case "date-time":
                        // ISO-8601: YYYY-MM-DDTHH:mm:ss(Z|+-HH:mm) NOTE: the time zone offset is
                        // mandatory.
                        pattern =
                                "20\\d{2}-(?:" // YYYY-
                                        + "(?:01|03|05|07|08|10|12)-(?:0[1-9]|[1-2][0-9]|3[0-1])|" // MM-DD (31 days)
                    ...
                    case "email":
                        pattern =
                                "[a-z][a-z0-9_.-]{1,8}@[a-z][a-z0-9-.]{2,12}\\.[a-z]{2,4}"; // (simplistic but
                        // sufficient)
                        break;
                    case "hostname":
                        pattern =
                                "[a-z][a-z0-9-.]{2,12}\\.[a-z]{2,4}"; // (simplistic but sufficient)
                        break;
                    case "ipv4":
                        pattern =
                                "(?:(?:25[0-5]|2[0-4][0-9]|[1-9][0-9]|[0-9])\\.){3}"
                    ...
                    case "uuid":
                        pattern =
                                "[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}"; // 8-4-4-4-12
                        break;
                    case "password":
                        // Fall through.
                    default:
    ...
    public Map<String, ModelsMap> postProcessAllModels(Map<String, ModelsMap> objs) {
        ...
        // When populating operation bodies we need to import enum types, which requires the class
        // that defines them.
        if (generateOperationBody) {
            for (ModelsMap value : objs.values()) {
                for (ModelMap mo : value.getModels()) postProcessModel(mo.getModel());
            }
        }
    ...
    private void postProcessModel(CodegenModel cm) {
        // NOTE: if supportsInheritance is false, cm.vars == cm.allVars so we only have to update
        // one list.
        ...
        if (supportsInheritance) {
            ...
            if (cm.parentModel != null) postProcessModel(cm.parentModel);
        }
    }
    ...
    public OperationsMap postProcessOperationsWithModels(
        ...
        if (generateOperationBody) {
            // We generate the operation body in code because the logic to do so is far too
            // complicated to be expressed
            ...
            if (operations != null) {
                ...
                for (CodegenOperation op : ops) {
                    ...
                    for (CodegenParameter cp : op.allParams) {
                        ...
                        String localVar =
                                appendLocalVariable(
                                        buffer, INDENT, op, var, new ArrayList<>(), models);
                        if (loadTestDataFromFile) {
                            buffer.append(NL)
                                    .append(INDENT)
                                    .append(var.dataType)
                                    ...
                                    .append(localVar)
                                    .append(" = cache.")
                                    .append(getCacheMethod(var))
                                    .append("(\"/")
                                    .append(op.operationId)
                                    .append('/')
                                    .append(var.name)
                                    .append('"');
                            if (var.isListContainer)
                                buffer.append(", ").append(var.getComponentType()).append(".class");
                            else if (var.isMap) buffer.append(", Map.class");
                    ...
                    if (!(Boolean)
                            op.vendorExtensions.getOrDefault("x-java-is-response-void", false)) {
                        ...
                        if (loadTestDataFromFile) {
                            ...
                            buffer.append(");") // split
                                    ...
                                    .append(NL)
                                    .append(INDENT)
                                    .append("} catch (CacheException e) {") // split
                                    .append(NL)
                                    .append(INDENT)
                                    .append("    throw new RuntimeException(e);") // split
                                    .append(NL)
                                    .append(INDENT)
                                    .append("}");
                        } else {
                        ...
                        if (buffer.indexOf(NL) == 0) buffer.delete(0, NL.length());
                        op.vendorExtensions.put("x-java-operation-body", buffer.toString());
                    }
                }

                // DefaultGenerator already processed all the imports from the generated operations,
                // but these imports
                // did not include the ones we've just added to support the code in the operation
                // bodies. Therefore it
                // is necessary to recompute the imports and overwrite the existing ones. The code
    ...
    public Map<String, Object> postProcessSupportingFileData(Map<String, Object> objs) {
        ...
        if (loadTestDataFromFile) {
            // DefaultGenerator doesn't provide any mechanism for emitting supporting files other
            // than by a Mustache
            // template, so we're obliged to serialize the caches to JSON strings and use templates
            // to write them.
            try {
                if (testDataCache.root().isDirty()) {
                    ...
                    supportingFiles.add(
                            new SupportingFile(
                                    "testData.mustache", testDataFile.getAbsolutePath()));
                }
            ...
            try {
                if (testDataControlCache.root().isDirty()) {
                    ...
                    supportingFiles.add(
                            new SupportingFile(
                                    "testDataControl.mustache",
                                    testDataControlFile.getAbsolutePath()));
                }
            } catch (CacheException | UnsupportedEncodingException e) {
    ...
    public void processOpts() {
        ...
        if (additionalProperties.containsKey(GENERATE_OPERATION_BODY)) {
            boolean generateOperationBody =
                    convertPropertyToBooleanAndWriteBack(GENERATE_OPERATION_BODY);
            this.setGenerateOperationBody(generateOperationBody);

            boolean loadTestDataFromFile =
                    convertPropertyToBooleanAndWriteBack(LOAD_TEST_DATA_FROM_FILE);
            ...
            if (loadTestDataFromFile) {
                ...
                else testDataFileStr = outputFolder() + "/src/main/resources/test-data.json";
                ...
                try {
                    ...
                    additionalProperties.put(
                            TEST_DATA_FILE, testDataFileStr.replaceAll("\\\\", "\\\\\\\\"));
                ...
                testDataCache =
                        JsonCache.Factory.instance
                                .get("test-data")
                                .mergePolicy(MergePolicy.KEEP_EXISTING)
                ...
                if (additionalProperties.containsKey(TEST_DATA_CONTROL_FILE))
                    testDataControlFileStr =
                            (String) additionalProperties.get(TEST_DATA_CONTROL_FILE);
                else testDataControlFileStr = outputFolder() + "/test-data-control.json";
                ...
                if (this.testDataControlFile.exists()) {
                    try {
                        ...
                        LOGGER.error(
                                "Unable to load test data control file " + testDataControlFileStr,
                                e);
                    }
                }
            }
