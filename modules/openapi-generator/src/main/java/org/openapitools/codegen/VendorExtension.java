...
public enum VendorExtension {
    ...
    X_ITEMS_EXTRA_ANNOTATION("x-items-extra-annotation", ExtensionLevel.FIELD, "Custom annotation to be placed on the type argument of a collection (List, Set), set on the array property itself. Typically used for per-element bean validation constraints (e.g. `@NotNull`), but accepts any type-use annotation.", null),
