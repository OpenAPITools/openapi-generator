...
public enum VendorExtension {
    ...
    X_REQUEST_BODY_EXTRA_ANNOTATION("x-request-body-extra-annotation", ExtensionLevel.OPERATION, "Custom annotation(s) to be added to the request body parameter; accepts a string or list of strings. Declared on the operation because the request body typically `$ref`s a shared model (so the annotation cannot be placed next to the `$ref`); the value is rendered by being merged into the body parameter's `x-field-extra-annotation`", null),
