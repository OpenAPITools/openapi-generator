...
* For the extra-annotation extensions, put multiple annotations in one value separated by *spaces*
  ...
  Commas inside a value need no escaping in any form (Gradle map, CLI, or Maven), so
  `@Size(min = 0, max = 10)` works directly; each CLI/Maven occurrence of
  `--inject-model-vendor-extensions`/`--inject-operation-vendor-extensions` is exactly one
  key=value pair (everything after the first `=` is taken literally). Use multiple occurrences
  for multiple distinct targets, or repeat the *same* key across occurrences to append another
  annotation to it (e.g. `@Foo` then `@Bar` becomes `@Foo @Bar`) instead of the later one
