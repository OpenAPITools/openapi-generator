...
public class AuthorTemplate extends OpenApiGeneratorCommand {
    ...
    void execute() {
        ...
        log(
                "Requesting '{}' from embedded resource directory '{}'",
                generatorName,
                templateDirectory);
        ...
        try {
            URI uri =
                    Objects.requireNonNull(
                                    this.getClass().getClassLoader().getResource(templateDirectory))
                            .toURI();

            if ("jar".equals(uri.getScheme())) {
                Optional<FileSystemProvider> provider =
            ...
            if (StringUtils.isNotEmpty(library) && !generatedFiles.isEmpty()) {
                ...
                if (!Files.isDirectory(targetLibrary)) {
                    LOGGER.warn(
                            "The library '{}' was not extracted. Please verify the spelling and retry.",
                            targetLibrary);
                }
                        ...
                        .forEach(
                                p -> {
                                    if (p.startsWith(targetLibrary)) {
                                        // We don't care about empty directories, and not need to
                                        // check directory for files.
                                        if (!Files.isDirectory(p)) {
                                            // warn if the file was not written
                                            if (Files.notExists(p)) {
                                                LOGGER.warn(
                                                        "An expected library file was not extracted: {}",
                                                        p.toAbsolutePath());
                                            }
                                        }
                                    } else {
                                        LOGGER.warn(
                                                "The library filter '{}' extracted an unexpected library path: {}",
                                                library,
                                                p.toAbsolutePath());
                                    }
                                });
            ...
            LOGGER.info(
                    "Extracted templates to '{}' directory. Refer to https://openapi-generator.tech/docs/templating for customization details.",
                    outputDirPath);
