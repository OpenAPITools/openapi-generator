...
public class ListGenerators extends OpenApiGeneratorCommand {
    ...
    @Option(
            name = {"-d", "--docsite"},
            description = "format for docusaurus site output",
            hidden = true)
    private Boolean docusaurus = false;

    @Option(
            name = {"--github-nested-index"},
            description = "format for github index at docs/generators/README.md",
            hidden = true)
    ...
    @Option(
            name = {"-i", "--include"},
            description =
                    "comma-separated list of stability indexes to include (value: all,beta,stable,experimental,deprecated). Excludes deprecated by default.",
    ...
    private void appendForType(
            StringBuilder sb, CodegenType type, String typeName, List<CodegenConfig> generators) {
        List<CodegenConfig> list =
                generators.stream()
            ...
            list.forEach(
                    generator -> {
                        ...
                        if (docusaurus || githubNestedIndex) {
                            ...
                            if (meta != null
                                    && meta.getStability() != null
                                    && meta.getStability() != Stability.STABLE) {
