...
/**
 * A delegate to be called by the {@link GenApiController}}. Implement this interface with a {@link
...
public interface GenApiDelegate {
    ...
    /**
     * @param language The target generator (language is a misnomer here, but kept for API
     *     consistency).
     * @param generatorInput The configuration settings to be used during client generation.
     * @see GenApi#generateClient
     * @return A {@link ResponseEntity} referencing the unique download id and a link to download
                            ...
                            for (MediaType mediaType :
                                ...
                                if (mediaType.isCompatibleWith(MediaType.valueOf("*/*"))) {
                                    ApiUtil.setExampleResponse(
                                            request,
                                            "*/*",
                                            "{  \"code\" : \"d40029be-eda6-4d62-b1ef-d05e2e91a72a\",  \"link\" : \"http://localhost:80/api/gen/download/d40029be-eda6-4d62-b1ef-d05e2e91a72a\"}");
                                    break;
                                }
                            }
                        });
    ...
    /**
     * @param framework The target generator name (framework is a slight misnomer here, as we may
     *     have a framework like Spring implemented in multiple languages).
     * @param generatorInput The configuration settings to be used during server generation.
     * @see GenApi#generateServerForLanguage
     * @return A {@link ResponseEntity} referencing the unique download id and a link to download
     *     the requested server code.
     */
